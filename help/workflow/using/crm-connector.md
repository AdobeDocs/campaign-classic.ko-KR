...
**CRM 커넥터**&#x200B;에서는 Adobe Campaign과 CRM 간의 데이터 동기화를 구성할 수 있습니다.
...
1. **[!UICONTROL Remote fields]** 섹션으로 이동하여 가져올 필드를 입력합니다.
   ...
   필요한 경우 **[!UICONTROL Conversion]** 열의 드롭다운 목록을 통해 데이터 형식을 변경합니다. 가능한 전환 유형은 이 [페이지](../../platform/using/crm-connectors.md#data-format)에 자세히 설명되어 있습니다.
...
1. 필요에 따라 가져올 데이터를 필터링할 수도 있습니다. 이렇게 하려면 **[!UICONTROL Edit the filter...]** 링크를 클릭합니다.
   ...
   다음 예에서 Adobe Campaign은 2012년 7월 13일 이후 일부 활동이 기록된 연락처만 가져옵니다.

   ![](assets/crm_task_import_filter.png)

   데이터 필터링 모드에 연결된 제한 사항은 [데이터](#filter-on-data) 섹션의 필터에서 자세히 설명합니다.
...
## 데이터 {#filter-on-data} 필터

다양한 CRM을 사용하여 효율적인 작업을 수행하려면 다음 규칙을 사용하여 필터를 만들어야 합니다.

* 각 필터링 수준은 논리 연산자 유형을 하나만 사용할 수 있습니다.
* EXCEPT(AND NOT) 연산자는 지원되지 않습니다.
* 비교는 null 값(&#39;is empty&#39;/&#39;is not empty&#39; 유형이 아님) 또는 숫자만 고려해야 할 수 있습니다. 즉, **[!UICONTROL Value]** 열(오른쪽 열)을 평가한 후 이 평가의 결과는 숫자여야 합니다.
* **[!UICONTROL Value]** 열의 데이터는 JavaScript로 평가됩니다.
* JOIN 비교는 지원되지 않습니다.
* 왼쪽 열의 표현식은 필드여야 합니다. 여러 표현식, 숫자 등의 조합일 수 없습니다.

예를 들어, 다음과 같은 이유로 아래 설명된 필터링 조건이 CRM 가져오기에 유효하지 않습니다.
...
1. **[!UICONTROL Mapping]** 섹션에서 내보낼 필드와 CRM에서 해당 매핑을 지정합니다.
   ...
   필요한 경우 **[!UICONTROL Conversion]** 열의 드롭다운 목록을 통해 데이터 형식을 변경합니다. 가능한 전환 유형은 이 [섹션](../../platform/using/crm-connectors.md#data-format)에 자세히 설명되어 있습니다.
...
## 데이터 형식 및 {#data-format-and-error-processing} 처리 오류
...
인스턴스에 대한 오류를 처리하려면 대기 활동을 추가하고 워크플로 재시도를 예약할 수 있습니다.
...
**[!UICONTROL Reject]** 아웃바운드 전환을 사용하여 오류 메시지 및 코드와 관련된 특정 열이 포함된 출력 스키마에 액세스할 수 있습니다. 이러한 열은 다음과 같습니다.
...
**[!UICONTROL Behavior]** 탭에서는 거부 처리를 활성화할 수 있습니다. 이 옵션은 **[!UICONTROL CRM connector]** 활동에 대한 두 번째 출력 전환을 생성합니다. 자세한 정보는 이 [섹션](../../platform/using/crm-connectors.md#error-processing)을 참조하십시오.

**[!UICONTROL Process rejects]** 옵션이 비활성화된 경우에도 거부된 각 열에 대한 경고가 생성됩니다.

## 연락처 가져오기 {#example-of-how-to-configure-a-contact-import}를 구성하는 방법의 예

다음 예에서 활동은 Oracle On Demand CRM에서 연락처를 가져오도록 구성됩니다. 가져오기 전에 CRM 필드가 Adobe Campaign 데이터베이스에 이미 있는 필드와 일치하도록 선택되므로
