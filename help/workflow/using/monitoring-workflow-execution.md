...
이 **[!UICONTROL Display progress information]** 아이콘을 사용하면 실행 화면에 상태 및 활동 결과를 표시할 수 있습니다.

![](assets/s_user_segmentation_toolbar_progr.png)

이 옵션을 선택하면 실행된 활동이 파란색으로 표시되고 보류 중인 활동이 깜박이며 경고가 주황색으로 표시되고 오류는 빨간색으로 표시됩니다. 이 옵션은 활동 속성에 정의된 결과 레이블과 1초를 초과하는 작업의 지속 시간이 뒤에 아웃바운드 전환에서의 활동 결과도 표시합니다

![](assets/s_user_segmentation_results.png)

## 로그 표시 {#displaying-logs}

로그에는 워크플로우의 내역 또는 감사 추적이 포함됩니다. 모든 사용자 작업, 수행된 모든 작업 및 발생한 오류를 등록합니다. 다음을 수행할 수 있습니다.

* 세부 사항 **[!UICONTROL Tracking]** 에서 탭을 선택합니다. 이 목록에는 모든 워크플로우 메시지가 포함되어 있습니다.

   ![](assets/new-workflow-display-log-tab.png)

* 활동별로 로그 메시지를 필터링합니다. 이를 수행하려면 다이어그램 **[!UICONTROL Display the tasks and the log]** 의 도구 모음에서 다이어그램 아래 **[!UICONTROL Log]** 와 **[!UICONTROL Tasks]** 탭을 표시하려면 을(를) 클릭합니다. 모든 관련 메시지를 보려면 활동을 선택합니다. 이 목록에는 활동이 선택되지 않은 경우 모든 메시지가 포함됩니다.
...
* 지정된 작업에 연결된 메시지만 표시합니다. 이렇게 하려면 **[!UICONTROL Tasks]** 탭을 선택한 다음 다이어그램에서 활동을 선택하여 목록을 제한합니다. 작업을 두 번 클릭하여 정보를 표시합니다. 창의 마지막 탭에는 로그가 포함되어 있습니다.
   ...
   이 **[!UICONTROL Details...]** 단추를 사용하면 활동 실행에 대한 모든 추가 정보를 표시할 수 있습니다. 예를 들어 다음 예제와 같이 유효성 검사 연산자와 승인 중에 입력한 주석을 볼 수 있습니다.

   ![](assets/new-workflow-display-tasks-activity-details.png)

>[!NOTE]
>
>워크플로우를 다시 시작하면 로그가 삭제되지 않습니다. 모든 메시지가 유지됩니다. 이전 실행의 메시지를 삭제하려면 내역을 삭제해야 합니다.
...
* 활동 로그
   ...
   실행 로그와 각 활동의 세부 사항을 볼 수도 있습니다. 다음 두 가지 방법으로 작업을 수행할 수 있습니다.
   ...
   1. 타깃팅된 활동을 마우스 오른쪽 단추로 클릭하고 선택합니다 **[!UICONTROL Display logs]**.
...
워크플로가 수동적으로 진행되면 워크플로우의 작업 테이블이 자동으로 삭제됩니다. 작업 테이블을 유지하려면 작업을 통해 목록에 **[!UICONTROL List update]** 저장합니다(목록 업데이트 [참조](../../workflow/using/list-update.md)).

## 오류 관리 {#managing-errors}

오류가 발생하면 워크플로우가 일시 중지되고 오류가 발생했을 때 실행 중인 활동이 빨간색으로 표시됩니다. 워크플로우 개요(**[!UICONTROL Monitoring]** 우주 > **[!UICONTROL Workflows]** 링크)에서는 아래와 같이 오류가 있는 워크플로우만 표시할 수 있습니다.
...
워크플로우가 오류가 발생하면 워크플로우 감독 그룹에 속한 운영자에게 이메일 주소가 프로필에 나열되는 한 해당 운영자에게 이메일로 알림을 보냅니다. 이 그룹은 워크플로우 속성 **[!UICONTROL Supervisor(s)]** 필드에서 선택됩니다.
...
오류가 발생할 경우 일시 중지되지 않고 실행을 계속하도록 워크플로우를 구성할 수 있습니다. 이렇게 하려면 워크플로우를 편집하고 **[!UICONTROL Properties]****[!UICONTROL Error management]** 섹션에서 **[!UICONTROL Ignore]** 필드 **[!UICONTROL In case of error]** 의 옵션을 선택합니다. 그런 다음 프로세스를 일시 중지하기 전에 무시할 수 있는 연속 오류 수를 지정할 수 있습니다.

이 경우 오류 작업이 중단됩니다. 이 모드는 캠페인을 나중에 다시 시도하도록 설계된 워크플로우(주기적 작업)에 특히 적합합니다.
...
워크플로우의 실행 문제 해결에 대한 자세한 내용은 [전용 섹션을 참조하십시오](../../production/using/workflow-execution.md).

## 처리 오류 {#processing-errors}

활동과 관련하여 이 옵션은 **[!UICONTROL Process errors]** 오류가 생성될 때 활성화되는 특정 전환을 표시합니다. 이 경우 워크플로우는 오류 모드로 전환되지 않고 계속 실행됩니다.

파일 시스템 오류(파일을 이동할 수 없음, 디렉터리에 액세스할 수 없음 등)를 고려한 오류입니다.

이 옵션은 활동 구성과 관련된 오류(예: 잘못된 값)를 처리하지 않습니다. 잘못된 구성과 관련된 오류로 인해 이 전환이 활성화되지 않습니다(디렉토리가 없음).

워크플로가 일시 중지된 경우(오류 발생 후 수동 또는 자동) 단추가 중지된 워크플로우 실행을 **[!UICONTROL Start]** 다시 시작합니다. 잘못된 활동(또는 일시 중지된 활동)이 다시 실행됩니다. 이전 활동은 다시 실행되지 않습니다.
...
모든 워크플로우를 표시하려면 **[!UICONTROL Workflows]** 링크를 클릭합니다. 드롭다운 목록을 사용하여 해당 상태에 따라 플랫폼의 워크플로우를 표시합니다.

![](assets/wf-monitoring_edit-wf.png)

오류가 있는 워크플로우의 링크를 클릭하여 열고 해당 로그를 확인합니다.

![](assets/wf-monitoring_edit-task-wf.png)

## 동시 다중 실행 방지 {#preventing-simultaneous-multiple-executions}

하나의 워크플로우에서는 동시에 여러 개의 실행 작업이 실행될 수 있습니다. 어떤 경우에는 이러한 문제가 발생하지 않도록 해야 합니다.

예를 들어 스케줄러가 매시간마다 워크플로우 실행을 트리거하도록 할 수 있지만, 경우에 따라 전체 워크플로우의 실행은 1시간 이상 걸립니다. 워크플로우가 이미 실행 중인 경우 실행을 건너뛸 수 있습니다.
...
이벤트 **[!UICONTROL Scheduler]** 가 매 분 마다 트리거됩니다. 다음 **[!UICONTROL Test]** 활동은 isRunning 인스턴스 변수 **를 테스트하여 실행을** 계속할지 여부를 결정합니다.

![](assets/wkf_simultaneous_execution2.png)

>[!NOTE]
>
>**isRunning** 은 이 예제에 대해 선택된 변수 이름입니다. 내장 변수가 아닙니다.

예 분기 **[!UICONTROL Test]** 의 다음 활동 **은** 초기화 스크립트에서 인스턴스 변수를 설정해야 합니다 ****.

```
instance.vars.isRunning = true
```

예 분기의 마지막 **활동은** 초기화 스크립트에서 변수를 false로 되돌려야 합니다 ****.
...
* 워크플로우 속성 **탭에서 인스턴스 변수의 현재 값을 확인할 수** 있습니다 ****.
* 인스턴스 변수는 워크플로우를 다시 시작하면 재설정됩니다.
* JavaScript에서 정의되지 않은 값은 테스트에서 false이므로 인스턴스 변수를 초기화하기 전에 테스트할 수 있습니다.
...
사용 사례는 다음 섹션에 나와 있습니다. [데이터 업데이트](../../workflow/using/coordinating-data-updates.md)조정
...
기본적으로 워크플로가 일시 중지된 경우 작업 테이블은 삭제되지 않습니다. 빌드 8880부터 너무 오랫동안 일시 중지된 워크플로우가 자동으로 중지되고 작업 테이블이 삭제됩니다. 이 동작은 다음과 같이 트리거됩니다.

* 7일 이후에 일시 중지된 워크플로우는 모니터링 대시보드(및 모니터링 API)에 경고로 나타나며 알림이 감시자 그룹에 전송됩니다.
* 기술 워크플로우가 트리거되는 매주 동일한 **[!UICONTROL cleanupPausedWorkflows]** 현상이 발생합니다. 워크플로우에 대한 자세한 내용은 [이 섹션을 참조하십시오](../../workflow/using/delivery.md).
* 4개의 알림 후(즉, 기본적으로 일시 중지된 상태로 1개월) 워크플로우가 조건 없이 중지됩니다. 로그가 중지된 후 워크플로우에 표시됩니다. 다음 실행 워크플로우에서 테이블이 **[!UICONTROL cleanup]** 삭제됩니다

이러한 기간은 NmsServer_PausedWorkflowPeriod 옵션을 통해 구성할 수 있습니다.

워크플로우 감독자에게 통보됩니다. 워크플로우를 수정한 작성자 및 마지막 사용자도 알림을 받습니다. 관리자는 알림을 받지 않습니다.
...
* **[!UICONTROL Paused]**: list all pausted workflows
* **[!UICONTROL Failed]**: list all failed workflows.
* **[!UICONTROL Start Pending]**: list all workflows that are waiting to be started by the operationMgt process. 이 보기는&#x200B;**마케팅 캠페인**&#x200B;패키지에서만 사용할 수 있습니다(Campaign 표준 패키지 [설치 참조](../../installation/using/installing-campaign-standard-packages.md)).

![](assets/workflow-monitoring-views.png)

기본적으로 이러한 보기는&#x200B;**[!UICONTROL Audit]**&#x200B;폴더에서 액세스할 수 있습니다. 하지만 폴더 트리에서 선택한 위치에서 다시 만들 수 있습니다. 이렇게 하면 관리 권한이 없는 표준 사용자가 이러한 설정을 사용할 수 있습니다.
...
1. 필터를 저장하고 이름을 지정합니다. 그런 다음 필터 목록에서 바로 사용할 수 있습니다.
