...
| **twitter 통계 계산** (statsTwitter) | 소셜 네트워크(소셜 마케팅) | 이 워크플로우는 Twitter 및 방문 횟수에 연결된 통계를 계산합니다. |
...
| **작업 알림** (taskMgt) | 마케팅 리소스(MRM) | 이 워크플로우를 통해 마케팅 캠페인의 작업과 관련된 알림 메시지를 보낼 수 있습니다. |
...
| **받은 편지함 렌더링용 시드 네트워크 업데이트** (updateRenderingSeed) | 받은 편지함 렌더링(IR) | 이 워크플로우는 받은 편지함 렌더링에 사용되는 이메일 주소를 업데이트하며, HTTPS 포트가 deliverability.neolane.net용으로 열려 있는 경우에만 작동합니다. |
