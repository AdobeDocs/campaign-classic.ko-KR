...
* [맞춤형 날짜 필드를 통한 이메일](../../workflow/using/email-enrichment-with-custom-date-fields.md)향상
...
1. 옵션을 **[!UICONTROL Link definition]** 선택하고 **[!UICONTROL Next]** 단추를 클릭합니다. 만들 링크의 유형을 지정합니다. 이 예에서는 파일 받는 사람의 국가와 데이터베이스의 전용 테이블에 있는 사용 가능한 국가 목록의 국가를 조정합니다. 옵션을 **[!UICONTROL Define a link by searching for a reference among several options]** 선택합니다. 필드에서 국가 테이블을 **[!UICONTROL Target schema]** 선택합니다.
...
농축 활동에 대한 자세한 내용은 이 [섹션을 참조하십시오](../../workflow/using/enrichment.md).
...
기본적으로 **데이터** 연계 활동이 제안을 전달하는 데 사용되면 순위 및 가중치는 제안 표에 저장되지 않습니다.

기본적으로 **[!UICONTROL Offer engine]** 활동에는 이 정보가 저장됩니다.

그러나 다음과 같이 이 정보를 저장할 수 있습니다.

1. 쿼리 후에 전달 활동 전에 배치된 농축활동 중에서 오퍼 엔진에 대한 호출을 만듭니다. 이 [섹션을 참조하십시오](../../interaction/using/integrating-an-offer-via-a-workflow.md#specifying-an-offer-or-a-call-to-the-offer-engine).
