...
각 환경에는 Adobe Campaign 인스턴스가 포함되며 각 Adobe Campaign 인스턴스는 하나 이상의 데이터베이스에 연결됩니다. 응용 프로그램 서버는 하나 이상의 프로세스를 실행할 수 있습니다.이들 중 대부분은 인스턴스 데이터베이스에 직접 액세스할 수 있습니다.

이 섹션에서는 소스 환경을 대상 환경으로 복원하여 두 개의 동일한 작업 환경을 만드는 Adobe Campaign 환경 복제에 적용할 프로세스에 대해 자세히 설명합니다.

이렇게 하려면 다음 단계를 적용합니다.

1. 소스 환경의 모든 인스턴스에서 데이터베이스의 복사본을 만듭니다.
...
1. 시작하기 전에 대상 환경에서 **nms:freezeInstance.js** 자막 스크립트를 실행합니다.
   ...
   >[!NOTE]
   >
   >Adobe Campaign 컨텍스트에서 **자막**&#x200B;은 외부와 상호 작용하는 모든 프로세스를 중지할 수 있는 작업을 결합합니다.로그, 추적, 배달, 캠페인 워크플로우 등\
   >이 단계는 여러 번 메시지를 전달하지 않도록 하기 위해 필요합니다(명목상 환경에서 한 번, 중복된 환경에서 한 번 메시지 제공).
...
>[!IMPORTANT]
>
>전송 프로세스를 시작하기 전에 소스 및 대상 환경의 모든 인스턴스에 대해 데이터베이스의 전체 백업을 실행하는 것이 좋습니다. 이러한 방식으로 문제가 발생하면 백업을 복원하고 초기 구성으로 돌아갈 수 있습니다.

이 프로세스가 작동하려면 소스 및 대상 환경에 동일한 인스턴스 수, 동일한 목적(마케팅 인스턴스, 전달 인스턴스) 및 유사한 구성이 있어야 합니다. 기술 구성은 소프트웨어 사전 요구 사항을 준수해야 합니다. 두 환경 모두에 동일한 구성 요소를 설치해야 합니다.

## 구현 {#implementation}

### 전송 프로시저 {#transfer-procedure}

이 섹션에서는 사례 연구를 통해 소스 환경을 대상 환경으로 전송하는 데 필요한 단계를 이해하는 데 도움이 됩니다.여기서 목표는 프로덕션 환경(**prod** 인스턴스)을 개발 환경(**dev** 인스턴스)으로 복원하여 &#39;live&#39; 플랫폼에 가능한 가까운 컨텍스트에서 작동하도록 하는 것입니다.

다음 단계는 세심한 주의를 기울여 수행해야 합니다.소스 환경 데이터베이스를 복사할 때 일부 프로세스가 계속 진행 중일 수 있습니다. 메시지 전송을 두 번 방지하고 데이터 일관성을 유지합니다.
...
먼저 소스 환경 데이터베이스를 모두 복사합니다. 작업은 데이터베이스 엔진에 따라 다르며 데이터베이스 관리자의 책임입니다.
...
대상 데이터베이스에 소스 데이터베이스를 저장하기 전에 대상 환경(개발) 구성을 내보내야 합니다. 이렇게 하려면 다음 두 테이블의 컨텐트를 내보냅니다.**xtoption** 및 **nmsextaccount**.
...
>[!NOTE]
>
>패키지 내보내기에서 내보낼 줄 수는 1,000줄입니다. 옵션 또는 외부 계정의 수가 1,000개를 초과하는 경우 여러 개의 내보내기를 수행해야 합니다.
> 
...
1. 배달 상태 업데이트가 올바른지 확인합니다.

   ```
   SELECT iState, count(*) FROM neolane.nmsdelivery GROUP BY iState;
   ```

1. 워크플로 상태 업데이트가 올바른지 확인:
...
가져오기 전에 여러 줄의 파일 값을 확인합니다(예:옵션 테이블 및 외부 계정 테이블에 대한 납품 또는 중간 소싱 계정에 대한 &#39;NmsTracking_Pointer&#39;)
...
1. Adobe Campaign 콘솔에서 이전에 패키지 가져오기 기능을 통해 만든 options_dev.xml 패키지를 가져옵니다.
   ...
   옵션이 **[!UICONTROL Administration > Platform > Options]** 노드에서 정말로 업데이트되었는지 확인합니다.

1. Adobe Campaign 콘솔에서 이전에 패키지 가져오기 기능을 통해 만든 extaccount_dev.xml을 가져옵니다

   외부 데이터베이스가 **[!UICONTROL Administration > Platform > External accounts]**&#x200B;에서 실제로 가져왔는지 확인합니다.
...
개발 플랫폼에 이미 존재했던 사용자를 찾으려면 사용자를 변경합니다.
