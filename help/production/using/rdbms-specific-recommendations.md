...
1. 다음 명령을 실행하면 큰 테이블과 색인을 찾을 수 있습니다.
...
PostgreSQL에서 사용할 수 있는 일반적인 명령은 **진공 가득** 찬 **다음**&#x200B;다시 색인화입니다.
...
* 첫 번째 함수를 사용하면 원본 테이블의 복사본인 작업 테이블(*_tmp***)을 만들 수 있습니다.
* 그런 다음 두 번째 함수를 사용하여 원래 테이블을 삭제하고 작업 테이블과 인덱스의 이름을 변경합니다.
...
1. 먼저 관리자 권한이 있는 로그인으로 Microsoft SQL Server Management Studio에 연결합니다.
1. 폴더로 이동하고 **[!UICONTROL Management > Maintenance Plans]** 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL Maintenance Plan Wizard]**
1. 첫 번째 페이지가 나타나면 **[!UICONTROL Next]** 클릭합니다.
1. 생성할 유지 관리 계획 유형을 선택합니다(각 작업에 대해 별도의 일정 또는 전체 계획에 대해 단일 일정). 그런 다음 **[!UICONTROL Change...]** 단추를 누릅니다.
1. 창에서 **[!UICONTROL Job schedule properties]** 원하는 실행 설정을 선택하고 을 **[!UICONTROL OK]** 클릭한 다음 을 **[!UICONTROL Next]** 클릭합니다.
...
1. 드롭다운 목록에서 작업을 실행할 데이터베이스를 **[!UICONTROL Database Check Integrity]** 선택합니다.
1. 데이터베이스를 선택하고 을 클릭한 **[!UICONTROL OK]** 다음 을 클릭합니다 **[!UICONTROL Next]** .
...
1. 색인을 다시 구성하거나 다시 구성합니다.

   * 인덱스 단편화 비율이 10%에서 40% 사이인 경우 다시 구성하는 것이 좋습니다.
   ...
   * 인덱스 단편화 비율이 40%보다 높은 경우 다시 구성하는 것이 좋습니다.

      인덱스 다시 작성 작업에 적용할 옵션을 선택한 다음 을 클릭합니다 **[!UICONTROL Next]** .

      >[!NOTE]
      >
      >인덱스 재구축 프로세스는 프로세서 사용 측면에서 더욱 제한적이며 데이터베이스 리소스를 잠깁니다. 다시 구성하는 동안 색인을 사용할 수 있게 하려면 **[!UICONTROL Keep index online while reindexing]** 옵션을 확인 표시합니다.

1. 활동 보고서에 표시할 옵션을 선택한 다음 을 클릭합니다 **[!UICONTROL Next]** .
1. 유지 관리 계획에 대해 구성된 작업 목록을 확인하고 을 클릭합니다 **[!UICONTROL Finish]** .
...
WdbcOptions_ **TempDbName** 옵션을 사용하면 Microsoft SQL Server에서 작업 테이블에 대해 별도의 데이터베이스를 구성할 수 있습니다. 백업 및 복제를 최적화합니다.
