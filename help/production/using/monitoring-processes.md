...
* **[!UICONTROL Memory]** : 실제 메모리(RAM)에 대한 정보입니다.
   ...
   **[!UICONTROL Alert]** : 이 표시기는 메모리 소비가 전체 용량의 90%에 도달할 때 표시됩니다.
   ...
   Adobe Campaign 서버 **[!UICONTROL Warning]** **[!UICONTROL Alert]** 가 설치된 컴퓨터에 RAM을 추가하면 이 문제를 해결할 수 있습니다. 전용 컴퓨터에 Adobe Campaign 서버를 설치하기로 결정할 수도 있습니다.
...
* **[!UICONTROL Number of processes too old]** : 하루 이상 활성화된 Adobe Campaign 프로세스에 대한 정보입니다.
   ...
   **[!UICONTROL Alert]** : 프로세스 수가 1인 경우 이 표시기가 표시됩니다.
   ...
   표시기가 **[!UICONTROL Alert]** 표시되면 관련 프로세스가 SQL 데이터베이스 엔진으로 잠겨 있거나 무한 루프에 고정되어 있을 수 있습니다. Adobe Campaign에서 제공하는 **감시** 프로세스는 매일 모든 프로세스를 자동으로 다시 시작하고 이 문제를 해결할 수 있습니다. 하지만, 당신은 다시 시작하도록 스스로 관련된 과정을 중단할 수도 있다.
...
* **[!UICONTROL Core Files]** : Adobe Campaign 프로세스 충돌 후 생성된 파일에 대한 정보. 이러한 파일을 통해 충돌 원인을 진단할 수 있습니다.
...
Adobe Campaign을 사용하면 표시기를 사용자 정의할 수 있습니다. 이렇게 하려면:
...
기술 **[!UICONTROL Billing]** 워크플로우는 시스템 활동 보고서를 이메일로 &#39;대금 청구&#39; 운영자에게 보냅니다. 매월 25일에 기본적으로 트리거됩니다.
...
연산자 이름 및 로그인은 데이터베이스 권한으로 Adobe Campaign 클라이언트 콘솔에서 이전에 구성해야 합니다.
...
해당 데이터베이스 권한 및 제한 사항을 사용하여 이전에 Adobe Campaign 클라이언트 콘솔에서 연산자 및 해당 로그인을 구성해야 합니다.
...
Adobe Campaign은 다양한 배포 구성을 활성화합니다(자세한 내용은 [이 섹션 참조](../../installation/using/hosting-models.md)). 이 섹션에서는 설치 유형에 따라 적용되는 다양한 자동 모니터링 기술에 대해 자세히 설명합니다.
...
>[!CAUTION]
>
>이 도구를 사용하여 인스턴스를 모니터링할 수 있지만 Adobe Campaign에서는 지원되지 않습니다. 자세한 내용은 캠페인 관리자에게 문의하십시오.
