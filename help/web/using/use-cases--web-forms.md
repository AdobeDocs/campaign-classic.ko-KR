...
## 이중 옵트인 {#create-a-subscription--form-with-double-opt-in}으로 구독 양식 만들기

정보 서비스를 제공하는 경우 수신자는 모든 연결된 커뮤니케이션을 수신하려면 가입해야 합니다. 부적절한 통신을 방지하고 수신자가 의도적으로 구독했는지 확인하려면 구독 확인 요청을 보내 이중 옵트를 만드는 것이 좋습니다. 가입은 사용자가 확인 메시지에 포함된 링크를 클릭해야만 적용됩니다.

이 예는 다음 시나리오를 기반으로 합니다.

1. 임시 서비스 가입 확인란이 포함된 웹 사이트에서 뉴스레터 구독 양식을 만듭니다. 이 서비스를 사용하면 구독 확인 메시지를 전달할 수 있습니다.
1. 웹 양식에 연결된 전달 템플릿으로 구독 확인 배달 만들기 여기에는 뉴스레터 가입에 대한 양식을 호출하고 구독 승인 메시지를 표시하는 확인 링크가 포함되어 있습니다.

### 1단계 - 정보 서비스 {#step-1---creating-information-services} 만들기
...
1. 구독 확인 메시지를 전송하기 위한 배달 템플릿을 만듭니다.
1. **[!UICONTROL Email parameters]**&#x200B;에서 **[!UICONTROL To]** 단추를 클릭하여 배달 템플릿을 받는 사람 대신 구독 대상 매핑과 연결합니다.

   ![](assets/s_ncs_admin_survey_double-opt-in_sample_1d.png)

1. 이 배달을 받는 사람이 승인을 확인하지 않았기 때문에 여전히 데이터베이스 차단 목록에 있습니다. 이러한 사람이 이 통신을 수신하려면 이 템플릿을 기반으로에서 받는 사람을 대상으로 게재하는 전달을 승인해야 차단 목록 합니다.
...
1. 배달을 개인화하고 메시지 컨텐츠에 확인 링크를 삽입합니다. 이 링크를 사용하면 웹 양식에 액세스하여 구독 확인을 기록할 수 있습니다.

   ![](assets/s_ncs_admin_survey_double-opt-in_sample_1b.png)

1. DCE를 사용하여 URL을 웹 양식에 연결합니다. 웹 양식이 아직 만들어지지 않았으므로 만든 대로 값을 바꾸십시오.
...
### 3단계 - 구독 양식 {#step-3---creating-the-subscription-form} 만들기

웹 양식을 통해 수신자의 구독 및 구독 확인을 모두 수행할 수 있습니다.

웹 양식 작업 과정에는 다음 활동이 포함됩니다.

![](assets/s_ncs_admin_survey_double-opt-in_sample_4c.png)

이렇게 하려면 아래 단계를 수행합니다:

1. 웹 양식을 만들고 템플릿 **[!UICONTROL Newsletter subscription (subNewsletter)]**&#x200B;을 선택합니다.

   ![](assets/s_ncs_admin_survey_double-opt-in_sample_5a.png)

1. 가입하려는 수신자에게 확인 메시지를 추가하려는 경우 **[!UICONTROL Edit]** 탭에서 기존 워크플로우를 구성해야 합니다.
...
1. 워크플로우에 **[!UICONTROL Test]** 활동을 추가합니다.
   ...
   **[!UICONTROL Test]** 활동은 수신자 이메일에 영향을 줄 수 있습니다. 이 경우 다음과 같이 구성합니다.
...
1. 워크플로우에 두 개의 **[!UICONTROL Script]** 활동을 추가합니다.
   ...
   첫 번째 **[!UICONTROL Script]** 활동은 뉴스레터에 대한 가입이 확인될 때까지 받는 차단 목록 사람을에 추가합니다. 컨텐츠는 다음과 같아야 합니다.
   ...
   두 번째 **[!UICONTROL Script]** 활동은 사용자에게 보낼 배달을 승인하고 뉴스레터에 가입합니다. 스크립트 마지막 두 줄을 사용하면 받는 사람을 임시 폴더에서 다른 폴더로 전송할 수 있으며 구독이 확인되는 즉시 기존 프로필과 조정할 수 있습니다.
...
1. 양식 페이지에 입력한 정보를 저장하도록 **[!UICONTROL Storage]** 활동을 구성합니다.
   ...
   이 활동을 통해 수신자 프로필을 전용 임시 폴더에 만들어 통신 정보를 보낼 수 있는 데이터베이스의 프로필과 별도로 설정할 수 있습니다.
...
1. 2개의 **[!UICONTROL End]** 활동을 추가하여 사용자에 대한 메시지를 표시합니다.

   구독이 완료되면 두 번째 **[!UICONTROL End]** 상자에 확인 메시지가 표시됩니다.

   ![](assets/s_ncs_admin_survey_double-opt-in_sample_5h.png)

1. 이제 웹 양식이 만들어지고 구성되면 배달 템플릿에서 웹 양식을 참조하여 확인 메시지를 보낼 수 있습니다.
...
1. 웹 사이트 사용자가 구독 페이지에 로그인하여 양식을 승인합니다.
   ...
   이러한 사용자는 자신의 요청이 고려되었다는 메시지를 브라우저에 통해 알림을 받습니다.

   ![](assets/s_ncs_admin_survey_double-opt-in_sample_8d.png)

   사용자가 **[!UICONTROL Temp]** 폴더의 Adobe Campaign 데이터베이스에 추가되고 해당 사용자는 이메일에 대한 가입이 확인될 때까지 해당 차단 목록 프로필을에 있습니다.
...
## 선택한 값 {#displaying-different-options-depending-on-the-selected-values}에 따라 다른 옵션 표시

다음 예에서, 사용자에게 차량 유형을 선택하라는 메시지가 표시됩니다. 선택한 유형에 따라 사용 가능한 차량 범주를 표시할 수 있습니다. 즉, 오른쪽 열에 표시되는 항목은 사용자의 선택에 따라 달라집니다.

![](assets/s_ncs_admin_survey_condition_sample0.png)

* 사용자가 &#39;개인 차량&#39;을 선택하면 &#39;콤팩트&#39;와 &#39;미니밴&#39; 중에서 선택할 수 있다.
