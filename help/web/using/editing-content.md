...
>[!NOTE]
>
>고급 표현식 편집은 [이 페이지](../../platform/using/defining-filter-conditions.md#list-of-functions).
...
을(를) 추가할 수 있습니다 **테두리** 선택한 블록으로 이동합니다. 테두리는 다음 세 가지 옵션을 사용하여 정의됩니다. 스타일, 크기 및 색상.

![](assets/dce_popup_border.png)

을(를) 정의할 수도 있습니다 **배경색** 색상 차트에서 색상을 선택하여 선택합니다.
...
다음 **필수 필드** 옵션을 사용하면 사용자가 해당 필드를 입력한 경우에만 페이지의 승인을 승인할 수 있습니다. 필수 필드를 입력하지 않은 경우 오류 메시지가 표시됩니다.

라디오 단추 및 확인란의 경우 **추가 구성이 필요합니다.**.

실제로 사용된 템플릿에 기본적으로 값이 포함되어 있지 않으면 편집기에서 완료해야 합니다.

방법은 다음과 같습니다.

* 을(를) 클릭합니다. **[!UICONTROL Edit]** 아이콘.

   ![](assets/dce_sidebar_options.png)

* 선택한 필드에 항목화된 목록 값(선택한 필드로 정의됨)을 입력합니다. **[!UICONTROL Value]** 필드.
...
* 양식 필드가 포함된 블록을 **[!UICONTROL Delete]** 아이콘.
* 을(를) 사용하여 새 블록을 만들어 선택한 필드를 복제합니다 **[!UICONTROL Duplicate]** 아이콘.
* 편집 **[!UICONTROL Form data]** 창을 사용하여 데이터베이스 필드를 양식 영역에 연결하는 방법 **[!UICONTROL Edit]** 아이콘.
...
>[!NOTE]
>
>다음 **[!UICONTROL None]** 값을 사용하면 단추를 활성화할 수 없습니다.
...
>[!NOTE]
>
>각 링크 유형에 대해 열기 모드를 구성할 수 있습니다. 에서 대상 창을 선택합니다. **Target** 드롭다운 목록. 이 값은 **`<target>`** HTML 태그 사이에 Analytics JavaScript 코드를 배치했습니다.
>
...
다음 **외부 URL에 연결** 옵션을 사용하면 소스 컨텐츠에서 임의의 URL을 열 수 있습니다.
...
다음 **웹 응용 프로그램에 연결** 옵션을 사용하면 Adobe Campaign 웹 애플리케이션에 액세스할 수 있습니다.

![](assets/dce_toolbar_imgblock_appweb.png)

해당 필드에서 웹 애플리케이션을 선택합니다.

제안된 웹 응용 프로그램 목록은 **[!UICONTROL Resources > Online > Web Applications]** 노드 아래에 있어야 합니다.
...
>[!NOTE]
>
>사용 가능한 작업은 [단추에 작업 추가](#adding-an-action-to-a-button) 섹션을 참조하십시오.

### 링크 삭제 {#delete-a-link}

링크가 삽입되면 도구 모음에는 두 개의 새 아이콘이 표시됩니다. **링크 편집** 및 **링크 중단** 생성된 링크와 상호 작용할 수 있습니다.
...
* **글꼴 확대** 아이콘: 선택한 텍스트의 크기를 늘립니다(추가 `<span style="font size:">`)
* **글꼴 감소** 아이콘: 선택한 텍스트의 크기를 줄입니다(추가 `<span style="font size:">`)
* **굵게** 아이콘: 선택한 텍스트를 굵게 표시(텍스트 줄바꿈) `<strong> </strong>` 태그)
* **기울임체** 아이콘: 선택한 텍스트를 기울임체로 만들기(텍스트 줄바꿈을  `<em> </em>` 태그)
* **밑줄** 아이콘: 선택한 텍스트에 밑줄을 긋습니다(텍스트 줄바꿈을 `<span style="text-decoration: underline;">` 태그)
* **왼쪽 정렬** 아이콘: 선택한 블록의 왼쪽으로 텍스트를 정렬합니다(style=&quot;text-align: left;&quot;
* **가운데** 아이콘: 선택한 블록의 가운데로 텍스트를 정렬합니다(style=&quot;text-align: center;&quot;)
...
디지털 콘텐츠 편집기를 사용하면 작업할 수 있습니다 **모든 유형의 이미지** 와 호환됩니다.

>[!CAUTION]
>
>에서 외부 파일을 호출해서는 안 됩니다 **script** 태그로 래핑됩니다. 이러한 파일은 Adobe Campaign 서버로 가져오지 않습니다.
...
* **캡션** 이미지에 연결된 캡션( **alt** HTML 속성).
* **Dimension** 이미지 크기를 픽셀 단위로 지정할 수 있도록 해줍니다.
...
다음 **개인화 필드** 삽입 아이콘 옵션을 사용하면 수신자의 이름과 같은 데이터베이스 필드를 콘텐츠에 추가할 수 있습니다. 이 옵션은 텍스트 유형 블록에만 사용할 수 있습니다.
...
예제는 [개인화 필드 삽입](creating-a-landing-page.md#inserting-a-personalization-field) 섹션을 참조하십시오.

### 개인화 블록 삽입 {#inserting-a-personalization-block}

다음 **개인화 블록** 옵션을 사용하면 컨텐츠에 동적 및 개인화된 블록을 삽입할 수 있습니다. 예를 들어 로고나 인사말 메시지를 추가할 수 있습니다. 텍스트 유형 블록에는 사용할 수 없습니다.

![](assets/dce_toolbar_textblock_persoblock.png)

삽입되면 개인화 블록 이름이 편집기에 나타나고 노란색으로 강조 표시됩니다. 개인화가 생성되면 수신자 프로필에 자동으로 조정됩니다.

내장 개인화 블록과 사용자 지정 개인화 블록을 정의하는 방법에 대한 자세한 내용은 다음을 참조하십시오. [이 페이지](../../delivery/using/personalization-blocks.md).
