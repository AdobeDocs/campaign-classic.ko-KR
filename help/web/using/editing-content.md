...
>[!NOTE]
>
>고급 표현식 편집이 [이 페이지에 표시됩니다](../../platform/using/defining-filter-conditions.md#list-of-functions).

![](assets/dce_popup_visibilitycondition.png)

이러한 조건은 XTK 식 구문을 채택합니다(예: **ctx.recipient).@email != &quot;&quot;** 또는 **ctx.recipient.@status==&quot;0&quot;**). 기본적으로 모든 필드가 표시됩니다.
...
You can add a **border** to a selected block. 테두리는 다음 세 가지 옵션을 사용하여 정의됩니다.스타일, 크기 및 색상.
...
The **Required field** option lets you only authorize the page&#39;s approval if the user has filled in the field. 필수 필드를 채우지 않으면 오류 메시지가 표시됩니다.
...
* 선택한 필드에 항목별 목록 값(선택한 필드에 정의됨)을 **[!UICONTROL Value]** 입력합니다.
...
* 아이콘을 사용하여 양식 필드가 포함된 블록을 **[!UICONTROL Delete]** 삭제합니다.
* 아이콘을 사용하여 새 블록을 만들어 선택한 필드를 **[!UICONTROL Duplicate]** 복제합니다.
* 아이콘을 **[!UICONTROL Form data]** 사용하여 데이터베이스 필드를 양식 영역에 연결하려면 창을 **[!UICONTROL Edit]** 편집합니다.

   ![](assets/dce_toolbar_formblock_edition.png)

## 단추에 동작 추가 {#adding-an-action-to-a-button}

사용자가 단추를 클릭하면 연결된 작업을 정의할 수 있습니다. 이렇게 하려면 드롭다운 목록에서 수행할 작업을 선택합니다.
...
외부 URL **에** 연결 옵션을 사용하면 소스 컨텐츠의 URL을 열 수 있습니다.

![](assets/dce_toolbar_imgblock_externallink.png)

문제의 링크 주소를 **URL** 필드에 입력합니다. URL 필드는 다음과 같이 입력해야 합니다. **https://www.myURL.com**.

### 웹 응용 프로그램에 대한 링크 {#link-to-a-web-application}
...
제안된 웹 응용 프로그램의 목록은 노드에서 사용 가능한 응용 프로그램에 **[!UICONTROL Resources > Online > Web Applications]** 해당합니다.

### 작업에 연결 {#link-to-an-action}
...
>[!NOTE]
>
>사용 가능한 작업은 단추에 작업 [추가 섹션에서](#adding-an-action-to-a-button) 자세히 설명합니다.

### 링크 삭제 {#delete-a-link}

링크가 삽입되면 도구 모음에는 두 개의 새 아이콘이 표시됩니다. **링크를** 편집하고 **만든** 링크와 상호 작용할 수 있는 링크를중단합니다.
...
* **확대 글꼴** 아이콘:선택한 텍스트 크기 증가(추가 `<span style="font size:">`)
* **글꼴** 아이콘 감소:선택한 텍스트 크기 축소(추가 `<span style="font size:">`)
...
* **가운데** 아이콘:선택한 블록의 텍스트 가운데 맞춤(add style=&quot;text-align:center;&quot;)
* **오른쪽** 정렬 아이콘:선택한 블록의 오른쪽에 텍스트를 정렬합니다(add style=&quot;text-align:right;&quot;
* **배경색** 아이콘 변경:선택한 블록의 배경색을 변경할 수 있습니다(add style=&quot;background-color:rgba(170, 86, 255, 0.87)
...
디지털 컨텐츠 편집기를 사용하면 브라우저와 호환되는 **모든 유형의 이미지** 작업을 수행할 수 있습니다.

DCE와 호환하려면 **&quot;Flash&quot; 유형 애니메이션을** 다음과 같이 HTML 페이지에 삽입해야 합니다.
...
>[!CAUTION]
>
>HTML 페이지의 **스크립트** 태그로 외부 파일을 호출해서는 안 됩니다. 이러한 파일은 Adobe Campaign 서버로 가져올 수 없습니다.
...
이미지가 포함된 블록을 선택하면 다음 속성에 액세스할 수 있습니다.

* **캡션을** 사용하면 이미지에 연결된 캡션을 정의할 수 있습니다(대체 **HTML 속성에** 해당).
* **Dimension** 를 사용하면 이미지 크기를 픽셀 단위로 지정할 수 있습니다.
...
기본적으로 제공된 필드는 **[!UICONTROL Recipient]** 테이블에서 제공됩니다. 필요한 경우 웹 응용 프로그램 속성을 편집하여 다른 테이블을 선택합니다.

필드 이름이 편집기에 나타나고 노란색으로 강조 표시됩니다. 개인화가 생성되면(예: 랜딩 페이지를 미리 볼 때) 타깃팅된 수신자의 프로필로 대체됩니다.

개인화 필드 [삽입 섹션에 예가 나와](../../web/using/creating-a-landing-page.md#inserting-a-personalization-field) 있습니다.
...
개인화 블록 이름이 삽입되면 편집기에 노란색으로 강조 표시됩니다. 개인화가 생성될 때 수신자 프로필에 자동으로 적용됩니다.

내장된 개인화 블록과 맞춤형 개인화 블록을 정의하는 방법에 대한 자세한 내용은 [이 페이지를 참조하십시오](../../delivery/using/personalization-blocks.md).
