...
사용자가 양식을 채울 때 형식이나 구성에 따라 특정 필드에 검사가 자동으로 수행됩니다. 이렇게 하면 특정 필드를 필수 입력 필드로 만들거나(](#defining-mandatory-fields) 참조) 입력한 데이터의 형식을 확인할 수 있습니다([데이터 형식 확인](#checking-data-format) 참조). [ 확인은 페이지 승인 중에 수행됩니다(출력 전환을 가능하게 하는 링크나 단추를 클릭하여).
