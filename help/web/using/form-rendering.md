...
양식 설정을 사용하면 페이지를 생성하는 데 사용되는 템플릿을 선택할 수 있습니다. 이러한 항목에 액세스하려면 양식 세부 사항 도구 모음에서 **[!UICONTROL Settings]** 단추를 클릭하고 **[!UICONTROL Rendering]** 탭을 선택합니다. 기본적으로 사용할 수 있는 여러 템플릿(스타일 시트)이 있습니다.

![](assets/s_ncs_admin_survey_rendering_select.png)

편집기의 하단 섹션에서 선택한 템플릿의 렌더링을 볼 수 있습니다.
...
기존 행을 선택하고 **[!UICONTROL Detail]** 단추를 클릭하여 원하는 대로 설정합니다.

![](assets/s_ncs_admin_survey_render_edit_header_detail.png)

선의 내용을 변경하고 테두리를 추가하고 관련 탭을 통해 글꼴 속성을 변경할 수 있습니다. **[!UICONTROL OK]**&#x200B;을 클릭하여 변경 내용을 확인합니다.

**[!UICONTROL Position]** 필드를 사용하면 페이지 머리글과 바닥글에 요소의 위치를 정의할 수 있습니다.
...
* **[!UICONTROL Width]** 영역:백분율 또는 em, 포인트 또는 픽셀(기본값)로 표시할 수 있습니다.
* 최대 **[!UICONTROL Length]**:허용되는 최대 문자 수(텍스트, 숫자 및 암호 유형 컨트롤의 경우),
* **[!UICONTROL Lines]**:유형 영역의  **[!UICONTROL Multi-line text]** 줄 수,
* **[!UICONTROL Style inline]**:CSS 스타일 시트를 추가 설정으로 오버로드할 수 있습니다. 이러한 문자는 아래 예와 같이 **;** 문자를 사용하여 구분됩니다.

   ![](assets/s_ncs_admin_survey_advanced_tab_inline.png)

### 머리글 및 바닥글 정의 {#defining-headers-and-footers}

필드는 페이지와 동일한 이름의 루트가 있는 트리 구조로 시퀀스가 지정됩니다. 이름을 수정하려면 선택하십시오.
...
### HTML 헤더 {#adding-elements-to-html-header}에 요소 추가
...
사용자가 양식을 채울 때 형식이나 구성에 따라 특정 필드에 검사가 자동으로 수행됩니다. 이렇게 하면 특정 필드를 필수 입력 필드로 만들거나([ 참조) 입력한 데이터의 형식을 확인할 수 있습니다(](#defining-mandatory-fields)데이터 형식 확인[ 참조). ](#checking-data-format) 확인은 페이지 승인 중에 수행됩니다(출력 전환을 가능하게 하는 링크나 단추를 클릭하여).
...
### 데이터 형식 {#checking-data-format} 확인

데이터베이스의 기존 필드에 값이 저장되어 있는 양식을 확인하는 경우 저장소 필드에 대한 규칙이 적용됩니다.

값이 변수에 저장되는 양식을 확인하는 경우 승인 규칙은 변수의 형식에 따라 다릅니다.

예를 들어, 아래와 같이 **[!UICONTROL Number]** 검사를 만들어 클라이언트 번호를 저장하는 경우:
...
사용자가 선택한 값을 기반으로 페이지에 필드 표시를 구성할 수 있습니다. 이는 한 필드 또는 필드 그룹에 적용할 수 있습니다(컨테이너로 그룹화된 경우).
...
* **[!UICONTROL Variables]** 하위 트리에는 이 양식에 사용할 수 있는 변수가 포함되어 있습니다. 자세한 내용은 [로컬 변수](../../web/using/web-forms-answers.md#storing-data-in-a-local-variable)에 데이터 저장을 참조하십시오.

자세한 내용은 다음 사용 사례를 참조하십시오.[선택한 값](../../web/using/use-cases--web-forms.md#displaying-different-options-depending-on-the-selected-values)에 따라 다른 옵션을 표시합니다.
...
다른 웹 양식에서 필드 또는 컨테이너를 가져올 수 있습니다. 주소 블록, 뉴스레터 구독 영역 등과 같이 양식에 삽입할 재사용 가능한 블록 라이브러리를 만들 수 있습니다.
