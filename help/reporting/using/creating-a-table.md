...
>[!CAUTION]
>
>보고서에서 분석할 데이터를 수집하려면 테이블을 포함하는 **[!UICONTROL Page]** 활동 앞에 **[!UICONTROL Query]** 또는 **[!UICONTROL Script]** 활동을 추가해야 합니다. 이러한 활동에 대한 자세한 내용은 [데이터 수집을 참조하여](../../reporting/using/collecting-data-to-analyze.md) 및 [스크립트 활동](../../reporting/using/advanced-functionalities.md#script-activity)을 분석하십시오.
...
이 링크를 통해 캠페인에 연결된 캠페인(**[!UICONTROL Label (Campaign)]**, 게재 목록(**[!UICONTROL Label]** )을 나열할 수 있으며, 각 캠페인에 대해 추가하기 전에 게재당 전송된 메시지 수(**[!UICONTROL Processed)]**)를 카운트할 수 있습니다(**[!UICONTROL Sum(@processed)]**).
...
1. 보고서 차트로 이동하고 **[!UICONTROL Query]** 활동을 배치합니다. [데이터 수집 을 참조하여](../../reporting/using/collecting-data-to-analyze.md)을 분석하십시오.
...
추가하는 선의 특성은 커서 위치에 따라 다릅니다. 예를 들어, 헤더 라인을 추가하려면 커서를 헤더에 놓은 다음 **[!UICONTROL Add > A line above/below]** 을 클릭합니다.
...
* 예를 들어, 셀에 대한 레이블을 입력합니다.
   ...
   이렇게 하려면 데이터베이스에 필드를 삽입하는 것과 동일한 프로세스를 사용하되 표현식을 선택하지 마십시오. **[!UICONTROL Label]** 필드에 레이블을 입력합니다. 그대로 표시됩니다.
...
셀 형식을 정의하려면 **[!UICONTROL Cell format...]** 메뉴를 사용하여 선택한 셀에 사용할 수 있는 모든 서식 옵션에 액세스할 수 있습니다.
...
**[!UICONTROL Value]** 탭에서는 글꼴과 다양한 값 속성을 변경하거나 그 특성에 따라 형식을 정의할 수 있습니다.
...
필요한 경우 테이블 템플릿(**[!UICONTROL Administration > Configuration > Form rendering]** )에서 테두리를 정의할 수 있습니다.
...
캠페인 스키마인 **[!UICONTROL Campaigns (nms)]**&#x200B;에 대한 새 보고서를 만듭니다.
...
1. **[!UICONTROL Page]** 활동을 열고 해당 제목을 입력합니다(예: 이 경우 **게재**).
...
1. 그룹 창에서 &#39;Campaign&#39; 링크의 **외부 키를 참조하고 창을 닫습니다.**
...
1. 세부 정보 라인의 두 번째 셀을 편집하고 전달 **[!UICONTROL Label]**&#x200B;을 선택합니다.

   ![](assets/s_advuser_report_listgroup_011.png)

1. 이 셀의 형식을 편집하고 **[!UICONTROL Click]** 탭을 엽니다. 사용자가 게재 이름을 클릭할 때 동일한 창에서 열도록 적절한 옵션을 구성합니다.

   ![](assets/s_advuser_report_listgroup_0111.png)

   이렇게 하려면 **[!UICONTROL Next page]** 유형 작업을 선택하고 **[!UICONTROL In the same window]** 을 열린 옵션으로 선택합니다.

   ![](assets/s_advuser_report_listgroup_0112.png)

1. 창의 아래 섹션에서 **[!UICONTROL Add]** 을(를) 클릭하고 이전에 만든 쿼리에 정의된 대로 게재의 기본 키의 별칭과 일치하는 **`/vars/selectedDelivery`** 경로와 **[!UICONTROL @deliveryId]** 표현식을 지정합니다. 이 수식을 사용하면 선택한 게재에 액세스할 수 있습니다.

   ![](assets/s_advuser_report_listgroup_010.png)

1. 그룹의 바닥글 라인의 두 번째 셀을 편집하고 **[!UICONTROL Total per campaign]** 을 레이블로 입력합니다.

   ![](assets/s_advuser_report_listgroup_012.png)

1. 그룹의 헤더 라인의 세 번째 셀을 편집하고 **[!UICONTROL Number of messages sent]** 을 레이블로 입력합니다.
...
1. 그룹의 바닥글 라인의 세 번째 셀을 편집하고, 처리된 게재 표시자를 선택하고, **[!UICONTROL Sum]** 집계를 적용합니다.

   ![](assets/s_advuser_report_listgroup_015.png)

1. 세부 정보 라인의 네 번째 셀을 편집하고 **오류 전달 오류율**&#x200B;을 표현식으로 선택합니다.
...
1. 배달 오류 비율을 나타내는 값 막대를 표시하려면 이 셀을 선택합니다.
   ...
   **[!UICONTROL Preview]** 탭을 사용하여 테이블의 데이터가 올바르게 선택되어 구성되어 있는지 확인하는 것이 좋습니다. 이 작업이 완료되면 테이블 서식을 계속 지정할 수 있습니다.

1. 캠페인당 총 수와 처리된 총 메시지 수를 표시하는 셀에 **[!UICONTROL Bold]** 스타일을 적용합니다.

   ![](assets/s_advuser_report_listgroup_024.png)

1. 캠페인 이름을 표시하는 그룹 헤더 라인의 첫 번째 셀을 클릭하고 **[!UICONTROL Edit > Merge to right]** 을 선택합니다.
...
1. 마우스 오른쪽 단추 클릭 메뉴를 사용하여 그룹과 새 목록을 추가하고 **수신자별 전자 메일 도메인**&#x200B;으로 호출합니다.
1. **[!UICONTROL Table data XPath...]** 링크를 클릭하고 **[!UICONTROL Recipient delivery logs]** 링크를 선택합니다.
...
1. **[!UICONTROL Data]** 탭에서 테이블을 다음과 같이 조정합니다.
   ...
   * 세부 정보 라인의 두 번째 셀에서 **[!UICONTROL Email domain(Recipient)]** 표현식을 추가합니다.
   * 세부 정보 라인의 세 번째 셀에서 **[!UICONTROL count(primary key)]** 표현식을 추가합니다.

   ![](assets/s_advuser_report_listgroup_019.png)

1. 마우스 오른쪽 단추 클릭 메뉴를 사용하여 페이지에 파이 차트를 추가하고 **전자 메일 도메인** 레이블을 지정합니다. 자세한 내용은 [차트 유형 및 변형](../../reporting/using/creating-a-chart.md#chart-types-and-variants)을 참조하십시오.
...
1. 그런 다음 **[!UICONTROL Advanced settings]** 을 클릭하고 수신자 게재 로그에 대한 링크를 선택합니다.
...
이 구성 결과를 보려면 **[!UICONTROL Preview]** 탭을 클릭하고 **[!UICONTROL Global]** 옵션을 선택하십시오.
