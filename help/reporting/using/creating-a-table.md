...
A **[!UICONTROL List with group]** 테이블 유형을 사용하면 테이블의 데이터를 그룹화하고 그에 대한 통계를 생성할 수 있습니다. 예를 들어 데이터에 대한 합계와 소계를 만들 수 있습니다. 각 그룹에는 고유한 머리글, 세부 정보 및 바닥글 라인이 있습니다.

>[!CAUTION]
>
>다음 **[!UICONTROL Page]** 테이블을 포함하는 활동 앞에는 **[!UICONTROL Query]** 또는 **[!UICONTROL Script]** 활동으로 보고서에서 분석할 데이터를 수집합니다. 이러한 활동에 대한 자세한 내용은 [분석할 데이터 수집](../../reporting/using/collecting-data-to-analyze.md) 및 [스크립트 활동](../../reporting/using/advanced-functionalities.md#script-activity).
...
캠페인을 나열할 수 있습니다(**[!UICONTROL Label (Campaign)]**, 게재 목록(**[!UICONTROL Label]** )가 캠페인에 연결되고, 게재당 보낸 메시지 수를 셀 수 있도록 해줍니다(**[!UICONTROL Processed)]**, 각 캠페인에 대해 추가하기 전에(**[!UICONTROL Sum(@processed)]** ).
...
1. 보고서 차트로 이동하여 **[!UICONTROL Query]** 활동. 을(를) 참조하십시오 [분석할 데이터 수집](../../reporting/using/collecting-data-to-analyze.md).
1. 소스 테이블을 입력하고 통계가 고려될 테이블의 필드를 선택합니다.
1. 배치 **[!UICONTROL Page]** 차트의 활동. 자세한 내용은 다음을 참조하십시오. [정적 요소](../../reporting/using/creating-a-new-report.md#static-elements).
...
1. 테이블 및 해당 콘텐츠 만들기
1. 에서 완료된 보고서 표시 **[!UICONTROL Preview]** 탭. 그런 다음 보고서를 게시하고 필요한 경우 다른 형식으로 내보낼 수 있습니다. 자세한 내용은 다음을 참조하십시오. [보고서 내보내기](../../reporting/using/actions-on-reports.md#exporting-a-report).

### 줄 및 열 추가 {#adding-lines-and-columns}

기본적으로 **[!UICONTROL List with group]** 유형 테이블에는 머리글, 세부 정보 라인 및 바닥글 라인이 포함됩니다.
...
**선 및 열**: 줄이나 열을 추가하거나 삭제하려면 기존 줄이나 열로 이동하여 마우스 오른쪽 버튼 클릭 메뉴를 사용합니다.

![](assets/s_advuser_ergo_listgroup_006.png)

추가하는 선의 특성은 커서의 위치에 따라 다릅니다. 예를 들어, 머리글 줄을 추가하려면 머리글에 커서를 놓은 다음 **[!UICONTROL Add > A line above/below]**.

![](assets/s_advuser_ergo_listgroup_006a.png)

열의 너비는 다음을 통해 수정할 수 있습니다. **[!UICONTROL Column format]** 항목.

**그룹**: 그룹을 추가하려면 줄로 이동하여 드롭다운 메뉴에서 일치하는 항목을 선택합니다.
...
사용 **[!UICONTROL Expression]** 표시할 값을 선택하는 메뉴 항목입니다.
...
사용 **[!UICONTROL Carriage return]** 데이터를 Excel로 내보낼 때 필드: **[!UICONTROL Yes]** 캐리지 리턴을 강제 수행하는 값입니다. 내보낼 때 이 값은 유지됩니다. 자세한 내용은 다음을 참조하십시오. [보고서 내보내기](../../reporting/using/actions-on-reports.md#exporting-a-report).

다음 **[!UICONTROL Cell format]** 창을 사용하면 다음 탭에 액세스할 수 있습니다.
...
다음 **[!UICONTROL Value]** 탭에서는 글꼴 및 다양한 값 속성을 변경하거나 해당 속성에 따라 형식을 정의할 수 있습니다.

![](assets/s_advuser_ergo_listgroup_009.png)

데이터 표시 형식을 변경합니다. 예: **[!UICONTROL Number]**, **[!UICONTROL Monetary]** 및 **[!UICONTROL Percentage]** 형식을 사용하면 오른쪽에 있는 숫자를 정렬하고 소수점을 표시할 수 있습니다.
...
다음 **테두리** 탭에서는 표의 선 및 열에 테두리를 추가할 수 있습니다. 셀에 테두리를 추가하면 큰 보고서를 Excel로 내보낼 때 성능 문제가 발생할 수 있습니다.

![](assets/s_advuser_ergo_listgroup_014.png)

필요한 경우 테이블 템플릿에서 테두리를 정의할 수 있습니다(**[!UICONTROL Administration > Configuration > Form rendering]** ).
...
다음 **[!UICONTROL Click]** 탭에서는 사용자가 셀 또는 표의 콘텐츠를 클릭할 때의 작업을 정의할 수 있습니다.

아래 예에서 셀의 값을 클릭하면 보고서의 두 번째 페이지가 표시됩니다. 이 페이지에는 셀의 전달에 대한 정보가 포함됩니다.

![](assets/s_advuser_ergo_listgroup_015.png)

다음 **추가** 탭에서는 색상이 지정된 표식이나 값 표시줄 같은 데이터에 시각적 개체를 연결할 수 있습니다. 색상이 지정된 표시는 테이블이 차트에서 범례로 표시될 때 사용됩니다. 자세한 내용은 구현 예를 참조하십시오. [5단계 - 두 번째 페이지 만들기](#step-5---create-the-second-page)
...
캠페인 스키마와 관련된 새 보고서 만들기, **[!UICONTROL Campaigns (nms)]**.

![](assets/s_advuser_report_listgroup_001.png)

클릭 **[!UICONTROL Save]** 를 클릭하여 보고서를 만듭니다.
...
1. 쿼리의 소스가 적용되는 스키마를 변경하는 것부터 시작합니다. **[!UICONTROL Deliveries (nms)]** 스키마.
1. 다음을 클릭합니다. **[!UICONTROL Edit query]** 고급 필드를 연결하여 표시합니다.
...
1. 클릭 **[!UICONTROL Finish]** 이 조건을 저장하려면 을 클릭합니다. **[!UICONTROL Ok]** 쿼리 편집기를 닫습니다.

### 3단계: 첫 번째 페이지 만들기 {#step-3--create-the-first-page}

이 단계에서는 보고서의 첫 페이지를 구성하려고 합니다. 이를 구성하려면 다음 단계를 적용합니다.

1. 를 엽니다. **[!UICONTROL Page]** 활동 및 제목 입력(예: ) **게재** 이 경우.
...
1. 다음을 클릭합니다. **[!UICONTROL Table data XPath...]** 링크를 클릭하고 게재 링크(예: )를 선택합니다. `[query/delivery]`.

   ![](assets/s_advuser_report_listgroup_005.png)

1. 다음을 클릭합니다. **[!UICONTROL Data]** 테이블의 탭 및 레이아웃 변경: 오른쪽에 열 3개를 추가합니다.
...
1. 그룹 창에서 다음을 참조합니다. **&#39;캠페인&#39; 링크의 외래 키** 창을 닫습니다.

   ![](assets/s_advuser_report_listgroup_007.png)

1. 그룹 헤더의 첫 번째 셀을 편집하고 **[!UICONTROL Label]** 표현식으로서의 캠페인 필드.
...
1. 이 셀의 형식을 편집하고 **[!UICONTROL Click]** 탭. 사용자가 게재 이름을 클릭할 때 동일한 창에서 열리도록 적절한 옵션을 구성합니다.

   ![](assets/s_advuser_report_listgroup_0111.png)

   이렇게 하려면 **[!UICONTROL Next page]** 작업 입력 및 선택 **[!UICONTROL In the same window]** 을(를) 열기 옵션으로 사용할 수 있습니다.

   ![](assets/s_advuser_report_listgroup_0112.png)

1. 창의 아래쪽에서 을 클릭합니다. **[!UICONTROL Add]** 및 지정 **`/vars/selectedDelivery`** 경로 및 **[!UICONTROL @deliveryId]** 이전에 만든 쿼리에 정의된 대로 게재 기본 키의 별칭과 일치하는 표현식입니다. 이 수식을 사용하면 선택한 게재에 액세스할 수 있습니다.

   ![](assets/s_advuser_report_listgroup_010.png)

1. 그룹의 바닥글 라인 중 두 번째 셀을 편집하고 를 입력합니다 **[!UICONTROL Total per campaign]** 레이블로.

   ![](assets/s_advuser_report_listgroup_012.png)

1. 그룹 머리글 라인의 세 번째 셀을 편집하고 을 입력합니다. **[!UICONTROL Number of messages sent]** 레이블로.
...
1. 그룹의 바닥글 라인 중 세 번째 셀을 편집하고, 처리된 게재 표시기를 선택한 다음 **[!UICONTROL Sum]** 집계합니다.

   ![](assets/s_advuser_report_listgroup_015.png)

1. 세부 정보 라인의 네 번째 셀을 편집하고 **오류 게재 오류율** 표현식으로.

   ![](assets/s_advuser_report_listgroup_016.png)

1. 게재 오류율을 나타내는 값 막대를 표시하려면 이 셀을 선택합니다.

   이렇게 하려면 셀 형식에 액세스한 다음 **[!UICONTROL More]** 탭. 다음 항목 선택 **[!UICONTROL Value bar]** 드롭다운 목록에서 항목을 선택하고 **[!UICONTROL Hide the cell value]** 옵션을 선택합니다.

   ![](assets/s_advuser_report_listgroup_023.png)

   이제 보고서 렌더링을 볼 수 있습니다. 다음을 클릭합니다. **[!UICONTROL Preview]** 탭을 클릭하고 다음을 선택합니다. **[!UICONTROL Global]** 옵션: 캠페인에 연결된 Adobe Campaign 데이터베이스의 모든 게재 목록을 표시합니다.

   ![](assets/s_advuser_report_listgroup_025.png)

   다음을 사용하는 것이 좋습니다. **[!UICONTROL Preview]** 탭에서 테이블의 데이터가 올바르게 선택 및 구성되어 있는지 확인합니다. 이 작업이 완료되면 표의 서식을 지정할 수 있습니다.

1. 적용 **[!UICONTROL Bold]** 캠페인당 합계 및 처리된 총 메시지 수를 표시하는 셀에 스타일을 지정합니다.

   ![](assets/s_advuser_report_listgroup_024.png)

1. 캠페인 이름을 표시하는 그룹 헤더 라인의 첫 번째 셀을 누르고 를 선택합니다 **[!UICONTROL Edit > Merge to right]**.
...
1. 다음 항목 뒤에 새 쿼리 추가 **[!UICONTROL Page]** 활동 및 해당 스키마 편집: **[!UICONTROL Recipient delivery logs]** 스키마.
...
1. 다음을 클릭합니다. **[!UICONTROL Next]** 단추 두 번: 다음 위치로 이동합니다. **[!UICONTROL Data filtering]** 단계.
   ...
   구문은 다음과 같습니다. &#39;게재&#39; 링크의 외래 키는 설정 값과 같습니다 `$([vars/selectedDelivery])`
...
1. 선택 취소 **[!UICONTROL Enable output transitions]** 옵션: 보고서의 마지막 페이지이며 다른 활동이 뒤에 옵니다.

   ![](assets/s_advuser_report_listgroup_028.png)

1. 마우스 오른쪽 버튼 클릭 메뉴를 사용하여 그룹이 있는 새 목록을 추가하고 호출합니다. **수신자당 이메일 도메인**.
1. 다음을 클릭합니다. **[!UICONTROL Table data XPath...]** 및 선택 **[!UICONTROL Recipient delivery logs]** 링크를 클릭합니다.

   ![](assets/s_advuser_report_listgroup_029.png)

1. 다음에서 **[!UICONTROL Data]** 탭을 누르고 다음과 같이 테이블을 조정합니다.
...
1. 마우스 오른쪽 단추 클릭 메뉴를 사용하여 페이지에 파이 차트를 추가하고 **이메일 도메인** 레이블을 지정합니다. 자세한 내용은 다음을 참조하십시오. [차트 유형 및 변형](../../reporting/using/creating-a-chart.md#chart-types-and-variants).
1. 다음을 클릭합니다. **[!UICONTROL Variants]** 링크 및 선택 취소 **[!UICONTROL Display label]** 및 **[!UICONTROL Display caption]** 옵션.
...
1. 다음에서 **[!UICONTROL Data]** 탭에서 데이터 소스를 변경합니다. 선택 **[!UICONTROL Context data]** 을 클릭합니다.
...
1. 다음에서 **[!UICONTROL Chart type]** 섹션에서 **[!UICONTROL Email domain]** 변수를 채우는 방법에 따라 페이지를 순서대로 표시합니다.
1. 그런 다음 수행할 계산을 추가합니다. 합을 연산자로 선택합니다.

   ![](assets/s_advuser_report_listgroup_0202.png)

1. 다음을 클릭합니다. **[!UICONTROL Detail]** 버튼을 클릭하여 카운트에 사용할 필드를 선택한 다음 구성 창을 닫습니다.
...
이 구성의 결과를 보려면 **[!UICONTROL Preview]** 탭을 클릭하고 다음을 선택합니다. **[!UICONTROL Global]** 옵션을 선택합니다.
...
피벗 테이블을 만드는 방법에 대한 자세한 내용은 [이 섹션](../../reporting/using/ac-cubes.md).
