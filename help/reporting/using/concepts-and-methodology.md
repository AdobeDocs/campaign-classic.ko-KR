...
**[!UICONTROL Add]** 단추를 클릭하여 새 저장소를 만들고 bin으로 그룹화할 값을 나열합니다.
...
SQL 마스크를 사용하여 여러 값을 필터에 결합할 수 있습니다. 이렇게 하려면 **[!UICONTROL Use an SQL mask]** 열에서 **[!UICONTROL Yes]** 을 선택하고 **[!UICONTROL Value or expression]** 열에 적용할 SQL 필터를 입력합니다.

아래 예에서는 **yahoo**(yahoo.fr, yahoo.com, yahoo.be 등)나 **ymail**(ymail.com, ymail.eu 등)로 시작하는 모든 이메일 도메인이 **YAHOO! 레이블 아래에 그룹화됩니다.**&#x200B;뿐만 아니라 rocketmail. **comdomain을 사용하는** 주소도 있습니다.
...
범위를 수동으로 정의하려면 **[!UICONTROL Add]** 버튼을 클릭하고 **[!UICONTROL Define a range]** 을 선택합니다.

![](assets/s_advuser_cube_class_05.png)

그런 다음 하한 및 상한 을 지정하고 **[!UICONTROL Ok]** 을 클릭하여 확인합니다.

### 자동으로 저장소 생성 {#generating-bins-automatically}
...
* 가장 자주 사용하는 값 복구
   ...
   다음 예에서는 가장 자주 사용하는 4개의 값이 표시되는 반면, 나머지 값은 &#39;기타&#39; 카테고리에서 계산되고 그룹화됩니다.

* 슬롯 형태로 저장소 생성

   다음 예에서는 Adobe Campaign이 자동으로 같은 크기의 값 슬롯 4개를 만들어 데이터베이스의 값을 표시합니다.
...
이 열거형을 사용하여 보고서를 만들려면 **[!UICONTROL Email domain]** 차원을 사용하여 큐브를 만드십시오. **[!UICONTROL Enable binning]** 옵션을 선택하고 **[!UICONTROL Dynamically link the values to an enumeration]** 을 선택합니다. 그런 다음 위에 표시된 대로 **Domains** 열거형을 선택합니다. 지정된 별칭이 없는 모든 값은 **Others** 레이블 아래에 다시 그룹화됩니다.
...
1. **[!UICONTROL Workflow]** 탭을 클릭하여 합계 워크플로우를 만듭니다.
...
1. 측정값 목록 위에 있는 **[!UICONTROL Add]** 단추를 클릭하고 측정 유형 및 계산할 공식을 선택합니다.

   ![](assets/s_advuser_cube_create_a_measure.png)

1. 필요한 경우 연산자에 따라 작업이 관련되는 표현식을 선택합니다.

   **[!UICONTROL Advanced selection]** 버튼을 사용하면 복잡한 계산 공식을 만들 수 있습니다. 이 작업에 대한 자세한 정보는 [이 섹션](../../platform/using/about-queries-in-campaign.md)을 참조하십시오.
...
보고서가 속한 카테고리와 관련성을 지정합니다. 자세한 내용은 [이 페이지](../../reporting/using/configuring-access-to-the-report.md#report-display-context)에 **표시 시퀀스**&#x200B;및 **필터링 옵션 정의** 섹션을 참조하십시오.
...
선택 항목을 기반으로 필터를 만들 수도 있습니다. 이렇게 하려면 소스 셀, 줄 및 열을 선택한 다음 **[!UICONTROL Add a filter]** 아이콘을 클릭합니다.
