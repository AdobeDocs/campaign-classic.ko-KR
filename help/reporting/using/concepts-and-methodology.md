...
베이닝을 사용하면 기준에 따라 값을 그룹화하여 데이터 표시를 단순화할 수 있습니다. 사용 가능한 정보에 따라 연령 그룹을 정의하고, 이메일 도메인을 함께 그룹화하며, 값 열거형으로 제한하며, 데이터를 명시적으로 제한하여 전용 라인 또는 열 등에서 다른 모든 데이터를 표시하고 그룹화할 수 있습니다.

전반적으로 세 가지 유형의 회신을 사용할 수 있습니다.

1. 수동으로 정의된 값 범위 사용 예: 연령, 평균 장바구니, 연 배송 수 등). For more on this, refer to [Defining each bin](#defining-each-bin).
1. 열거형의 값에 따라 동적으로 다음을 수행합니다.열거에 포함된 값만 표시하고 다른 모든 값은 &#39;Others&#39;로 그룹화됩니다. 자세한 내용은 동적으로 저장소 [관리를 참조하십시오](#dynamically-managing-bins).
1. 값 범위를 사용하면 다른 모든 항목이 함께 그룹화됩니다. 예를 들어, 18세에서 25세, 26세에서 59세, 그리고 다른 연령대들. 자세한 내용은 값 범위 [만들기를 참조하십시오](#creating-value-ranges).

치수를 생성할 때 적절한 상자를 선택합니다.

![](assets/s_advuser_cube_class_00.png)

저장소를 수동으로 만들거나 기존 열거형에 연결할 수 있습니다.

Adobe Campaign은 자동 회신을 위한 보조 기능도 제공합니다.값은 N 그룹으로 분류하거나 데이터베이스에서 가장 자주 사용하는 값에 따라 그룹화할 수 있습니다.
...
다음 예에서 언어는 세 가지 카테고리로 그룹화됩니다.영어/독일어/네덜란드어, 프랑스어/이탈리아어/스페인어 및 기타

![](assets/s_advuser_cube_class_03.png)

SQL 마스크를 사용하여 여러 값을 필터로 결합할 수 있습니다. 이렇게 하려면 열 **[!UICONTROL Yes]** 을 **[!UICONTROL Use an SQL mask]** 선택하고 열에 적용할 SQL 필터를 **[!UICONTROL Value or expression]** 입력합니다.

아래 예에서 **yahoo** (yahoo.fr, yahoo.com, yahoo.be 등)로 시작하거나 ymail **** (ymail.com, ymail.eu 등)로 시작하는 모든 이메일 도메인 레이블 **YAHOO! 아래에 그룹화됩니다.**&#x200B;및 **rocketmail.com** 도메인 관련 주소

![](assets/s_advuser_cube_class_03b.png)

### 동적으로 저장소 관리 {#dynamically-managing-bins}

열거형을 통해 값을 동적으로 관리할 수 있습니다. 즉, 열거에 포함된 값만 표시됩니다. 열거형 값이 변경되면 큐브의 컨텐츠가 자동으로 조정됩니다.

이 유형의 값 배닝을 만들려면 다음 단계를 적용합니다.

1. 새 차원을 만들고 회전을 활성화합니다.
...
그런 다음 하한 및 상한 **[!UICONTROL Ok]** 을 지정하고 클릭하여 확인합니다.
...
* 가장 자주 사용하는 값 복구
   ...
   다음 예에서는 가장 자주 사용되는 4개의 값이 표시되는 반면 나머지 값은 계산되어 &#39;기타&#39; 범주로 그룹화됩니다.

* 슬롯 형태로 저장소 생성

   다음 예에서, Adobe Campaign은 자동으로 같은 크기의 값 슬롯 4개를 만들어 데이터베이스의 값을 표시합니다.

이 경우 팩트 스키마에서 선택된 필터는 무시됩니다.

### 열거형 {#enumerations}

보고서의 연관성 및 가독성을 개선하기 위해 Adobe Campaign을 사용하면 특정 열거형을 만들어 다른 값을 동일한 저장소에 다시 그룹화할 수 있습니다. 시작할 것으로 예약된 이러한 열거형은 큐브에서 참조한 다음 보고서에 표시됩니다.

또한, Adobe Campaign은 다음 예와 같이 데이터베이스에 있는 모든 연락처의 이메일 도메인 목록을 ISP별로 재그룹화하는 도메인 목록을 표시할 수 있도록 하는 도메인 목록을 제공합니다.
...
이 열거형을 사용하여 보고서를 만들려면 차원을 사용하여 큐브를 **[!UICONTROL Email domain]** 만듭니다. Choose the **[!UICONTROL Enable binning]** option then **[!UICONTROL Dynamically link the values to an enumeration]**. 그런 다음 위에 표시된 **도메인** 열거형을 선택합니다. 지정된 별칭이 없는 모든 값은 기타 레이블 아래에 **다시** 그룹화됩니다.

![](assets/nmx_add_dimension.png)

그런 다음 이 큐브를 기반으로 보고서를 만들어 값을 표시합니다.

관련 보고서를 업데이트하려면 열거형만 수정해야 합니다. 예를 들어, **Adobe** 값을 만들고 **adobe.com** 별칭을 추가하면 보고서가 자동으로 열거형 수준의 Adobe 값으로 업데이트됩니다.
...
1. 탭을 **[!UICONTROL Workflow]** 클릭하여 집계 워크플로우를 만듭니다.

   ![](assets/s_advuser_cube_agregate_04.png)

   * 이 **[!UICONTROL Scheduler]** 활동을 통해 계산 업데이트 빈도를 정의할 수 있습니다. 스케줄러는 [이 섹션에 자세히 설명되어 있습니다](../../workflow/using/scheduler.md).
...
필요한 만큼 여러 측정을 만들 수 있습니다.그런 다음 표에 표시하거나 숨길 측정값을 선택합니다. For more on this, refer to [Displaying measures](#displaying-measures).
...
1. 필요한 경우 연산자에 따라 연산자가 원하는 표현식을 선택합니다.
   ...
   이 **[!UICONTROL Advanced selection]** 단추를 사용하여 복잡한 계산 공식을 만들 수 있습니다. 이 작업에 대한 자세한 정보는 [이 섹션](../../platform/using/about-queries-in-campaign.md)을 참조하십시오.
...
## 조치 표시 {#displaying-measures}

필요에 따라 테이블에 측정값의 표시를 구성할 수 있습니다.

* 측정 단위의 표시 순서(표시 [순서 참조](#display-sequence)),
* 보고서에 표시/숨길 정보(표시 [구성 참조](#configuring-the-display))
* 표시할 측정 단위:백분율, 총, 소수 수 등 (표시되는 [측정 유형 변경을 참조하십시오](#changing-the-type-of-measure-displayed).)

### 표시 시퀀스 {#display-sequence}

큐브에서 계산된 측정값은 **[!UICONTROL Measures]** 단추를 통해 구성됩니다.

표시 순서를 변경하려면 라인을 이동합니다. 다음 예에서 프랑스어 데이터는 목록 아래쪽으로 이동합니다.즉, 마지막 열에 표시됩니다.
...
각 측정 단위 내에서 적용할 단위와 형식을 정의할 수 있습니다.
...
이렇게 하려면 **[!UICONTROL Show the report properties]** 아이콘을 클릭하고 옵션을 **[!UICONTROL Share this report]** 활성화합니다.

![](assets/cube_share_option.png)

보고서가 속하는 카테고리와 연관성을 지정합니다. 자세한 내용은 [이 페이지](../../reporting/using/configuring-access-to-the-report.md#report-display-context) 에서 **시퀀스 표시 및 필터링 옵션**&#x200B;정의 섹션 **을** 참조하십시오.
...
1. 필터를 만들면 보고서 위에 필터가 표시됩니다.
   ...
   필요한 만큼 필터를 결합할 수 있습니다.모두 이 지역에서 보여집니다.

   ![](assets/neolap_multiple_filters.png)

필터를 수정(추가, 제거, 변경)할 때마다 보고서를 다시 계산해야 합니다.

또한 선택한 항목을 기반으로 필터를 만들 수도 있습니다. 이렇게 하려면 소스 셀, 선 및 열을 선택한 다음 **[!UICONTROL Add a filter]** 아이콘을 클릭합니다.
