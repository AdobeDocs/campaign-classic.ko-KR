...
Adobe Experience Platform을 사용하면 Campaign Classic과 Adobe Real-time Customer Data Platform(RTCDP) 간에 데이터를 공유할 수 있습니다. 이렇게 하면 Campaign 워크플로우에서 Adobe Experience Platform 대상자를 타겟팅한 다음, 전송, 열기 및 클릭과 같은 이러한 대상자와 관련된 Adobe 실시간 고객 데이터 플랫폼으로 다시 전송할 수 있습니다.
...
## Campaign Classic과 Adobe Experience Platform 연결

Adobe Experience Platform과 Campaign Classic 간에 데이터를 공유하려면 먼저 Adobe Campaign을 **대상**(으)로 연결하고 AWS S3 또는 Azure Blob 저장소 위치를 Adobe Experience Platform에서 **Source**(으)로 연결해야 합니다.

커넥터가 구성되면 워크플로우를 사용하여 Campaign Classic으로 데이터 가져오기 또는 내보내기를 설정할 수 있습니다.
