...
* **Campaign Standard에서 쿼리의 기반이 되는 데 필요한 프로필 필드를 찾을 수 없습니다.**
  ...
  또한 수신자에게 첨부되고 nms:recipients과(와) 관련된 테이블에 저장된 모든 데이터는 기본적으로 Campaign Standard에 동기화되지 않습니다.
...
* **Campaign v7에서 기본 nms가 아닌 다른 프로필 차원을 사용하고 있습니다:recipient. Campaign Standard과 동기화하려면 어떻게 해야 합니까?**

  Campaign Standard에서는 이름이 **프로필**&#x200B;인 고유한 타깃팅 리소스를 사용합니다. Campaign Standard 연결 기능의 기본 구현은 Campaign v7 수신자와 Campaign Standard 프로필 간의 기본 매핑을 제공합니다.
...
* **워크플로우를 통해 Campaign Standard과 프로필 목록을 공유하려고 하는데 Campaign Standard에서 대상을 찾을 수 없습니다**.
  ...
  프로필로 구성된 목록을 공유하고 어느 것도 Campaign Standard과 동기화되지 않는 경우 Campaign Standard에서 사용할 수 없는 빈 쿼리 대상자를 생성합니다.
...
* **Campaign Standard에서 프로필, 대상자 또는 랜딩 페이지를 편집할 수 없습니다. 어떤 의미입니까?**
  ...
  Campaign v7에서 동기화된 리소스는 데이터 일관성을 보장하기 위해 Campaign Standard에서 읽기 전용 모드에 있습니다. 이러한 요소 중 하나를 편집해야 하는 경우 Campaign v7에서 편집한 다음 Campaign Standard에서 변경 사항을 복제할 수 있습니다.

* **프로필 게재 로그 복제 워크플로우에서 [ACS] 오류가 발생했습니다. 어떻게 해야 합니까?**

  Campaign Classic 및 Campaign Standard 인스턴스를 모두 사용하여 추적된 URL이 있는 이메일을 보내는 경우 동기화 중에 중복 URL tagIds 문제가 발생할 수 있습니다. 이 경우 **[ACS] 프로필 게재 로그 복제**(newRcpDeliveryLogReplication) 워크플로우가 실패하고 다음 오류가 발생합니다.
