...
1. **[!UICONTROL Administration > Platform > External accounts]**(으)로 이동합니다. 언급된 SFTP 계정은 Adobe에서 구성했어야 하며, 필요한 정보는 사용자에게 전달되어야 합니다.
...
**받는 사람 - 방문자 ID**&#x200B;이(가) Audience Manager 내부에 만들어집니다. 기본적으로 방문자 ID에 대해 구성된 바로 사용 가능한 데이터 소스입니다. Campaign에서 만든 세그먼트는 이 데이터 소스의 일부가 됩니다.
