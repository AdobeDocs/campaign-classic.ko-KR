...
**파이프라인이 실패했습니다. 오류: &quot;마스크 파이프라인에 해당하는 작업이 없습니다.@&lt; 인스턴스 >&quot;**
...
이 오류 메시지는 기존 Omniture Base OAuth를 사용하여 인증이 구성되었음을 의미합니다. 인증을 업그레이드하려면 [Adobe Experience Cloud 트리거에 대한 Adobe I/O 구성](../../integrations/using/about-triggers.md#implement) 설명서를 참조하십시오.
...
스테이지 인스턴스에서 통합 인증을 변경해도 프로덕션 인스턴스의 구성에 영향을 주지 않습니다. 스테이지 인스턴스를 업그레이드한 다음 인증을 Adobe IO로 업데이트하고 스테이지 인스턴스에서 트리거를 테스트하도록 선택할 수 있습니다.
