...
ACS 커넥터는 Adobe Campaign v7 및 Adobe Campaign Standard을 연결시킵니다. 이 기능은 Campaign v7의 통합 기능으로, Campaign Standard에 데이터를 자동으로 복제하여 두 애플리케이션 중 최고의 성능을 제공합니다. Campaign v7에는 기본 마케팅 데이터베이스를 관리하는 고급 도구가 있습니다. Campaign v7의 데이터 복제를 통해 Campaign Standard은 사용자에게 친숙한 환경에서 리치 데이터를 활용할 수 있습니다.
...
>
>이 기능을 사용하려면 Adobe ID(IMS)를 사용하여 Campaign에 연결해야 합니다. Adobe ID [를 통해 연결을 참조하십시오](../../integrations/using/about-adobe-id.md).
...
>
>이러한 구현을 직접 수행하려고 시도하지 마십시오. 그것은 Adobe Campaign 컨설턴트들에게 엄히 보관되어 있다.
...
이 기능을 사용하려면 패키지를 **[!UICONTROL ACS Connector]** 설치해야 합니다. 이 작업은 항상 Adobe 기술 관리자 또는 컨설턴트가 수행합니다.
