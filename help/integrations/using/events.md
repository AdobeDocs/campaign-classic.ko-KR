...
* **[!UICONTROL @offset]** 은 메시지에 대한 &quot;포인터&quot;입니다. 큐 내에서 메시지의 순서를 나타냅니다.
...
이벤트는 오프셋 순서에 따라 한 번에 하나씩 처리됩니다. 각 스레드는 서로 다른 파티션을 [!DNL pipelined] 처리합니다.

검색된 마지막 이벤트의 &#39;오프셋&#39;은 데이터베이스에 저장됩니다. 따라서 프로세스가 중지되면 마지막 메시지부터 다시 시작됩니다. 이 데이터는 기본 제공 스키마 xtk:pipelineOffset에 저장됩니다.

이 포인터는 각 인스턴스 및 각 소비자와 관련이 있습니다. 따라서 많은 인스턴스가 서로 다른 소비자를 사용하여 동일한 파이프라인에 액세스하는 경우 각각 모든 메시지를 동일한 순서로 가져옵니다.
...
이 샘플 Javascript 코드는 다양한 기능에서 eVar01을 구문 분석합니다.
...
이 코드의 성능은 높은 빈도로 실행되며 다른 마케팅 활동에 부정적인 영향을 미칠 수 있으므로 최적이어야 합니다. 특히 Marketing 서버에서 시간당 100만 개가 넘는 트리거 이벤트를 처리하거나 제대로 조정되지 않은 경우 더욱 그렇습니다.
...
| 쇼핑객_id | 문자열 32 | shopper_id | 구매자의 내부 식별자. 조정 워크플로우에 의해 설정됩니다. 0이면 Campaign에서 고객을 알 수 없음을 의미합니다. |
...
조정은 Adobe Analytics의 고객을 Adobe Campaign 데이터베이스로 연결하는 프로세스입니다. 예를 들어 일치 기준은 쇼핑객_id일 수 있습니다.

성능상의 이유로 일치는 작업 과정 의해 배치 모드로 수행되어야 합니다.
워크로드를 최적화하려면 빈도 간격을 15분으로 설정해야 합니다. 따라서 Adobe Campaign의 이벤트 수신과 마케팅 작업 과정에 의한 처리 사이의 지연 시간은 최대 15분입니다.

### JavaScript의 단위 조정 옵션 {#options-unit-reconciliation}

JavaScript 의 각 트리거에 대해 조정 쿼리를 실행할 수 있습니다. 성능에 더 큰 영향을 미치고 더 빠른 결과를 제공합니다. 반응성이 필요한 특정 사용 사례에 필요할 수 있습니다.
