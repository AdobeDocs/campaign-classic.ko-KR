...
>[!IMPORTANT]
>
>프록시가 구성되어 있고 재정의할 수 있는 경우(또는 존재하지 않는 경우), 테스트할 IP 주소를 왜곡할 수 있습니다.
>
...
>[!IMPORTANT]
>
>이 운영 모드는 릴레이 서버 내부 성능(지연, 반밴드..)으로 인해 처리량을 크게 줄일 수 있으므로 배달 시 심각한 제한 사항을 의미합니다. 또한, SMTP 트래픽을 분석하여 발견한 동기 배달 오류를 평가할 수 있는 능력이 제한되며 릴레이 서버를 사용할 수 없을 경우 전송을 수행할 수 없습니다.
...
>[!IMPORTANT]
>
>선호도 구성은 한 서버에서 다른 서버로 일관되어야 합니다. MTA를 실행하는 모든 응용 프로그램 서버에서 구성 변경 사항을 복제해야 하므로 선호도 구성을 위해 Adobe에 문의하는 것이 좋습니다.
...
기본적으로 인스턴스는 외부 URL에 연결할 수 없습니다. 하지만, 인스턴스가 URL에 연결할 수 있도록 외부 URL을 인증된 URL 목록에 추가할 수 있습니다. 이렇게 하면 파일 및/또는 데이터를 전송할 수 있도록 SFTP 서버나 웹 사이트 등의 외부 시스템에 Campaign 인스턴스를 연결할 수 있습니다.
...
* **자유로운**:에 허용 목록에 추가하다 속하지 않는 모든 URL이 허용됩니다.
* **경고**:에 속하지 않은 모든 URL은 허용 목록에 추가하다 허용되지만 JS 인터프리터는 관리자가 수집할 수 있도록 경고를 전송합니다. 이 모드에서는 JST-310027 경고 메시지가 추가됩니다.
...
>[!NOTE]
>
>값은 구성 및 네트워크 제약 조건에 따라 조정되며, 특히 특정 구성이 설치용으로 개발된 경우입니다.

## 허가된 외부 명령 제한 {#restricting-authorized-external-commands}

>[!NOTE]
>
>다음 구성은 온-프레미스 설치에 대해서만 필요합니다.

기술 관리자는 빌드 8780에서 Adobe Campaign에서 사용할 수 있는 인증된 외부 명령 목록을 제한할 수 있습니다.

이렇게 하려면 다음 등 사용할 수 없게 하려는 명령 목록이 포함된 텍스트 파일을 만들어야 합니다.
...
>[!IMPORTANT]
>
>사용자 정의 수도를 사용하면 안 됩니다. 표준 장치를 시스템에 설치해야 합니다.
...
이 경우 배포 마법사 창 위쪽 부분에 있는 공개 리소스에 대한 새 URL이 이 폴더를 가리켜야 합니다.
...
1. 다음과 같이 모듈과 일치하는 **[!UICONTROL wfserver]** 라인을 수정합니다.
   ...
   여러 속성을 정의하는 경우 공백 없이 쉼표로 구분해야 합니다.
   ...
   관련성이 정의된 워크플로우만 실행하려면 친화성 목록의 끝에 쉼표를 추가하지 마십시오. 예를 들어 다음과 같이 줄을 수정합니다.
...
HTTP 연결은 proxyHTTP 매개 변수에서 정의됩니다.
