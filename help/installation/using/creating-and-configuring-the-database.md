...
1. 데이터베이스 만들기 또는 재활용: 새 데이터베이스를 만들거나 기존 데이터베이스를 다시 사용하려면 이 옵션을 선택합니다. 을(를) 참조하십시오. [사례 1: 데이터베이스 만들기/재활용](#case-1--creating-recycling-a-database).
1. 기존 데이터베이스 사용: 관리자가 빈 데이터베이스를 이미 만들어 사용하고 이를 사용하려면 이 옵션을 선택합니다. 또는 기존 데이터베이스의 구조를 확장합니다. 을(를) 참조하십시오. [사례 2: 기존 데이터베이스 사용](#case-2--using-an-existing-database).
...
지원되는 데이터베이스는 Campaign에 나열됩니다 [호환성 매트릭스](../../rn/using/compatibility-matrix.md).
...
* 대상 **Oracle** 엔진, 채우기 **TNS 이름** 응용 프로그램 서버에 대해 정의됩니다.
* 대상 **PostgreSQL** 또는 **DB2** 엔진에서는 데이터베이스 서버에 액세스하려면 응용 프로그램 서버에 정의된 DNS 이름(또는 IP 주소)을 지정해야 합니다.
* 대상 **Microsoft SQL Server** 엔진, 다음을 정의해야 합니다. 데이터베이스 서버에 액세스하기 위해 응용 프로그램 서버에 정의된 DNS 이름(또는 IP 주소)입니다. **DNS** 또는 **DNS`\<instance>`** (인스턴스 모드),
...
에서 **[!UICONTROL Server access]** 창에서 데이터베이스 서버 액세스를 정의합니다.

![](assets/s_ncs_install_db_oracle_creation02.png)

이렇게 하려면 이름 및 암호를 입력합니다 **관리 시스템 계정** 데이터베이스에 액세스할 수 있는 권한(예:
...
* 데이터베이스가 유니코드여야 하는지 여부를 나타냅니다.
   ...
   다음 **[!UICONTROL Unicode database]** 옵션을 사용하면 언어에 관계없이 모든 문자 유형을 유니코드로 저장할 수 있습니다.

   >[!NOTE]
   >
   >oracle 데이터베이스 사용 시 **[!UICONTROL Unicode storage]** 옵션을 사용하면 **NCLOB** 및 **NVARCHAR** 유형 필드.
   > 
   >이 옵션을 선택하지 않으면 Oracle 데이터베이스의 문자 세트(charset)가 모든 언어로 데이터 저장을 사용하도록 설정해야 합니다(AL32UTF8이 권장됨).

* 데이터베이스의 시간대를 선택하고 UTC로 지정할지 여부를 지정합니다(가능한 경우).

   자세한 내용은 [시간대 관리](../../installation/using/time-zone-management.md).

### 4단계 - 설치할 패키지 {#step-4---packages-to-install}

설치할 패키지를 선택합니다.

&quot;상호 작용&quot; 또는 &quot;소셜 마케팅&quot;과 같이 설치할 수 있는 솔루션 및 옵션을 확인하려면 라이선스 계약을 참조하십시오.

![](assets/s_ncs_install_modules.png)

### 5단계 - 만들기 단계 {#step-5---creation-steps}

다음 **[!UICONTROL Creation steps]** 창을 사용하면 테이블을 만드는 데 사용되는 SQL 스크립트를 표시하고 편집할 수 있습니다.

![](assets/s_ncs_install_db_oracle_creation04.png)

* oracle, Microsoft SQL Server 또는 PostgreSQL 데이터베이스의 경우 관리자는 **저장소 매개 변수** 데이터베이스 개체를 만들 때 사용할 대상.

   이러한 매개변수는 정확한 테이블스페이스 이름을 받습니다(경고: 대/소문자를 구분합니다.) 각각 **[!UICONTROL Administration > Platform > Options]** 노드 아래의 다음 옵션을 참조하십시오. [이 섹션](../../installation/using/configuring-campaign-options.md#database)):
...
* oracle 데이터베이스의 경우 Adobe Campaign 사용자는 일반적으로 Oracle 라이브러리에 대한 액세스 권한이 있어야 합니다 **설치** 그룹에 속해 있어야 합니다.
...
인스턴스에 연결된 데이터베이스에 대한 연결 설정이 파일에 저장됩니다 **`/conf/config-<instance>.xml`** Adobe Campaign 설치 디렉터리에 있습니다.
...
* 대상 **Oracle** 엔진, 채우기 **TNS 이름** 응용 프로그램 서버에 대해 정의됩니다.
* 대상 **PostgreSQL** 또는 **DB2** 엔진에서는 데이터베이스 서버에 액세스하려면 응용 프로그램 서버에 정의된 DNS 이름(또는 IP 주소)을 지정해야 합니다.
* 대상 **Microsoft SQL Server** 엔진, 다음을 정의해야 합니다.
...
에서 **[!UICONTROL Database]** 창에서 데이터베이스 연결 설정을 정의합니다.
...
* 이 데이터베이스와 연결된 계정의 이름과 암호를 입력하십시오.
   ...
   >[!NOTE]
   >
   >스키마 이름과 사용자 이름이 모두 일치하는지 확인합니다. 데이터베이스를 만드는 권장 방법은 campaign 콘솔 클라이언트를 사용하는 것입니다.
...
&quot;상호 작용&quot; 또는 &quot;리드&quot;와 같이 설치 권한이 있는 솔루션 및 옵션을 확인하려면 라이선스 계약을 참조하십시오.
...
데이터베이스 만들기가 완료되면 다시 연결하여 인스턴스 구성을 완료할 수 있습니다.
