...
지원되는 데이터베이스는 Campaign [호환성 매트릭스](../../rn/using/compatibility-matrix.md)에 나열되어 있습니다.
...
이렇게 하려면 데이터베이스에 액세스할 권한이 있는 **관리 시스템 계정**&#x200B;의 이름과 암호를 입력합니다.

* **** Oracle 데이터베이스 시스템,
* **** Microsoft SQL Server 데이터베이스의 경우,
...
이제 배포 마법사를 시작하여 인스턴스 구성을 완료해야 합니다. [배포 마법사](../../installation/using/deploying-an-instance.md#deployment-wizard)를 참조하십시오.

인스턴스에 연결된 데이터베이스에 대한 연결 설정은 Adobe Campaign 설치 디렉토리에 있는 **`/conf/config-<instance>.xml`** 파일에 저장됩니다.
