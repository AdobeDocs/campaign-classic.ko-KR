...
1. **matchild**&#x200B;은(는) 메시지 작성에 필요한 모든 정보(콘텐츠, 개인화 요소, 첨부 파일, 이미지 등)를 로드하고 메시지를 **전자 메일 트래픽 셰이퍼**&#x200B;에 전달합니다.
...
중단된 메시지가 **mta**(으)로 반환되고 더 이상 **mtachild**&#x200B;에서 관리되지 않습니다.

**mta**&#x200B;은(는) 응답 코드와 규칙에 따라 이 메시지의 절차(복구, 중단, 격리 등)를 결정합니다.
