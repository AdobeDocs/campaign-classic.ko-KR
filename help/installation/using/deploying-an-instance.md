...
**관리 > 캠페인 관리 > 비게재 항목 관리 > 게재 로그 자격** 노드에 나열된 규칙을 적용하여 바운스는 Adobe Campaign에서 자동으로 처리됩니다. 자세한 내용은 [바운스 메일 관리](../../delivery/using/understanding-delivery-failures.md#bounce-mail-management)를 참조하세요.
...
동기화해야 하는 각 공용 리소스에 대해 `<url>` 부분의 `<relay>` 요소에 상태 특성을 추가해야 합니다.
