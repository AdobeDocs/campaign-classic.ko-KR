...
>[!NOTE]
>
>서버측 구성은 Adobe에 의해 호스팅되는 배포에 대해서만 Adobe이 수행할 수 있습니다. 다양한 배포에 대한 자세한 내용은 [호스팅 모델](../../installation/using/hosting-models.md) 또는 [이 페이지](../../installation/using/capability-matrix.md).
...
* **[!UICONTROL Namespace]** : 짧은 식별자를 소문자로 입력합니다. 목적은 업그레이드의 경우 특정 구성과 공장 구성을 구별하는 것입니다. 기본 네임스페이스는 다음과 같습니다 **cus** - 고객용.
...
* **[!UICONTROL Email channel]** : 이메일 배달을 활성화하려면 을(를) 참조하십시오. [전자 메일 채널 매개 변수](#email-channel-parameters).
* **[!UICONTROL Tracking]** : 대상 모집단(열기 및 클릭)을 추적하기 위해 을(를) 참조하십시오. [구성 추적](#tracking-configuration).
* **[!UICONTROL Managing bounced emails]** : 받는 전자 메일을 선택하는 데 사용되는 POP 계정을 정의하려면 을(를) 참조하십시오. [바운스된 이메일 관리](#managing-bounced-emails).
* **[!UICONTROL LDAP integration]** : LDAP 디렉토리를 통해 사용자 인증을 구성하려면 을(를) 참조하십시오. [LDAP를 통해 연결](../../installation/using/connecting-through-ldap.md).
...
* **[!UICONTROL Reply address text]** : 수신자가 **[!UICONTROL Reply]** 전자 메일 클라이언트 소프트웨어의 버튼
* **[!UICONTROL Reply address]** : 수신자가 **[!UICONTROL Reply]** 전자 메일 클라이언트 소프트웨어의 버튼
...
Adobe Campaign 데이터베이스에서 모든 이메일 주소는 다음과 같이 만들어야 합니다. `x@y.z`. 다음 **x**, **y** 및 **z** 문자는 비워 둘 수 없으며 권한이 없는 문자를 포함하지 않아야 합니다.
...
다음 두 가지 목록을 사용할 수 있습니다. **유럽만** 또는 **미국만**. 필요한 경우 다른 문자를 추가할 수 있습니다.
...
* **[!UICONTROL Delivery duration of messages]** : 이 시간 이후에는 게재가 중지됩니다(기본적으로 5일).
* **[!UICONTROL Online resources validity duration]** : 미러 페이지를 생성하기 위해 수신자 프로필의 정보를 유지하는 시간입니다.
* **[!UICONTROL Exclude recipients who no longer wish to be contacted]** : 이 옵션을 선택하면 받는 사람차단 목록에게 연락하지 않습니다.
* **[!UICONTROL Automatically ignore doubles]** : 이 옵션을 선택하면 중복 주소에 대한 게재가 수행되지 않습니다.

>[!NOTE]
>
>호스팅 또는 하이브리드 설치의 경우 로 업그레이드한 경우 [향상된 MTA](../../delivery/using/sending-with-enhanced-mta.md), **[!UICONTROL Delivery duration of the messages]** 은 로 설정된 경우에만 사용됩니다 **3.5일 이하**. 3.5일 이상의 값을 정의하면 고려되지 않습니다.

### 다시 시도 매개 변수 {#retry-parameters}

복구에 대한 정보는 **복구 기간** 및 **복구 횟수** 필드: 수신자에 연결할 수 없는 경우(예: 받은 편지함이 가득 찬 경우) 기본적으로 프로그램은 각 시도(최대 배달 시간 동안) 간 1시간 간격으로 5회 수신자에게 연락하려고 합니다. 이러한 값은 필요에 따라 변경할 수 있습니다.

>[!NOTE]
>
>호스팅 또는 하이브리드 설치의 경우 로 업그레이드한 경우 [향상된 MTA](../../delivery/using/sending-with-enhanced-mta.md), Campaign 다시 시도 매개 변수는 더 이상 사용되지 않습니다. 소프트 바운스 다시 시도 및 그 사이의 시간은 메시지의 이메일 도메인에서 돌아오는 바운스 응답의 유형과 심각도를 기반으로 Enhanced MTA에 의해 결정됩니다.
...
이 단계는 **이메일 채널** 및 **반송 메일** 관리 옵션은 배포 마법사의 첫 번째 단계에서 선택됩니다. 을(를) 참조하십시오. [일반 매개 변수](#general-parameters).
...
* **[!UICONTROL Security]** : 필요한 경우 을 선택합니다. **[!UICONTROL SSL]** 드롭다운 목록에서
...
바운스는 Adobe Campaign에 나열된 규칙을 적용하여 자동으로 처리됩니다 **관리 > Campaign Management > 산출물이 아닌 관리 > 게재 로그 자격** 노드 아래에 있어야 합니다. 자세한 내용은 [반송 메일 관리](../../delivery/using/understanding-delivery-failures.md#bounce-mail-management).
...
>[!IMPORTANT]
>
>처리되지 않은 바운스 이메일을 전달하려면 **[!UICONTROL Address for errors]** 필드. 그러나 메일 서버에 많은 부하가 발생할 수 있으므로 사용되는 주소를 정기적으로 확인해야 합니다. 자세한 내용은 계정 담당자에게 문의하십시오.
...
웹 추적(추적 모드, 태그 만들기 및 삽입..)에 대한 자세한 내용은 [이 문서](../../configuration/using/about-web-tracking.md).
...
* 새 URL을 작성하는 데 배포 마법사의 이 페이지에 입력한 외부 URL(보안인지 여부)에 대한 정보를 사용합니다. 이 정보와 함께 수정된 링크에는 다음이 포함됩니다. 게재의 식별자, 수신자 및 URL입니다.
   ...
   추적 정보는 추적 서버에서 Adobe Campaign이 수집하여 수신자 프로필 및 게재에 연결된 데이터를 보강합니다( **[!UICONTROL Tracking]** 탭).

   내부 URL에 대한 정보는 Adobe Campaign 애플리케이션 서버가 추적 서버에 연락하기 위해서만 사용됩니다.

   자세한 내용은 [추적 서버](#tracking-server).

* URL이 구성되면 추적을 활성화해야 합니다. 이를 수행하려면 인스턴스를 추적 서버에 등록해야 합니다.

   자세한 내용은 [추적 저장](#saving-tracking).
...
1. 옵션을 선택합니다 **[!UICONTROL Externalize tracking management]** : 중간 소싱 서버를 추적 서버로 사용할 수 있습니다.
1. 을(를) 채우기 **[!UICONTROL External account]** 및 **[!UICONTROL Instance name]** 중간 소싱 서버에 연결할 수 있는 필드입니다.

   자세한 내용은 [중간 소싱 서버](../../installation/using/mid-sourcing-server.md).

1. 을(를) 클릭합니다. **[!UICONTROL Enable the tracking instance]** 서버에 대한 연결을 승인하는 단추.
...
링크를 클릭합니다 **추적 서버에 등록** 그런 다음 사용 가능한 옵션 중 하나를 선택합니다.
...
1. **기존 인스턴스에서 추적을 위한 지원 추가**
   ...
   암호를 입력합니다. **내부** 추적 인스턴스를 구성하기 위해 리디렉션 서버의 계정을 설정합니다.
...
1. **추적을 위한 새 인스턴스 만들기**
   ...
   인스턴스 이름, 암호 및 연결된 DNS 마스크(예: **[!UICONTROL Campaign*]**.

1. **이미 사전 구성된 추적 인스턴스의 유효성을 검사합니다**

   이 옵션은 **내부** account; 이 경우 추적 계정은 추적 서버에서 미리 구성되어 있습니다. 리디렉션 서버의 추적 계정 암호를 입력하여 추적 인스턴스의 유효성을 확인합니다.

   ![](assets/s_ncs_install_deployment_wiz_17.png)

   유효성을 검사할 인스턴스의 이름을 지정합니다.

클릭 **승인** 를 입력하여 추적 서버로 기록 프로세스를 시작합니다.

이전 창으로 돌아가면 추적 서버 수준에서 등록을 확인하는 메시지가 나타납니다.

![](assets/s_ncs_install_deployment_wiz_tracking_ok.png)

URL 검색에 연결된 매개 변수 **수정하면 안 됩니다.** 표준 설치 기타 모든 매개 변수는 Adobe에게 문의하십시오.
...
다시 시도하는 방법에 대한 정보는 **다시 시도 기간** 및 **다시 시도 횟수** 필드: 모바일에 연결할 수 없는 경우 기본적으로 프로그램은 최소 15분 간격으로 5번(최대 배달 기간) 다시 시도합니다. 이러한 값은 사용자의 요구 사항에 맞게 조정할 수 있습니다.
...
* **[!UICONTROL Store all phone numbers using the international format]** : 이 옵션은 **국내** 가져오거나 편집한 전화 번호입니다. 국내 형식(예: 425 55 0150)을 사용할지, 국제 형식(예: +1 425 555 0150)
...
1. 게재 미러 페이지에 액세스합니다.
   ...
   미러 페이지는 전자 메일의 컨텐츠를 표시하는 동적 페이지입니다. 수신자에게 전송된 메시지에 삽입된 링크를 통해 액세스되고 개인화된 요소를 포함할 수 있습니다. 미러 페이지에서는 수신자에게 게재 형식(텍스트 또는 HTML)에 관계없이 이메일 소프트웨어 대신 인터넷 브라우저에서 메시지를 읽을 수 있는 기회를 제공합니다. 그러나 필요한 HTML 컨텐츠가 정의된 경우에만 지정된 게재에 대해 미러 페이지가 생성됩니다.
...
1. 새 공개 리소스 URL입니다. 자세한 내용은 [공개 리소스 URL](#public-resources-url) 섹션을 참조하십시오.
1. 게재의 이미지 탐지 모드. 자세한 내용은 [게재 이미지 탐지](#delivery-image-detection) 섹션을 참조하십시오.
1. 게시 옵션. 자세한 내용은 [게시 모드](#publication-modes) 섹션을 참조하십시오.

공개 리소스는 **관리 > 리소스 > 온라인 > 공용 리소스** 노드 아래에 나열된 상태로 남아 있습니다. 라이브러리로 수집되고 이메일에 포함할 수 있지만 캠페인이나 작업, 콘텐츠 관리에도 사용됩니다.
...
* 공개 리소스의 경우 URL입니다 **https://** server **/res/**&#x200B;인스턴스&#x200B;****여기서&#x200B;**인스턴스**는 추적 인스턴스의 이름입니다.

### 게재 이미지 탐지 {#delivery-image-detection}

게재에서는 공용 리소스 라이브러리에 저장된 이미지나 서버에 저장된 기타 로컬 이미지나 이미지를 사용할 수 있습니다.

필드 **URL 마스크** 이미지를 자동으로 업로드할 때 건너뛸 URL 마스크 목록을 지정할 수 있습니다. 예를 들어 외부에서 액세스할 수 있는 사이트, 특히 인터넷 사이트에서 액세스할 수 있는 사이트에 저장된 이미지를 사용하는 경우 이 필드에 사이트 URL을 입력할 수 있습니다.

![](assets/s_ncs_install_deployment_wiz_img_mask.png)

여러 URL 마스크를 쉼표로 구분하여 지정할 수 있습니다.

* 이메일에서 이미지 사용 및 관리에 대한 자세한 내용은 [이 섹션](../../delivery/using/defining-the-email-content.md#adding-images).
...
* 추적 서버
   ...
   리소스는 다른 추적 서버에 자동으로 복사됩니다. 이러한 구성 요소는 단계에서 구성됩니다 [구성 추적](#tracking-configuration).
...
* 기타 Adobe Campaign 서버
   ...
   전용 서버의 매개 변수는 **[!UICONTROL Media URL(s)]**, **[!UICONTROL Password]** 및 **[!UICONTROL Instance name]** 필드.
...
이미지의 경우, 를 통해 지정된 &quot;이미지&quot; 폴더에서 해당 이미지를 복사해야 합니다 **NmsDelivery_ImageSubDirectory** 하나 이상의 전면 서버에 대한 옵션. 이러한 서버는 이미지를 저장하여 새로 구성된 URL을 통해 액세스할 수 있도록 합니다.

수동 게시 스크립트 없이 Adobe Campaign 서버에서 게시할 경우 기본적으로 게재 이미지가 `$(XTK_INSTALL_DIR)/var/res/img/ directory`. 해당 URL은 다음과 같습니다. **`https://server/res/img`**.

`XTK_INSTALL_DIR)/var/res/$(INSTANCE_NAME)`. 해당 URL은 다음과 같습니다. **`https://server/res/instance`** 여기서 instance 는 추적 인스턴스의 이름입니다.

>[!NOTE]
>
>공용 리소스 저장소 디렉터리를 변경할 수 있습니다. 자세한 내용은 [공개 리소스 관리](#managing-public-resources).

### 공용 리소스 동기화 {#synchronizing-public-resources}

이 기능을 사용하면 다음 작업을 수행할 수 있습니다 **공용 리소스 동기화** 여러 예비 서버에서

공용 리소스가 추적 서버에 없거나 리소스가 404 오류를 반환하는 경우 추적 서버가 예비 서버 중 하나에서 리소스를 찾으려고 시도합니다.

예비 서버 선언 및 구성은 마케팅 서버의 **serverConf.xml** 파일. 에서 사용할 수 있는 모든 매개 변수 **serverConf.xml** 여기에 나열되어 있습니다. [섹션](../../installation/using/the-server-configuration-file.md).
...
동기화해야 하는 각 공개 리소스에 대해 상태 속성을 `<url>` 요소의 요소 `<relay>` 부분:
...
* 블랙리스트: 404 오류를 반환하는 차단 목록 경우 URL이에 추가됩니다. 에 있는 URL의 지속 시간(초)차단 목록은 **timeout** 기본값이 60인 속성입니다.
...
데이터베이스 정리 워크플로우를 통해 데이터가 자동으로 삭제됩니다. 이 워크플로우를 구성 및 운영하는 방법과 삭제된 항목에 대한 자세한 내용은 이 항목을 참조하십시오 [문서](../../production/using/database-cleanup-workflow.md).
