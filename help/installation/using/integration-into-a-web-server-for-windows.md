...
* 그런 다음 클라이언트 콘솔은 다음과 같은 URL을 사용하여 연결합니다. [https:// `<computer>`08:80](https://myserver.adobe.com:8080).
...
웹 IIS 서버가 컴퓨터에 아직 설치되어 있지 않으면 **[!UICONTROL Add > Remove Programs > Enable or disable Windows functionalities]** 메뉴 아래의 제품에서 사용할 수 있습니다.
...
1. 를 통해 IIS를 엽니다. **[!UICONTROL Control panel > Administrative tools > Services]** 메뉴 아래의 제품에서 사용할 수 있습니다.
...
1. A **VBS** 스크립트를 사용하면 방금 만든 가상 디렉터리에서 Adobe Campaign 서버가 사용하는 리소스를 자동으로 구성할 수 있습니다. 시작하려면 **iis_neolane_setup.vbs** 에 있는 파일 `[INSTALL]\conf` 폴더, 위치 `[INSTALL]` 은 Adobe Campaign 설치 폴더에 액세스하기 위한 경로입니다.
   ...
   클릭 **[!UICONTROL OK]** 웹 서버가 추적 리디렉션 서버로 사용되는 경우 **[!UICONTROL Cancel]**.

   웹 서버에 여러 사이트가 이미 구성되어 있으면 중간 페이지가 표시되어 설치 시 적용되는 웹 사이트를 지정합니다. 사이트에 연결된 번호를 입력하고 **[!UICONTROL OK]**.
...
1. 에서 **[!UICONTROL Content View]** 탭에서 웹 사이트가 Adobe Campaign 리소스로 올바르게 구성되었는지 확인합니다.
...
1. 을(를) 선택합니다 **[!UICONTROL Features View]** 탭을 클릭하고 두 번 클릭합니다 **인증** 링크를 클릭합니다.

   ![](assets/s_ncs_install_iis7_parameters_step8.png)

1. 에서 **디렉토리 보안** 웹 사이트의 탭에서 익명 액세스가 활성화되어 있는지 확인합니다. 필요한 경우 **[!UICONTROL Edit]** 링크를 클릭하여 설정을 변경합니다.
...
1. 를 사용하여 IIS 서버를 다시 시작합니다. **isreset** 명령줄
...
1. Adobe Campaign 사이트의 ISAPI 필터를 편집하려면 **[!UICONTROL Driver mapping]** 아이콘.
...
예를 들어 **데이터 로드(파일)** 워크플로우에 활동을 입력하여 50MB 파일을 가져오면 오류가 발생하여 워크플로우가 제대로 실행되지 않습니다.

이 경우 이 제한을 늘려야 합니다.

1. 를 통해 IIS를 엽니다. **[!UICONTROL Start > (Control panel) > Administration tools]** 메뉴 아래의 제품에서 사용할 수 있습니다.
1. 에서 **연결** 창에서 Adobe 설치를 위해 만든 사이트를 선택한 다음 두 번 클릭합니다 **요청 필터링** 기본 창에서 클릭합니다.
1. 에서 **작업** 창, 선택 **기능 설정 편집** 에서 값을 편집할 수 있도록 **허용된 최대 콘텐츠 크기(바이트)** 필드.
...
1. 를 통해 IIS를 엽니다. **[!UICONTROL Start > Control Panel > Administrative tools]** 메뉴 아래의 제품에서 사용할 수 있습니다.
1. 에서 **연결** 창에서 Adobe Campaign 설치를 위해 만든 사이트를 선택한 다음 두 번 클릭합니다 **구성 편집기** 기본 창에서 클릭합니다.
1. 에서 **섹션** 드롭다운 목록에서 **system.webServer** > **httpErrors**.
1. 을(를) 선택합니다 **통과** 값에서 **existingResponse** 줄.
