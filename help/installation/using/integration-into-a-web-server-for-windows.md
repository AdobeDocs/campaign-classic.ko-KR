...
마찬가지로 추적 기능을 사용하려면 웹 서버를 사용해야 합니다. 추적 기능은 웹 서버 확장 모듈로만 사용할 수 있습니다.
...
IIS 웹 서버의 구성 절차는 대부분 그래픽입니다. 이 작업에는 웹 사이트(이미 만들어짐 또는 보류 중인 작성)를 사용하여 Adobe Campaign 서버의 리소스에 액세스하는 작업이 포함됩니다.Java(.jsp) 파일, 스타일 시트(.css, .xsl), 이미지(.png), 리디렉션 등을 위한 ISAPI DLL
...
1. 네트워크의 매개 변수(TCP 연결 포트, DNS 호스트, IP 주소)에 따라 사이트(예: Adobe Campaign)을 만들고 구성합니다.
   ...
   사이트의 이름과 가상 디렉토리의 액세스 경로를 최소한 지정해야 합니다. 웹 사이트 디렉토리에 액세스하기 위한 경로가 사용되지 않으므로 다음 디렉토리를 사용할 수 있습니다.
...
1. VBS **스크립트를** 사용하면 방금 만든 가상 디렉토리에서 Adobe Campaign 서버가 사용하는 리소스를 자동으로 구성할 수 있습니다. 이 파일을 시작하려면 폴더 **에 있는 iis_neolane_setup.vbs** 파일 `[INSTALL]\tomcat-7\conf` `[INSTALL]` 을 두 번 클릭합니다. 여기서 는 Adobe Campaign 설치 폴더에 액세스할 수 있는 경로입니다.
   ...
   웹 서버를 추적 리디렉션 서버로 **[!UICONTROL OK]** 사용하는 경우 을 클릭하고, 그렇지 않은 경우 을 클릭합니다 **[!UICONTROL Cancel]**.

   웹 서버에 여러 사이트가 이미 구성된 경우 설치가 적용되는 웹 사이트를 지정하는 중간 페이지가 표시됩니다.사이트에 연결된 번호를 입력하고 을 클릭합니다 **[!UICONTROL OK]**.
...
1. 탭에서 **[!UICONTROL Content View]** 웹 사이트가 Adobe Campaign 리소스로 올바르게 구성되었는지 확인합니다.
...
1. iisreset 명령줄을 사용하여 IIS 서버 **를** 다시 시작합니다.
...
리디렉션 모듈의 존재를 테스트하려면 다음 명령줄을 실행하십시오.
...
1. 아이콘을 클릭하여 Adobe Campaign 사이트의 ISAPI 필터를 **[!UICONTROL Driver mapping]** 편집합니다.
1. ISAPI 필터의 내용 확인:
...
1. [ **작업** ] 창 **에서** [ **최대 권한 있는 콘텐트 크기(바이트)** ] 필드에서 값을 편집할 수 있도록 [기능 설정편집]을 선택합니다.

   예를 들어 50MB의 파일 업로드를 인증하려면 &quot;52428800&quot;바이트보다 큰 값을 지정해야 합니다.

>[!NOTE]
>
>이 IIS 옵션에 대한 자세한 내용은 [공식 문서의 &quot;방법&quot; 섹션을 참조하십시오](https://www.iis.net/configreference/system.webserver/security/requestfiltering/requestlimits).
...
1. [ **연결** ] 창에서 Adobe Campaign 설치에 대해 만든 사이트를 선택한 다음 기본 창에서 **구성 편집기를** 두 번 클릭합니다.
1. 섹션 **드롭다운** 목록에서 **system.webServer** > **httpErrors를 선택합니다**.
1. 기존 **응답** 라인에서 **통과** 값을선택합니다.
