...
>[!NOTE]
>
>서버측 구성은 Adobe에 의해 호스팅되는 배포에 대해서만 Adobe이 수행할 수 있습니다. 서로 다른 배포에 대한 자세한 내용은 [호스팅 모델](../../installation/using/hosting-models.md) 섹션 또는 [이 페이지](../../installation/using/capability-matrix.md)를 참조하십시오. 호스팅 및 하이브리드 모델의 설치 및 구성 단계는 이 [섹션](../../installation/using/hosting-models.md)에 나와 있습니다.

첫 번째 매개 변수는 **shared** 노드 내에 있습니다. 인스턴스와 관련이 있습니다. 이는 모든 nlserver 명령(nlserver web, nlserver wfserver 등)에서 사용할 수 있습니다. 다른 섹션은 특정 nlserver 하위 명령과 관련되어 있습니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 내부 계정 보안 영역: 내부 계정에 대해 허가된 영역입니다.<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> 'lan'<br /> </td> 
  </tr> 
 </tbody> 
...
다음은 **dataStore** 노드의 다른 매개 변수입니다. 여기에서 서버 데이터 소스가 정의됩니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 디렉토리 내보내기: 내보낸 데이터의 대상 디렉터리 경로입니다.<br /> </td> 
   <td> 문자열<br /> </td> 
  ...
  <tr> 
   ...
   <td> ','로 다운로드할 권한이 있는 파일입니다. 문자열은 유효한 정규 Java 표현식이어야 합니다. <a href="file-res-management.md" target="_blank">업로드 가능한 파일 제한</a>.<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> '.+' <br /> </td> 
  </tr> 
  <tr> 
   <td> useVault<br /> </td> 
   <td> 저장소에 암호 저장: Hashicorp 자격 증명 모음 사용<br /> </td> 
   <td> 부울<br /> </td> 
   <td> false<br /> </td> 
  </tr> 
  <tr> 
   <td> vaultSecretPath<br /> </td> 
   <td> Vault<br />의 비밀 경로 </td> 
   <td> 문자열<br /> </td> 
   <td> '/v1/secret/campaign/'<br /> </td> 
  </tr> 
  <tr> 
   <td> vaultTokenPath<br /> </td> 
   <td> 저장소 토큰이 포함된 파일의 로컬 경로입니다. $(HOME)은 이 경로에서 사용할 수 있지만 다른 env 변수는 사용할 수 없습니다.<br /> </td> 
   <td> 문자열<br /> </td> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 시간대가 있는 날짜 필드: <a href="../../installation/using/time-zone-management.md" target="_blank">시간대 관리</a>를 참조하십시오.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> </td> 
  </tr> 
 </tbody> 
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 연결 유효성 검사 사이의 지연입니다.<br /> </td> 
   <td> Short<br /> </td> 
  </tr> 
  <tr> 
   <td> freeCnx<br /> </td> 
   <td> 풀에 남아 있는 사용 가능한 연결 수입니다.<br /> </td> 
   <td> Short<br /> </td> 
  </tr> 
  <tr> 
   <td> maxCnx<br /> </td> 
   <td> 새 연결을 거부하기 전에 허용되는 최대 연결 수입니다. 이 <a href="https://helpx.adobe.com/campaign/kb/how-to-increase-the-maximum-number-of-database-connections-from-.html">기술 정보</a>.<br /> </td> 
   <td> Short<br /> </td> 
  </tr> 
  <tr> 
...
다음은 **dataStore > virtualDir** 노드의 다른 매개 변수입니다. 가상 디렉터리를 실제 디렉터리에 매핑하는 구성입니다.
...
다음은 **dataStore > preprocessCommand** 노드의 다른 매개 변수입니다. 이는 &#39;파일 로드&#39; 워크플로우 활동을 사전 처리하기 위한 승인된 명령입니다.
...
>[!NOTE]
>
>**nameServer**에 참고: 기본적으로 은 네트워크를 사용합니다
>Windows에서 선언된 첫 번째 네트워크 인터페이스의 매개 변수
>UNIX에서 정의되지 않았습니다. 도메인 이름 서버(DNS)를 정의합니다
>MTA가 을 사용하여
>도메인.
>
>이 값이 정의되지 않으면 MTA는 호스트 네트워크 구성에서 이 정보를 검색합니다. 여러 DNS가 가능한 경우 다른 DNS 주소는 쉼표로 구분해야 합니다(예: 212.155.207.1,212.155.207.2). 게재 서버에 여러 개의 네트워크 인터페이스가 있는 경우 MTA에서 사용하는 DNS 목록이 첫 번째 인터페이스입니다. 이 경우 **nameServer** 매개 변수를 지정하여 모호성을 방지하는 것이 좋습니다.
...
자세한 내용은 [허가된 외부 명령 제한](../../installation/using/configuring-campaign-server.md#restricting-authorized-external-commands)을 참조하십시오.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> timeout<br /> </td> 
   <td> 전환 시간 제한: 최대 전환 시간(초)입니다. 이 임계값 이상으로 변환 프로세스가 중지되고 오류가 발생합니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 120<br /> </td> 
  </tr> 
  <tr> 
...
다음은 **ims** 노드의 다른 매개 변수입니다. [IMS](../../integrations/using/about-adobe-id.md)를 사용하여 다른 서비스에 연결하는 Campaign의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 기술 계정 암호 키(AES에서 암호화됨)<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
...
자세한 내용은 [보고 설명서](../../reporting/using/actions-on-reports.md#memory-allocation) 및 이 [기술 정보](https://helpx.adobe.com/campaign/kb/out-of-memory-error-in-js-code-activity-in-workflows.html)를 참조하십시오.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 최대 준비 시간: 배달 작업을 더 이상 준비하지 않을 기간(초).<br /> </td> 
   <td> Long<br /> </td> 
   <td> 3600<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> OpenOffice 서버가 수행할 수 있는 최대 전환 수입니다. 이 수 이상으로 서버가 다시 시작됩니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 1000<br /> </td> 
  </tr> 
  <tr> 
...
**proxyConfig > HTTP Proxy/Secure 프록시** 노드에서 다음 매개 변수를 구성합니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> action<br /> </td> 
   <td> URL이 인증된 목록(열거형)에 없는 경우 기본 작업입니다. 가능한 값은 'ignore'(경고 메시지 없이 승인, 보호를 사용하지 않도록 설정해야 함), 'warn'(경고 메시지 승인 및 발행) 및 'deny'(URL에 대한 액세스 금지)입니다.<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> 거부<br /> </td> 
  </tr> 
  <tr> 
...
각 URL에 대해 다음 매개 변수와 함께 **url** 노드를 추가합니다.
...
레코드가 **dnsSuffix**&#x200B;을 만족하지만 **urlRegEx**&#x200B;은(는) 만족하지 않으면 다음 레코드가 검사됩니다.
...
자세한 내용은 [전자 메일 보관 활성화(온-프레미스)](../../installation/using/email-archiving.md#activating-email-archiving--on-premise-)를 참조하십시오.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 보낸 메시지(열거형)의 보관 전략입니다. 가능한 값은 '0'(보관 안 함) 및 '1'(보낸 메시지를 SMTP 서버로 보관 전송)입니다.<br /> </td> 
   <td> Byte<br /> </td> 
   <td> 0<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 보관(열거형) 중에 사용된 압축 형식입니다. 가능한 값은 '0'(압축 없음) 및 '1'(zip 포맷을 사용하여 전송된 메시지 압축)입니다.<br /> </td> 
   <td> Byte<br /> </td> 
   <td> 1<br /> </td> 
  </tr> 
  <tr> 
   <td> expirationDelay<br /> </td> 
   <td> 처리되지 않은 전자 메일의 자동 보관 전 지연: 처리되지 않은 전자 메일이 보관되기 전 일 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 2<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 메모리 사용 경고: 지정된 프로세스에 사용된 RAM 양(Mb 단위)에 대한 경고.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 1600<br /> </td> 
  </tr> 
  <tr> 
...
다음은 Mail **노드의 다른 매개 변수입니다.** 인바운드 전자 메일 관리 모듈의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 맥스 호출 데이터의 공유 메모리에 저장된 문자 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 0<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 통계용으로 저장할 Proposition 바로 뒤에 정렬된 최대 적격의 오퍼 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 0<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 맥스 개인 식별을 위해 공유 메모리에 저장된 문자 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 16<br /> </td> 
  </tr> 
 </tbody> 
...
다음은 **mta** 노드의 다른 매개 변수입니다. 게재 에이전트의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> mta 프로세스에서 사용할 수 있는 최대 메모리 크기(MB)입니다. 이 제한보다 높은 경우 사용하는 메모리가 시스템에 해제되도록 프로세스가 다시 시작됩니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 1024<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 고려할 연결 임계값입니다. errorPeriodSec에 의해 지정된 기간의 총 연결 수가 임계값보다 엄격하게 낮은 경우 지정된 경로에 대해 오류 통계가 생성되지 않습니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 100<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 손실된 메시지 다시 시도: 하위 프로세스가 중지된 경우 게재 일부가 다시 시도됩니다.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> true<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 서명 메커니즘을 활성화합니다. 따라서 전자 메일 링크 추적에 대한 보안이 개선됩니다.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> true<br /> </td> 
  </tr>
  <tr> 
   ...
   <td> 게재 통계 서버의 주소입니다. 
    &lt;dns 또는 ip&gt; 
     ...
     &lt;포트&gt; 
       <code>]</code>. 자세한 내용은 
      <a href="../../installation/using/email-deliverability.md#coordinates-of-the-statistics-server" target="_blank">통계 서버의 좌표입니다</a>. 
      <br /> 
     </td> 
   <td> 문자열<br /> </td> 
  ...
  <tr> 
   ...
   <td> "true"로 설정하면 인스턴스가 <a href="../../delivery/using/sending-with-enhanced-mta.md" target="_blank">Enhanced MTA</a>.<br />를 사용하고 있습니다 </td> 
   <td> 부울<br /> </td> 
   <td> <br /> </td>b 
  </tr>
  <tr> 
  ...
  <tr> 
   <td> xMail<br /> </td> 
   <td> X-메일 필드: SMTP 메일 헤더의 'X-Mail' 필드 값입니다.<br /> </td> 
   <td> 문자열<br /> </td> 
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 재생 후: 캐시에서 파일을 자동으로 삭제하여 저장소를 되찾는 기간(초)입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 244800<br /> </td> 
  </tr> 
  <tr> 
...
**mta > relay** 노드에서 다음 매개 변수를 구성합니다. 메시지 배달에 대한 메일 서버의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 최대 자식 서버 수입니다. 실행 중인 최대 서버 수를 나타냅니다. 서버 메모리 리소스와 최적의 호환으로 이 수를 제한하는 것이 좋습니다. 이는 배달 중에 확인할 수 있습니다. 사용된 메모리는 사용 가능한 실제 메모리의 3분의 1을 초과할 수 없습니다. 그렇지 않으면 스왑 기능이 사용됩니다. <a href="../../installation/using/configuring-campaign-server.md#mta-child-processes" target="_blank">MTA 하위 프로세스</a>를 참조하십시오.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 2<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 시작 시 하위 서버 수입니다. 하위 서버 수는 동적으로 모니터링됩니다. MTA가 시작되면 이 값이 나타내는 수만큼 하위 서버가 만들어집니다. 일반적으로 호스트 리소스를 저장하기 위해 1초보다 빠르게 하위 서버를 시작할 수 없습니다. 그러나 MTA가 시작되면 자식 서버를 가능한 한 빨리 사용할 수 있도록 이 제한이 무시됩니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 0<br /> </td> 
  </tr> 
 </tbody> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 각 하위 서버에서 시작한 FCM에 대한 최대 병렬 Http 요청입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 8<br /> </td> 
  </tr> 
  <tr> 
   <td> maxMsgPerChild<br /> </td> 
   <td> 하위 서버당 최대 메시지 수입니다. 각 MTA 하위는 이 수의 메시지를 처리하여 종료합니다. MTA에서 메모리 또는 리소스 누수가 해가 되지 않도록(일반적으로 수천 개) 숫자를 지정하는 것이 중요합니다. MTA 코드에 알려진 메모리 누수가 없는 경우에도 포함된 JavaScript 및 XSL 엔진은 완전히 신뢰할 수 없습니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 5000000<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 연결을 재시도하기 전 초기 지연. 이 지연은 연결이 실패할 때마다 두 배가 됩니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 4<br /> </td> 
  </tr> 
  <tr> 
   <td> maxSessionsPerChild<br /> </td> 
   <td> 하위 서버별 최대 SMTP 세션 수입니다. 메시지를 배달하려면 MTA가 수신자 MTA와 SMTP 연결을 초기화합니다. 주어진 자식 서버에 대한 동시 및 활성 SMTP 세션의 최대 수는 이 값으로 제한됩니다. 이 값을 maxSpareServers에 곱하면 지정된 자식 서버에서 동시에 처리할 수 있는 최대 메시지 수를 얻을 수 있습니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 1000<br /> </td> 
  </tr> 
 </tbody> 
...
**mta > child > smtp > IPAffness** 노드에서 다음 매개 변수를 구성합니다. 최적화된 발신 SMTP 트래픽을 위해 IP 주소가 있는 관심 영역 관리를 구성합니다.

자세한 내용은 [IP 주소 목록을 참조하여](../../installation/using/email-deliverability.md#list-of-ip-addresses-to-use) 및 [관심사가 있는 아웃바운드 SMTP 트래픽 관리](../../installation/using/configuring-campaign-server.md#managing-outbound-smtp-traffic-with-affinities)를 참조하십시오.
...
자세한 내용은 [IP 주소 목록을 참조하여](../../installation/using/email-deliverability.md#list-of-ip-addresses-to-use) 을 사용하십시오.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> publicId<br /> </td> 
   <td> 연결된 공개 주소 ID입니다. 통계 서버의 키로 사용됩니다. 숫자여야 합니다. 이 <a href="../../installation/using/email-deliverability.md#managing-ip-addresses">section</a>.<br /> </td> 
   <td> Long<br /> </td> 
  </tr> 
  <tr> 
   <td> 가중치<br /> </td> 
   <td> 다른 IP에 상대적인 이 IP의 사용 빈도를 지정합니다(가중치가 클수록 주파수가 높아짐).<br /> </td> 
   <td> Long<br /> </td> 
  </tr> 
  <tr> 
...
다음은 **nmac** 노드의 다른 매개 변수입니다. 푸시 알림 게재에 대한 구성입니다.
...
다음은 **nmac > relay** 노드의 다른 매개 변수입니다. 이렇게 하면 메시지 전달에 대한 릴레이 사용(ios http2 커넥터)을 구성합니다.
...
다음은 파이프라인된 **처리된** 노드의 다른 매개 변수입니다. 파이프라인 서비스에 대한 이벤트 처리 모듈의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 토큰을 가져오는 개인 키(XtkKey 옵션을 사용하여 AES에서 암호화됨)<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 상태 서버 포트: 프로세스의 상태를 쿼리할 수 있는 HTTP 서버 포트입니다. 0인 경우 비활성<br /> </td> 
   <td> Long<br /> </td> 
   <td> 7781<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 포인터가 저장되기 전 지연: 포인터가 이 기간 동안 적어도 한 번 데이터베이스에 저장됩니다(활동이 낮은 경우 유용함).<br /> </td> 
   <td> Long<br /> </td> 
   <td> 5<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 이 기간 후 포기: 이 기간 후에도 처리가 계속 실패하면 이벤트를 중단합니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 300<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> allowHTTP<br /> </td> 
   <td> 운영자 로그온에 HTTP를 사용하도록 승인합니다.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> false<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> SMPP 커넥터에 의해 유지된 파일 작업 최대 일 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 60<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> Netsize를 사용하여 연결을 설정할 때 시간 제한(초)입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 30<br /> </td> 
  </tr> 
 </tbody> 
...
다음은 **stat** 노드의 다른 매개 변수입니다. 다음은 MTA 통계 모듈의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> port<br /> </td> 
   <td> 서버 수신 대기 포트입니다. 이 <a href="../../installation/using/email-deliverability.md#definition-of-the-server-port">section</a>.<br /> </td> 
   <td> Short<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
...
다음은 **추적** 노드의 다른 매개 변수입니다. 추적 서버의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 이전 빌드에서 생성된 잘못된 형식의 URL을 사용하지 않도록 설정합니다.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> false<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 개구부 중복 제거: Outlook과 같은 메일 판독기에서 메일 미리 보기의 효과를 제한하려면 열려 있는 추적 로그를 중복하여 제거합니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 1<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> 최대 X% 추적 무시: 아직 고려하지 않은 분개의 비율이 이 값에 도달하지 않는 한 추적 지표를 업데이트하지 마십시오.<br /> </td> 
   <td> Byte<br /> </td> 
   <td> 1<br /> </td> 
  </tr> 
  <tr> 
   <td> trackingIgnorePeriod<br /> </td> 
   <td> 추적 표시기 업데이트: 추적 표시기를 다시 계산하기 전의 최대 기간입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 86400<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 최대 쓰기 다시 시도: 로그 파일에 쓰기 오류가 발생할 경우 만들 수 있는 최대 파일 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 5<br /> </td> 
  </tr> 
  <tr> 
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> SubmitDelivery 호출에 대한 큐 크기: 큐에 대기 가능한 최대 SubmitDelivery SOAP 호출 수입니다.<br /> </td> 
   <td> Long<br /> </td> 
   <td> 50<br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td> maxProcessMemoryWarningMb<br /> </td> 
   <td> 메모리 사용 경고: 지정된 프로세스에 의해 사용된 RAM 양(Mb 단위)에 대한 경고<br /> </td> 
   <td> Long<br /> </td> 
   <td> 1600<br /> </td> 
  </tr> 
  <tr> 
   <td> notifRelay<br /> </td> 
   <td> 알림 릴레이: HostName:알림 릴레이를 사용하도록 설정하는 포트입니다.<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
...
**웹 > jsp > classpath** 노드에는 JVM을 시작할 때 사용할 모든 클래스 경로 목록이 포함되어 있습니다. 다음은 기본 구성입니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   ...
   <td> 각 쿼리 뒤에 JavaScript 컨텍스트의 가비지 수집기를 사용하도록 설정합니다.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> true<br /> </td> 
  </tr> 
  <tr> 
...
**웹 > jsp > classpath** 노드에는 JVM을 시작할 때 사용할 모든 클래스 경로 목록이 포함되어 있습니다.

### 릴레이 {#relay-2}

다음은 **웹 > relay** 노드의 다른 매개 변수입니다. 두 영역 간의 HTTP 요청에 대한 릴레이의 구성입니다.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> 금지된 문자(도메인): URI의 'authority' 섹션에 사용할 수 없는 문자 목록입니다.<br /> </td> 
   <td> 문자열<br /> </td> 
   <td> '.?#@/:' <br /> </td> 
  </tr> 
  <tr> 
...
다음 매개 변수로 각 URL에 대해 **웹 > 릴레이 > url** 노드를 추가합니다(삽입 순서는 우선순위를 정의합니다).

자세한 내용은 [동적 페이지 보안 및 relaes](../../installation/using/configuring-campaign-server.md#dynamic-page-security-and-relays) 및 [section](../../installation/using/deploying-an-instance.md#synchronizing-public-resources)을 참조하십시오.
...
<table> 
 ...
 <tbody> 
  ...
  <tr> 
   <td> 거부<br /> </td> 
   <td> 이러한 URL에 대한 액세스를 거부합니다(HTTP 403 오류 반환)<br /> </td> 
   <td> 부울<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   <td> relayHost<br /> </td> 
   <td> 원래 호스트 추가: 릴레이할 때 원본 요청의 HTTP 'Host' 헤더를 사용합니다.<br /> </td> 
   <td> 부울<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
...
릴레이에 전달된 응답에 추가할 각 HTTP 헤더에 대해 **웹 > relay > responseHeader** 노드를 추가합니다.
...
다음은 **웹 > redirection** 노드의 다른 매개 변수입니다. 이것은 리디렉션 모듈의 구성입니다.
...
<table> 
 ...
 <tbody> 
  <tr> 
   <td> IMSOrgId<br /> </td> 
   <td> Identity Management 시스템(IMS) 조직 식별자: Adobe Experience Cloud 내의 고유한 조직 식별자. 특히 VisitorID 서비스 및 IMS SSO에 사용됩니다. <br /> </td> 
   <td> 문자열<br /> </td> 
   <td> <br /> </td> 
  </tr> 
  <tr> 
...
자세한 내용은 [Configuring SpamAssassin](../../installation/using/configuring-spamassassin.md)을 참조하십시오.
...
자세한 내용은 [고가용성 워크플로우 및 관심도](../../installation/using/configuring-campaign-server.md#high-availability-workflows-and-affinities)를 참조하십시오.
