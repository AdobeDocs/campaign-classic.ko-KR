...
* 파일을 **/usr/local/neolane**&#x200B;에 복사합니다.
* Adobe Campaign Linux 계정(및 관련 그룹)을 만듭니다. 이 계정은 **/usr/local/neolane**&#x200B;을 홈 디렉터리로 사용하여 만듭니다
* 시작 시 사용할 자동 스크립트 **/etc/init.d/nlserver6**&#x200B;를 만들거나 시스템 단위를 만듭니다(20.1부터 시작).

>[!NOTE]
>
>명령을 실행하기 전에 **neolane** 시스템 사용자가 만들어지지 않았어야 합니다. **neolane** 사용자는 설치 중에 자동으로 만들어집니다.
>
>**neolane** 사용자에 연결된 **home** 디렉토리는 **[!UICONTROL /usr/local/neolane]**&#x200B;에서도 자동으로 생성됩니다. **[!UICONTROL /usr/local]** 디스크(몇 GB)에 충분한 공간이 있는지 확인하십시오.
...
1. 먼저 Adobe Campaign 패키지를 가져와야 합니다.
   ...
   * **nlserver6-v7-XXXX-0.x86_64.** rpmfor v7의 경우
...
1. 설치하려면 **root**&#x200B;로 연결하고 다음 명령을 실행합니다(여기서 **XXXX**&#x200B;는 Adobe Campaign 빌드 번호).
...
다음 보고서를 실행하는 데 필요한 &#39;bc&#39; 명령(자세한 내용은 [이 섹션](../../production/using/monitoring-processes.md#automatic-monitoring-via-adobe-campaign-scripts) 참조)은 모든 Linux 배포판에서 기본적으로 사용할 수 없습니다. 명령을 사용할 수 있는지 확인하려면 &#39;which bc&#39; 명령을 실행합니다. 없는 경우 설치해야 합니다.

CentOS를 사용하는 경우 bc.x86_64 패키지를 설치해야 합니다. 를 **root**&#x200B;로 연결하고 다음 명령을 실행합니다.
...
* LibreOffice의 경우:
   ...
   * Debian
      ...
      OOO_INSTALL_DIR, OOO_BASIS_INSTALL_DIR, OOO_URE_INSTALL_DIR에 대한 기본값이 제공됩니다. LibreOffice 설치 레이아웃이 다른 경우 **customer.sh**&#x200B;에서 재정의할 수 있습니다.
...
문제가 발생하면 [Oracle 설치 설명서](https://docs.oracle.com/)에 나열된 패키지가 올바르게 설치되었는지 확인하십시오.
...
이러한 명령을 사용하여 **config-default.xml** 및 **serverConf.xml** 구성 파일을 만들 수 있습니다. **serverConf.xml**&#x200B;에 사용 가능한 모든 매개 변수가 이 [section](../../installation/using/the-server-configuration-file.md)에 나열되어 있습니다.
