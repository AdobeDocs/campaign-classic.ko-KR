...
이러한 각 버전에 대해 Adobe Campaign은 다음과 같은 하나의 패키지를 제공합니다. **nlserver**. 이 패키지에는 지정된 버전에 대한 바이너리 및 구성 파일이 들어 있습니다.

설치 명령을 사용하면 다음 작업을 수행할 수 있습니다.

* 파일에 복사 **/usr/local/neolane**
* 로 만든 Adobe Campaign Linux 계정(및 관련 그룹)을 만듭니다 **/usr/local/neolane** 홈 디렉토리
* 자동 스크립트 만들기 **/etc/init.d/nlserver6** 시작 시 사용하거나 시스템 단위를 만듭니다(20.1 시작).

>[!NOTE]
>
>다음 **네올란** 명령을 실행하기 전에 시스템 사용자가 만들어지지 않아야 합니다. 다음 **네올란** 설치 중에 사용자가 자동으로 생성됩니다.
>
>다음 **홈** 에 연결된 디렉토리 **네올란** 사용자도 자동으로 **[!UICONTROL /usr/local/neolane]**. 에 충분한 공간이 있는지 확인하십시오. **[!UICONTROL /usr/local]** 디스크(몇 GB).

를 실행할 수 있습니다 **ping`hostname`** 명령을 사용하여 서버가 자체 서버에 도달할 수 있는지 확인합니다.

## RPM 패키지 기반 배포 {#distribution-based-on-rpm--packages}

RPM(RHEL, CentOS 및 SUSE) 운영 체제에 Adobe Campaign을 설치하려면 다음 단계를 수행합니다.

1. 먼저 Adobe Campaign 패키지를 가져와야 합니다.

   파일의 이름은 다음과 같습니다. 여기서 **XXXX** 는 Adobe Campaign 빌드 번호입니다.
...
1. 설치하려면 다음으로 연결하십시오 **루트** 다음 명령을 실행합니다(여기서 **XXXX** 는 Adobe Campaign 빌드 번호입니다.
...
netreport를 실행하는 데 필요한 &#39;bc&#39; 명령( [이 섹션](../../production/using/monitoring-processes.md#automatic-monitoring-via-adobe-campaign-scripts) 자세한 내용) 모든 Linux 배포판에서 기본적으로 사용할 수 없습니다. 명령을 사용할 수 있는지 확인하려면 &#39;which bc&#39; 명령을 실행합니다. 없는 경우 설치해야 합니다.

CentOS를 사용하는 경우 bc.x86_64 패키지를 설치해야 합니다. 다음으로 연결 **루트** 다음 명령을 실행합니다.
...
일부 매개 변수는 **customer.sh** 파일

처음 설치를 수행하는 경우 **customer.sh** 파일이 서버에 아직 없을 수 있습니다. 만들고 실행 권한이 있는지 확인합니다. 그렇지 않은 경우 다음 명령을 입력합니다.
...
>[!CAUTION]
>
>이 변경 사항은 파일 시스템(워크플로우 또는 JavaScript 스크립트를 통해 로드되는 파일)과의 상호 작용 및 파일 인코딩에 영향을 줍니다. 따라서 기본 환경을 사용하는 것이 좋습니다.

하지만 **일본어 인스턴스**&#x200B;를 채울 때는 UTF-8 환경을 사용해야 합니다.

UTF-8 환경을 사용하려면 다음 명령을 사용하십시오.
...
시스템 메시지를 올바르게 읽으려면 콘솔이 언어에 해당하는 코드 페이지(ISO-8859-1 또는 -15(프랑스어)에 있어야 합니다.)

### 환경 변수 {#environment-variables}

다음 환경 변수를 올바르게 정의해야 합니다.

특정 조합을 사용하려면 Adobe Campaign 실행에 사용되는 환경을 변경해야 합니다. 특정 파일(`/usr/local/neolane/nl6/customer.sh`)을 만들고 편집하여 Adobe Campaign 환경에 따른 수정 사항을 추가할 수 있습니다.

필요한 경우 를 편집합니다 **customer.sh** 파일을 **vi customer.sh** 구성을 조정하고 조정하거나 누락된 줄을 추가합니다.
...
* oracle 클라이언트의 경우:
   ...
   TNS_ADMIN 변수의 콘텐츠는 **tnsnames.ora** 파일.
...
* Java 개발 키트(JDK)의 경우:
   ...
   기본적으로 Adobe Campaign 환경의 구성 스크립트(`~/nl6/env.sh`)은 JDK 설치 디렉토리를 검색합니다. 이 동작은 100% 안정적이지 않으므로 사용해야 하는 JDK를 지정해야 합니다. 이렇게 하려면 **JDK_HOME** 다음 명령을 사용하는 환경 변수:
...
변경 사항을 적용하려면 Adobe Campaign 서비스를 다시 시작해야 합니다.
...
문제가 발생하는 경우 [Oracle 설치 설명서](https://docs.oracle.com/) 가 올바르게 설치됩니다.
...
이러한 명령을 사용하여 다음을 만들 수 있습니다 **config-default.xml** 및 **serverConf.xml** 구성 파일. 에서 사용할 수 있는 모든 매개 변수 **serverConf.xml** 여기에 나열되어 있습니다. [섹션](../../installation/using/the-server-configuration-file.md).
