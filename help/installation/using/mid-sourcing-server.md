...
중간 소싱 서버를 설치하는 작업은 일반적인 방법으로 서버를 설치하는 것과 동일한 프로세스를 따릅니다(표준 구성 참조). 자체 데이터베이스가 있는 독립 인스턴스로서 배달을 실행하는 데 사용할 수 있습니다. 간단히 말해, 원격 인스턴스가 중간 소싱 모드에서 전달을 실행할 수 있도록 추가 구성이 포함되어 있습니다.

>[!CAUTION]
>
>중간 소싱 서버가 설정되고 [동기화 워크플로우](../../workflow/using/about-technical-workflows.md)가 처음으로 실행되면 중간 소싱 외부 계정의 내부 이름을 업데이트하지 않아야 합니다.
