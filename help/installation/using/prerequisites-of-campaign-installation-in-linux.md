...
* LibreOffice, Debian용 LibreOffice [설치](#installing-libreoffice-for-debian) 및 [LibreOffice for CentOS](#installing-libreoffice-for-centos),
* 글꼴은 [MTA 통계](#fonts-for-mta-statistics) 및 [글꼴은 일본어 인스턴스를 참조하십시오](#fonts-for-japanese-instances).
...
Linux에 Adobe Campaign을 설치하려면 필요한 라이브러리가 있어야 합니다.

* 라이브러리 C는 TLS(스레드 로컬 저장소) 모드를 지원할 수 있어야 합니다. 이 모드는 Xen 지원이 비활성화된 일부 커널을 제외한 대부분의 경우 활성화됩니다.

   이것을 확인하려면 **uname -a | grep xen** 명령(예:

   명령이 아무 것도(빈 줄)도 반환하지 않으면 구성이 올바르다는 것을 의미합니다.

* OpenSSL의 **버전 0.9.8** 또는 **1.0** 이 있어야 합니다.

   RHEL 7 배포의 경우 OpenSSL 버전 1.0이 필요합니다.

* Adobe Campaign을 사용하려면 **도서관** 을 설치해야 합니다.
   ...
   * 디베이안 9:libicu57
   ...
   Adobe Campaign을 사용하려면 라이브-아레스 라이브러리가 설치되어 있어야 합니다. RHEL/CentOS에서 다음 명령을 실행합니다.
...
또한 **/etc/sysconfig/httpd** 파일에서 Adobe Campaign 환경 구성 스크립트를 참조하는 다음 줄이 추가되었습니다.

```
. ~neolane/nl6/env.sh
```

RHEL 및 CentOS에서는 SELinux가 활성화되어 있을 때 데이터베이스의 클라이언트 레이어와 호환성 문제가 발생했습니다. Adobe Campaign이 제대로 작동할 수 있는지 확인하려면 SELinux를 비활성화하는 것이 좋습니다.

**다음 프로세스를 적용합니다.**

* 파일 편집 **/etc/seleclinux/config**
...
보고서를 PDF 형식으로 내보내려면 일본어 인스턴스에 특정 문자 글꼴이 필요합니다.
...
1. 다음 글꼴을 설치합니다(선택 사항이지만 일본어 인스턴스의 경우 적극 권장).
...
1. 다음 글꼴을 설치합니다(선택 사항이지만 일본어 인스턴스 시 적극 권장).
...
Adobe Campaign은 버전 7.2부터 모든 버전의 PostgreSQL 클라이언트 라이브러리를 지원합니다.(**libpq.so.5**, **libpq.so.4****,** libpq.so.3.2 **및** libpq so.3.1).

또한 PostgreSQL을 Adobe Campaign과 함께 사용하려면 해당 **pgcrypto 라이브러리를 설치해야** 합니다.
...
>[!NOTE]
>
>Oracle 클라이언트를 이미 설치했지만 글로벌 환경(예:/etc/profile)이 제대로 구성되지 않았습니다. nl6/customer.sh **** 스크립트에 누락된 정보를 추가할 수 있습니다. 자세한 내용은 [환경 변수를 참조하십시오](../../installation/using/installing-packages-with-linux.md#environment-variables).

**문제 해결 및 우수 사례**

Oracle 클라이언트 또는 서버 업데이트, 버전 변경 또는 인스턴스의 첫 번째 설치 후에 문제가 나타날 수 있습니다.

클라이언트 콘솔에서 로그, 워크플로우 마지막 처리, 다음 처리 등에 예기치 않은 시간 경과(1시간 이상)가 있는 것을 보면 Oracle 클라이언트의 라이브러리와 Oracle Server 사이에 문제가 있을 수 있습니다. 그런 문제를 피하려면

1. 전체 클라이언트를 **사용해야 합니다**.

   Oracle Instant Client 버전을 사용할 때 다양한 문제가 발견되었습니다. 또한 인스턴스 클라이언트에서 표준 시간대 파일을 변경하는 것은 불가능합니다.

1. 클라이언트 버전 **과** 데이터베이스 서버 버전 **이** 동일한지 **확인하십시오**.

   클라이언트 및 서버 버전을 정렬하기 위해 Oracle의 호환성 매트릭스와 권장 사항에도 불구하고 버전을 혼합하면 문제가 발생하는 것으로 알려져 있습니다.

   ORACLE_HOME 값을 확인하여 예상 클라이언트 버전을 가리키는지 확인합니다(시스템에 여러 버전이 설치된 경우).

1. 클라이언트와 서버가 동일한 **표준 시간대 파일을 사용하는지 확인합니다**.

### DB2 {#db2}

지원되는 라이브러리 버전은 **libdb2.so입니다**.

## 구현 단계 {#implementation-steps}

Linux용 Adobe Campaign 설치은 다음 순서로 수행해야 합니다.서버 설치 뒤에 인스턴스 구성이 나옵니다.

설치 프로세스는 이 장에 설명되어 있습니다. 설치 단계는 다음과 같습니다.

* 1단계:응용 프로그램 서버 설치에서 Linux를 [사용하여 패키지 설치를 참조하십시오](../../installation/using/installing-packages-with-linux.md).
* 2단계:웹 서버와 통합(배포한 구성 요소에 따라 선택 사항).
