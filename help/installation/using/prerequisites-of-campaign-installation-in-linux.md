...
* 라이브러리 C는 TLS(Thread Local Storage) 모드를 지원할 수 있어야 합니다. 이 모드는 Xen 지원이 비활성화된 일부 커널을 제외한 대부분의 경우 활성화됩니다.
  ...
  이 항목을 확인하려면 **uname -a를 사용합니다. | grep xen** 명령의 예입니다.
...
64비트 Debian의 라이브러리 버전을 검색합니다(예: **libclntsh.so**, **libclntsh.so.19.1**, **libclntsh.so.18.1**, **libclntsh.so.12.1**, **libclntsh.so.11.1** 또는 **libclntsh.so.10.1**).

Oracle Technology Network에서 Linux RPM 패키지를 가져올 수 있습니다.

>[!NOTE]
>
>Oracle 클라이언트를 이미 설치했지만 전역 환경(예: /etc/profile)이 제대로 구성되지 않은 경우 누락된 정보를 **nl6/customer.sh** 스크립트에 추가할 수 있습니다. 자세한 내용은 [환경 변수](../../installation/using/installing-packages-with-linux.md#environment-variables)를 참조하십시오.

**문제 해결 및 모범 사례**

문제는 Oracle 클라이언트 또는 서버 업데이트, 버전 변경 또는 인스턴스의 첫 번째 설치 후에 나타날 수 있습니다.

로그, 워크플로우 마지막 처리, 다음 처리 등에 예기치 않은 시간 지연(1시간 이상)이 있다는 것을 클라이언트 콘솔에서 알게 되면 Oracle 클라이언트의 라이브러리와 Oracle 서버 간에 문제가 발생할 수 있습니다. 이러한 문제를 방지하려면

1. **전체 클라이언트**&#x200B;를 사용해야 합니다.

   Oracle Instant Client 버전을 사용할 때 다양한 문제가 발견되었습니다. 또한 인스턴트 클라이언트에서 시간대 파일을 변경할 수 없습니다.

1. **클라이언트 버전**&#x200B;과(와) **데이터베이스 서버 버전**&#x200B;이(가) **동일한지**&#x200B;확인하십시오.

   Oracle의 호환성 매트릭스와 권장 사항에도 불구하고 버전을 혼용하면 클라이언트 및 서버 버전에 문제가 발생하는 것으로 알려져 있습니다.

   또한 ORACLE_HOME 값을 확인하여 예상 클라이언트 버전(컴퓨터에 여러 버전이 설치된 경우)을 가리켜야 합니다.
