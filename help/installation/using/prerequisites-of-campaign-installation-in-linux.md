...
# Linux{#prerequisites-of-campaign-installation-in-linux}에 Campaign 설치 전제 조건
...
클라이언트 콘솔에서 로그, 워크플로 마지막 처리, 다음 처리 등에 예기치 않은 시간(1시간 이상)이 지연되는 것을 보면 Oracle 클라이언트의 라이브러리와 Oracle 서버 간에 문제가 있을 수 있습니다. 이러한 문제를 피하려면
