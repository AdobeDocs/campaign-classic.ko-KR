...
Adobe Campaign을 설치하는 데 필요한 기술 및 소프트웨어 구성은 [호환성 매트릭스](../../rn/using/compatibility-matrix.md)에 자세히 설명되어 있습니다.

미리 알림으로 다음 구성 요소를 설치하고 올바르게 구성해야 합니다.

* Apache는 [호환성 매트릭스](../../rn/using/compatibility-matrix.md),
...
* LibreOffice의 경우 [Debian](#installing-libreoffice-for-debian) 및 [LibreOffice for CentOS](#installing-libreoffice-for-centos)설치
...
* 라이브러리 C는 TLS(스레드 로컬 저장소) 모드를 지원할 수 있어야 합니다. 이 모드는 Xen 지원이 비활성화된 일부 커널을 제외하고 대부분의 경우 활성화됩니다.
   ...
   이를 확인하려면 **이름 바꾸기 -a를 사용할 수 있습니다 | grep xen** 명령(예:
...
1. **클라이언트 버전** 및 **데이터베이스 서버 버전**&#x200B;이 **same**&#x200B;인지 확인합니다.
