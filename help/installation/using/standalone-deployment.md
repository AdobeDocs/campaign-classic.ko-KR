...
이 유형의 구성은 수신자가 10만 명 미만이고 예를 들어 다음 소프트웨어 레이어를 사용하여 관리할 때 실행할 수 있습니다.
...
볼륨이 증가하면 이 아키텍처의 변형으로 데이터베이스 서버가 다른 컴퓨터로 이동되어 성능이 향상됩니다.
...
* 완전한 독립 실행형 및 낮은 구성 비용(아래 나열된 오픈 소스 소프트웨어를 사용하는 경우 청구 가능한 라이선스 필요 없음)
* 설치 및 네트워크 구성 간소화
...
* 응용 프로그램 서버는 리디렉션 서버를 호스팅하기 때문에 외부(예: DMZ에 있는 동안)에서 사용할 수 있어야 합니다.
...
* 두 개의 DNS 별칭 생성:
   ...
   * 콘솔 액세스 및 동일한 컴퓨터를 가리키는 두 번째 별칭이 내부 사용자에게 노출되었습니다.

* SMTP(25), DNS(53), HTTP(80), HTTPS(443), SQL(Oracle의 경우 1521, PostgreSQL의 경우 5432) 열도록 구성된 방화벽 포트. 자세한 내용은 [네트워크 구성을 참조하십시오](../../installation/using/network-configuration.md).
...
1. Adobe Campaign 서버가 설치되면 nlserver web -tomcat **** 명령 명령을 사용하여 응용 프로그램 서버(웹)를 시작하고 Tomcat이 포트 8080의 독립 실행형 웹 서버 모드에서 Tomcat을 시작하고 올바르게 시작하는지 확인합니다.
   ...
   * Linux용: [서버의 첫 번째](../../installation/using/installing-packages-with-linux.md#first-start-up-of-the-server)시작,
...
1. config- **demo.xml** 파일(config-default.xml **옆의 이전 단계에서 생성됨)을 편집하고 mta**(mta **, 작업 과정** 서버(워크플로), profbounceWebBounceStat 및 statStatic메일(통계 사용)이 활성화되었는지 확인합니다 **** **** **** (통계 사용). 그런 다음 통계 서버의 주소를 구성합니다.
...
1. URL을 **사용하여 서버 웹** 모듈을 테스트합니다.https://console.campaign.net/nl/jsp/logon.jsp
...
1. Adobe Campaign 클라이언트 콘솔을 시작(이전 다운로드 페이지에서 또는 Windows 설치에 대해 서버에서 직접 실행)하고, 서버 연결 URL을 https://console.campaign.net으로 설정하고 **내부** 로그인을 사용하여 연결합니다.
...
1. 암호 없이 **관리자** 로그인을 사용하여 클라이언트 콘솔에 다시 로그온하고 배포 마법사( **[!UICONTROL Tools > Advanced]** 메뉴)를 시작하여 인스턴스 구성을 마칩니다.

   자세한 내용은 인스턴스 [배포를 참조하십시오](../../installation/using/deploying-an-instance.md).

   설정할 기본 매개 변수는 다음과 같습니다.

   * 이메일 전달:보낸 사람 및 회신 주소 및 바운스 메일에 대한 오류 사서함.
   ...
   * 바운스 관리:바운스 메일을 처리할 매개 변수를 입력합니다(처리되지 않은 바운스 메일 **섹션을 고려하지는** 않음).
   * 액세스 위치:보고서, 웹 양식 및 미러 페이지에 대한 두 개의 URL을 제공합니다.
