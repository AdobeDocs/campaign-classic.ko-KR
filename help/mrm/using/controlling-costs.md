...
1. 예산 정의
   ...
   자세한 내용은 [예산 생성](#creating-a-budget).

1. 원가 계산 방법 정의

   서비스 공급자에 대해 비용 구조가 정의됩니다. 자세한 내용은 [서비스 공급자 및 해당 비용 범주 만들기](../../campaign/using/providers--stocks-and-budgets.md#creating-a-service-provider-and-its-cost-categories).

1. 캠페인 비용 정의(게재/작업)

   게재 및 작업에 따른 비용은 캠페인 템플릿에 대해 개별적으로 또는 전체적으로 입력됩니다. 자세한 내용은 [비용 및 주식 계산](../../campaign/using/marketing-campaign-deliveries.md#calculation-of-costs-and-stocks).

1. 통합

   작업, 게재 및 캠페인의 진행 상태에 따라 비용이 계산되어 해당 예산으로 전달됩니다.

   캠페인 만들기가 충분히 진행되면 캠페인 예산의 진행 상태를 다음으로 변경할 수 있습니다. **[!UICONTROL Specified]**. 그런 다음 프로그램의 계산된 비용이 캠페인에 계산된 비용과 함께 자동으로 입력됩니다. 자세한 내용은 [비용 약정, 계산 및 과금](#cost-commitment--calculation-and-charging).

## 예산 생성 {#creating-a-budget}

예산은 맵에서 을 통해 생성됩니다 **[!UICONTROL Campaign management > Budgets]** 노드 아래에 있어야 합니다. 다음 **[!UICONTROL New]** 도구 모음의 단추를 사용하면 예산을 만들 수 있습니다.

* 새 예산 추가

   을(를) 클릭합니다. **[!UICONTROL New]** 아이콘, 이름을 지정하고 예산을 저장합니다.

* 초기 금액 입력

   관련 필드에 할당된 금액을 지정합니다. 다른 금액은 자동으로 입력됩니다. 자세한 내용은 [금액 계산](#calculating-amounts).
...
* 비용
   ...
   캠페인, 작업 등에 대해 이 예산에 할당된 비용이 드는 비용 범주를 만듭니다. 연결할 수 있습니다. 자세한 내용은 [비용 범주](#expense-categories).
...
>관련 예산을 선택할 수 있습니다.
>
>자세한 내용은 [다른 예산 연결](#linking-a-budget-to-another).
...
>[!NOTE]
>
>비용 범주 및 구조는 [서비스 공급자 및 해당 비용 범주 만들기](../../campaign/using/providers--stocks-and-budgets.md#creating-a-service-provider-and-its-cost-categories).
...
예산 승인을 통해 캠페인에 커밋된 금액에 대해 검사할 수 있습니다. 다른 승인을 설정하기 위해 캠페인에 추가 체크포인트 스타일 작업을 만들 수 있습니다. 자세한 내용은 [작업 유형](../../mrm/using/creating-and-managing-tasks.md#types-of-task).
...
1. 비용 구조를 사용하여 서비스 공급자 및 서비스 템플릿을 만듭니다. **[!UICONTROL Administration > Campaigns]** 노드 아래에 있어야 합니다. 자세한 내용은 [서비스 공급자 및 해당 비용 범주 만들기](../../campaign/using/providers--stocks-and-budgets.md#creating-a-service-provider-and-its-cost-categories).
...
이 캠페인에 앞서 비용 구조를 만든 두 가지 작업을 추가합니다( 참조). [2단계 - 서비스 공급자를 구성하고 비용 구조를 정의합니다](#step-2---configuring-the-service-provider-and-defining-the-cost-structures)). 이렇게 하려면 캠페인 대시보드에서 **[!UICONTROL Add a task]** 버튼을 클릭합니다. 작업 이름을 지정하고 **[!UICONTROL Save]**.
