...
* 마케팅 리소스 만들기 작업을 통해 리소스를 관리하는 경우 작업에서 **[!UICONTROL Finished]** 상자를 선택하는 한 작업이 **[!UICONTROL Publish the marketing resource]**(으)로 설정되면 리소스가 실행됩니다([마케팅 리소스 만들기 작업](../../mrm/using/creating-and-managing-tasks.md#marketing-resource-creation-task) 참조).
