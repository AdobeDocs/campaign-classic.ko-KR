...
* **기존 데이터 센터와 공용 클라우드 간에 패키지 크기에 차이가 있습니까?**
...
* **마이그레이션 후 문제를 해결하기 위한 계획은 무엇입니까?**
  ...
  운영 시스템을 마이그레이션하기 전에 광범위한 테스트를 수행합니다. 그러나 문제가 발생하면 [고객 지원 Adobe](https://experienceleague.adobe.com/?support-solution=Campaign#support)이(가) 주 연락 창구로 남게 됩니다. Adobe은 필요한 경우 고급 지원을 제공하기 위해 전문가 팀을 구성했습니다.
...
* **허용 목록에 추가하다의 IP는 어떻게 처리됩니까? Campaign에서 들어오는 트래픽에 대해 고객이 새 IP 주소를 허용 목록에 추가하다에 추가해야 합니까?**

  Adobe 서버의 IP 주소가 변경됩니다. 따라서 고객은 시스템에 새 IP 주소를 허용 목록에 추가하다에 추가해야 할 수 있습니다.

  [허용 목록에 추가하다의 IP에 대해 자세히 알아보세요](#config).
...
* **마이그레이션이 추적, 리소스 및 웹 응용 프로그램에 대한 새 URL을 생성합니까?**

  아니요. 기존 URL은 그대로 유지됩니다.

* **하위 도메인이 Neolane.net에서 campaign.adobe.com으로 변경됩니까?**

  마이그레이션 후 `neolane.net`과(와) `campaign.adobe.com`이(가) 모두 준비됩니다. 간단하게 하기 위해: neolane.net 을 Public Cloud(AWS)의 새 인스턴스로 리디렉션하므로 고객이 변경할 필요가 없습니다.

* **IP 준비 방법은 무엇입니까?**

  우선 Adobe 전달성 은 플랫폼의 전달성 상태를 평가하고 새 IP로 전환하는 계획을 권장합니다

  마이그레이션 후에는 준비 작업이 필요하지 않습니다. 예외가 있을 수 있으며, 이 경우 [고객 지원 Adobe](https://experienceleague.adobe.com/?support-solution=Campaign#support)이 고객에게 연결됩니다.
...
* **특정 파일 확장명에 액세스할 수 있는 위험이 있습니까?**
  ...
  고객은 글꼴 파일, Outlook 모임 파일을 공용 리소스 폴더에 로드할 수 있도록 허용할 수 있습니다. 이 구성은 현재 `config-<instance>.xml` 파일에서 수행됩니다. 이 파일은 구성 파일과 함께 복사됩니다.

* **새 서버에서 시간대가 변경되고 있습니까? 고객이 현재 시간대를 유지할 수 있습니까?**
...
* **이동해야 하는 데이터 센터별 허용 목록 구성이 있습니까?**
...
* **기존 서버에서 보낸 메시지에 대한 정보. 링크에 항상 액세스할 수 있습니까?**
