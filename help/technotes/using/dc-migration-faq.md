...
* **데이터 손실 위험이 있습니까?**
   ...
   데이터베이스는 기존 데이터 센터에서 덤프되고 AWS(Public Cloud)에서 복원됩니다. 새 데이터 센터에서 다시 시작하면 애플리케이션이 마이그레이션 이전의 정확한 상태에서 다시 시작됩니다. 일부 예약된 작업이 지연되는 경우를 제외하고는 사용자에게 아무런 차이가 없습니다.

* **레거시 데이터 센터와 Public Cloud 간에 패키지 크기에 차이가 있습니까?**

   현재 데이터베이스 크기, 디스크 크기 등을 기준으로 새로운 패키지 정의를 사용하여 AWS(Public Cloud)에서 프로비저닝하고 있습니다. 예를 들어 고객이 기존 데이터 센터에 하나의 애플리케이션 서버를 가지고 있는 경우 패키지 정의를 기반으로 하여 AWS(Public Cloud)에서 두 개의 애플리케이션 서버를 보유할 수 있습니다.
...
* **의 IP는 어떻게 허용 목록에 추가하다 처리됩니까? 고객이 Campaign에서 들어오는 트래픽에 대해 새 IP 주소허용 목록에 추가하다를에 추가해야 합니까?**
   ...
   [](#config) 의 IP에 대한 자세한 내용을 보려면 여기를 클릭하십시오허용 목록에 추가하다.

* **SFTP/FTP 액세스를 위해에 추가된 허용 목록에 추가하다 포트를 어떻게 처리합니까?**

   SFTP 구성(공개 키 +의 IP)허용 목록에 추가하다는 기존 데이터 센터에서 AWS(Public Cloud)로 이동됩니다. 고객에게 필요한 작업이 없습니다.
...
* **마이그레이션이 Genesis 커넥터를 사용하는 Adobe Analytics에 대한 연결에 영향을 줍니까? Adobe Analytics 측의에 캠페인 IP 주소허용 목록에 추가하다를 추가하는 방법은 무엇입니까?**
...
* **API 연결이 마이그레이션의 영향을 받습니까?**
   ...
   [](#config) 에 대한 자세한 내용을 보려면 여기를 클릭하십시오허용 목록에 추가하다.
