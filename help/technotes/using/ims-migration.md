...
Campaign 외부 시스템에서 Campaign 마케팅 인스턴스 또는 실시간 메시지 센터 인스턴스로 API를 호출하는 경우, Adobe에서는 아래에 자세히 설명된 대로 기술 연산자를 Adobe Developer Console을 통해 기술 계정으로 마이그레이션할 것을 강력히 권장합니다.
...
* Campaign Hosted 및 Managed Services 고객
  ...
  다른 경우에는 Adobe 기술 팀이 기존 운영자 그룹 및 명명된 권한을 Admin Console 내의 제품 프로필로 마이그레이션할 수 있도록 Adobe 전환 관리자(Managed Services 사용자용) 또는 Adobe 고객 지원 센터(다른 호스트된 사용자용)에 연결해야 합니다.
...
* Campaign On-premise 및 Hybrid 고객
  ...
  [Adobe 설명서](https://developer.adobe.com/developer-console/docs/guides/projects/){target="_blank"}에서 Adobe Developer Console 콘솔 내에서 제품 프로필에 대해 자세히 알아볼 수 있습니다.
...
이 기술 계정을 사용하여 메시지 센터 인스턴스에 대한 API를 호출하는 경우 메시지 센터와 연결된 마케팅 인스턴스에 대해 messagecenter로 끝나는 Adobe 제품 프로필을 선택해야 합니다.
...
Adobe IMS(Identity Management System) 인증을 사용할 때 WSDL 파일을 생성하려면 postman 호출에 Authorization: Bearer &lt;IMS_Technical_Token_Token>을 추가해야 합니다.
