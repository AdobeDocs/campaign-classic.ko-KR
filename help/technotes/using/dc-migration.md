...
description: 공개 클라우드로 Campaign Classic 마이그레이션에 대해 자세히 알아보기
...
소중한 Adobe Campaign Classic 고객으로서 최고의 경험과 가치를 제공하기 위해 최선을 다하고 있습니다. 수년에 걸쳐 Dell은 클라우드에서 고객을 호스팅할 수 있는 가치와 안정성을 실현했습니다.  [연간 업그레이드 이니셔티브](../../rn/using/rn-overview.md#yearly-upgrade)의 일부로, 더 우수하고 안정적인 서비스를 제공하기 위해 모든 고객을 Adobe Managed Services(AWS의 공용 클라우드)로 이동하고 있습니다.

이 프로그램에는 세 가지 주요 목표가 있습니다.

* 인프라를 안전한 최신 환경(AWS)으로 이동하여 식별된 보안 취약점을 해결합니다.
* 번거로운 확장 프로세스를 제거하고 [향상된 MTA](../../delivery/using//sending-with-enhanced-mta.md)에 대한 액세스를 제공하며 모든 유지 관리 서비스 수준을 향상시킵니다.
...
* **MID/RT** - Adobe Cloud에서 호스팅되는 메시지 실행 서버(일괄 캠페인의 경우 MID, 실시간 단일 메시지의 경우 RT)
...
* **SFTP** - 보안 파일 전송 프로토콜입니다. [자세히 알아보기](../../platform/using/sftp-server-usage.md).


>[!NOTE]
>공용 클라우드로 Campaign Classic v7를 마이그레이션하면 **Adobe Managed Services**&#x200B;을 사용하는 고객에게만 영향을 줍니다.
...
**Adobe Campaign Classic 고객을 위한 강력하고 안정적인 솔루션 가져오기**
...
**연간 업그레이드를 통해 향후 업그레이드를 보다 쉽게 수행할 수 있음**
...
이 클라우드로의 마이그레이션은 Adobe Campaign 인스턴스의 [연간 업그레이드 프로그램&#x200B;](../../rn/using/rn-overview.md#yearly-upgrade)**에 대한**&#x200B;첫 단계입니다. 퍼블릭 클라우드(AWS)가 아닌 데이터 센터에서 호스팅되는 경우 이 마이그레이션은 필수입니다.
...
Adobe은 기존 데이터 센터를 해제할 계획이며, 이 곳에서 실행 중인 Adobe Campaign 인스턴스를 새 참조 데이터 센터인 AWS으로 전송해야 합니다.

현재 위치가 **보안 및 성능 취약점**&#x200B;에 노출될 수 있으므로 이는 중요한 향후 경로입니다.

또한 이 마이그레이션은 이제 Adobe Campaign의 향후 빌드 업그레이드에 대한 **전제 조건**&#x200B;입니다. 기존 데이터 센터에서는 빌드 업그레이드가 더 이상 불가능합니다.

Adobe은 데이터 보안을 강화하고 Adobe Campaign의 미래를 선도하기 위해 최선을 다하고 있습니다. 공동 성공을 위해서는 귀하의 파트너십이 필요합니다!


**전담 고객 지원 담당자, 고객 성공 관리자, 제품 관리자, 엔지니어, TechOps 전문가 및 제품 컨설턴트로 구성된 팀**&#x200B;을 구성하여 원활하고 원활한 경험을 제공합니다. Adobe는 귀하에게 관련 프로젝트 및 연락처 정보를 제공하기 위해 최선을 다하고 있습니다.
...
[공개 클라우드 FAQ](dc-migration-faq.md)(으)로 Campaign 마이그레이션에서 자세히 알아보세요.
...
* CNAME 설정(예외)의 경우 클라이언트가 변경 사항을 구현하도록 요청됩니다. Adobe과의 협력이 필요할 것입니다.
