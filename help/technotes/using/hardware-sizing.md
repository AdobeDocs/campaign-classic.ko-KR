...
소프트웨어 호환성은 [호환성 매트릭스](../../rn/using/compatibility-matrix.md).
...
1. [큰 크기](#scenario-2) - 시스템에 2천만 명의 활성 수신자
1. [Enterprise](#scenario-3) - 5,000만 명의 활성 수신자, 트랜잭션 메시지 포함
...
을(를) 참조하십시오. [시나리오 1: 중간 크기 배포](#scenario-1) 프록시, 기본 설정 센터/구독 처리 및 디스크 공간 사용에 대한 자세한 설명
...
5천만 명의 수신자를 지원하는 배포는 기본적으로 과 동일합니다. [시나리오 2](#scenario-2): Campaign 웹 애플리케이션 트래픽은 Campaign 웹 서버로 라우팅되므로 큰 캠페인 실행 후 웹 트래픽이 버스트되더라도 Campaign 워크플로우 및 클라이언트 콘솔 사용자에게 영향을 주지 않습니다.
