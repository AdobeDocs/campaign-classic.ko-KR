...
Adobe Campaign 서버에서 리소스를 사용할 수 있도록 하려면 편집기 중간 영역에 리소스를 드래그하여 놓아 원하는 리소스를 추가해야 합니다. **[!UICONTROL Upload file to server...]** 링크를 클릭할 수도 있습니다.

![](assets/s_ncs_user_mkg_resource_file.png)

확인 메시지를 통해 업로드를 시작할 수 있습니다.

업로드가 완료되면 리소스가 사용 가능한 리소스 목록에 추가됩니다. Adobe Campaign 운영자가 액세스할 수 있습니다. **[!UICONTROL Preview]** 탭을 통해 파일을 보거나, 복사본을 만들어 수정하거나, **[!UICONTROL Edit]** 탭을 사용하여 서버에서 파일을 업데이트할 수 있습니다.

![](assets/s_ncs_user_mkg_resource_extract.png)

이 리소스를 모니터링, 추적 및 승인하는 운영자 또는 운영자 그룹을 선택하려면 **[!UICONTROL General]** 탭을 클릭하십시오. 검토자 선택은 **[!UICONTROL Advanced parameters]** 링크를 통해 수행됩니다.

* 리소스가 할당된 연산자는 리소스 추적을 담당합니다.
* 승인 운영자는 마케팅 리소스를 승인해야 합니다. 리소스 유효성 검사 프로세스가 시작되면 알림을 받게 됩니다.

   검토자를 선택하지 않으면 리소스 **[!UICONTROL cannot be]**&#x200B;이(가) 승인을 받습니다.

* 필요한 경우 교정을 지정할 수도 있습니다.

리소스에 대한 (지시) 가용성 날짜를 지정할 수 있습니다. 이 날짜 이후에는 **[!UICONTROL Late]** 상태로 표시됩니다.
...
리소스를 잠그려면 리소스 대시보드에서 리소스를 클릭한 후 **[!UICONTROL Lock]** 버튼을 클릭해야 합니다.

![](assets/s_ncs_user_mkg_resource_lock.png)

리소스의 **[!UICONTROL Tracking]** 탭에서 계획된 반품 날짜를 나타낼 수 있습니다.
...
[토론 ](../../campaign/using/discussion-forums.md) 포럼이 Adobe Campaign에서 작동하는 방식을 설명합니다.

## 마케팅 리소스 {#life-cycle-of-a-marketing-resource} 수명 주기

리소스가 만들어지면 Adobe Campaign 연산자가 리소스를 디자인, 교정, 승인 및 게시하도록 지정됩니다. 이러한 캠페인에 대해 기간을 결정할 수 있습니다.

**[!UICONTROL Tracking]** 탭에서는 리소스에 대해 수행된 작업을 모니터링할 수 있습니다.승인, 승인 참조, 관련 주석 또는 게시

**[!UICONTROL History]** 탭에는 이 리소스에 대해 수행된 파일 전송이 표시됩니다.

### 승인 프로세스 {#approval-process}

예상 가용 날짜가 **[!UICONTROL Tracking]** 탭에 지정된 경우 리소스 세부 사항에 표시됩니다. 이 날짜가 되면 리소스 대시보드의 **[!UICONTROL Submit for approval]** 버튼을 사용하여 승인 프로세스를 실행할 수 있습니다. 그런 다음 리소스 상태가 **[!UICONTROL Approval in progress]**&#x200B;으로 변경됩니다.

대시보드의 **[!UICONTROL Approve resource]** 버튼을 통해 리소스를 승인할 수 있습니다.

![](assets/s_ncs_user_task_valid_date.png)

그러면 승인된 운영자는 승인을 수락하거나 거부할 수 있습니다. 이 작업은 다음 중 한 방법으로 수행할 수 있습니다.(알림 메시지에서 링크를 클릭하여) 또는 콘솔을 통해( **[!UICONTROL Approve]** ) 전송된 이메일 메시지를 통해.

승인 창에서 댓글을 입력할 수 있습니다.

![](assets/s_ncs_user_mkg_resource_valid_ok.png)

**[!UICONTROL Tracking]** 탭에서는 모든 연산자가 승인 프로세스의 다양한 단계를 추적할 수 있습니다.
...
리소스를 게시하려면 마케팅 리소스 대시보드의 편집 영역에서 **[!UICONTROL Publish]** 버튼을 클릭합니다.

![](assets/s_ncs_user_mkg_resource_available.png)

워크플로우를 통해 리소스 게시를 자동화할 수도 있습니다.

리소스를 게시하면 다른 작업에서 리소스를 사용할 수 있게 됩니다(예: ). 게시와 같은 게시는 리소스의 특성에 따라 다릅니다.전단지의 경우, 게시란 파일을 프린터에 보내는 것과 웹 에이전시의 경우, 웹 사이트에 게시하는 등을 의미합니다.
...
* 리소스가 마케팅 리소스 만들기 작업을 통해 관리되는 경우 작업에서 **[!UICONTROL Publish the marketing resource]** 상자를 선택하면 작업이 **[!UICONTROL Finished]**&#x200B;로 설정될 때 실행됩니다( [마케팅 리소스 생성 작업](../../campaign/using/creating-and-managing-tasks.md#marketing-resource-creation-task) 참조)

워크플로우가 즉시 시작되지 않는 경우(예를 들어 워크플로우가 중지되는 경우) 리소스의 상태가 **[!UICONTROL Pending publication]**&#x200B;으로 변경됩니다. 워크플로우가 시작되면 리소스의 상태가 **[!UICONTROL Published]**&#x200B;으로 변경됩니다. 이 상태는 게시 프로세스에서 발생할 수 있는 오류를 고려하지 않습니다. 워크플로우의 상태를 확인하여 제대로 실행되었는지 확인합니다.
...
>[!NOTE]
>
>게재 아웃라인에 대한 자세한 내용은 [게재 아웃라인을 통해 연결된 리소스 연결 및 구조화를 참조하십시오](../../campaign/using/marketing-campaign-deliveries.md#associating-and-structuring-resources-linked-via-a-delivery-outline).
...
>[!NOTE]
>
>Adobe Campaign의 주식 관리에 대한 자세한 내용은 [주식 관리](../../campaign/using/providers--stocks-and-budgets.md#stock-management) 를 참조하십시오.

마케팅 리소스를 스톡 리소스와 연결하려면 스톡 맵을 편집하고 스톡을 편집하거나 만듭니다. 재고 라인을 추가하고 해당 마케팅 리소스를 선택합니다.

![](assets/s_ncs_user_task_in_a_stock.png)

필요한 경우 리소스를 선택한 후 리소스의 오른쪽에 있는 **[!UICONTROL Edit the link]** 아이콘(확대경)을 통해 선택한 리소스를 편집할 수 있습니다.
