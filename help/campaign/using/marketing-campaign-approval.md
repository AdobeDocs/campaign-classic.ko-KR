...
지정된 검토자인 Adobe Campaign 운영자에게 알림 메시지를 보내 승인 요청을 알립니다. 검토자에게 **적절한 권한** 을 참조하고, 보안 영역이 올바르게 정의되었는지 확인합니다. [자세히 알아보기](#selecting-reviewers)

승인 절차는에 나와 있습니다. [이 섹션](#checking-and-approving-deliveries).

>[!NOTE]
>
>게재 소유자만 게재를 시작할 수 있습니다. 다른 연산자(또는 연산자 그룹)가 전달을 시작할 수 있으면에서 검토자로 추가해야 합니다 **[!UICONTROL Delivery start:]** 필드.\
...
이 정보는 캠페인의 승인 로그에서도 사용할 수 있습니다. 이러한 로그는 다음을 통해 액세스됩니다. **[!UICONTROL Edit > Tracking > Approvals]** 탭.
...
승인이 필요한 모든 작업이 캠페인 템플릿에서 선택됩니다( **[!UICONTROL Properties]** > **[!UICONTROL Advanced campaign settings...]** > **[!UICONTROL Approvals]** 탭). 승인을 담당하는 연산자(이 옵션이 활성화되지 않은 경우 알림을 수신함)입니다. 이 작업에 대한 자세한 정보는 [이 섹션](#approving-processes)을 참조하십시오.

이 템플릿을 사용하여 만든 각 캠페인에 대해 이러한 설정을 재정의하고 각 캠페인 게재에 대해 개별적으로 다음 설정을 클릭할 수 있습니다. **[!UICONTROL Properties]** 단추, **[!UICONTROL Approvals]** 탭.
...
각 승인 유형에 대해 승인을 담당하는 운영자 또는 운영자 그룹이 게재의 드롭다운 목록에서 선택됩니다. 를 사용하여 연산자를 더 추가할 수 있습니다 **[!UICONTROL Edit...]** 링크를 클릭합니다. 이 창에서는 승인 마감일도 편집할 수 있습니다.

![](assets/s_user_validation_add_operator.png)

검토자를 지정하지 않으면 캠페인 관리자가 승인을 책임지고 알림을 받게 됩니다. 캠페인 관리자는에 지정됩니다. **[!UICONTROL Edit > Properties]** 캠페인의 탭:
...
>기본적으로 승인 운영자가 정의된 경우 캠페인 관리자는 승인을 수행하거나 게재를 시작할 수 없습니다. 다음을 만들어 이 동작을 수정하고 캠페인 관리자가 게재를 승인/시작하도록 승인할 수 있습니다. **NmsCampaign_Activate_OwnerConfirmation** 옵션 포함 **1** as a value.
...
승인할 정보를 확인하고 승인 수락 또는 거부 여부를 선택한 다음 필요한 경우 설명을 입력합니다. 클릭 **[!UICONTROL Ok]** 저장.
...
알림 메시지에서 사용할 수 있는 링크를 클릭합니다( 참조) [알림](#notifications)). 아래와 같이 로그인해야 합니다.
...
승인 모드(수동 또는 자동)를 선택하려면 **[!UICONTROL Edit > Properties]** 캠페인 또는 캠페인 템플릿의 탭을 클릭한 다음 **[!UICONTROL Advanced campaign settings...]** 그리고 마지막으로 **[!UICONTROL Approvals]** 탭.
...
이렇게 하려면 캠페인 또는 캠페인 템플릿의 승인 창( **[!UICONTROL Edit > Properties]** > **[!UICONTROL Advanced campaign settings...]** > **[!UICONTROL Approvals]** tab)을 눌러 선택 **[!UICONTROL Do not enable notification sending]**.
...
DM 게재의 경우 Adobe Campaign 운영자는 추출 파일을 라우터로 보내기 전에 볼 수 있으며, 필요한 경우 형식을 변경하고 추출을 다시 시작할 수 있습니다. 다음을 참조하십시오 [추출 파일 승인](#approving-an-extraction-file).

각 캠페인에 대해 게재 타겟인 콘텐츠를 승인할 수 있습니다(참조). [콘텐츠 승인](#approving-content)) 및 비용. 승인을 담당하는 Adobe Campaign 운영자는 이메일로 통보를 받을 수 있으며 콘솔 또는 웹 연결을 통해 승인을 수락하거나 거부할 수 있습니다. 다음을 참조하십시오 [게재 승인 단계](#approving-processes).
...
이 시점에서 캠페인의 상태는 다음과 같습니다. **[!UICONTROL To validate]**.
...
>[!NOTE]
>
>타깃팅 워크플로우에서 구성 문제와 연결된 오류가 메시지 준비 중에 발생하는 경우 **[!UICONTROL Restart message preparation]** 링크가 대시보드에 표시됩니다. 오류를 수정하고 이 링크를 클릭하여 타깃팅 단계를 건너뛰고 메시지 준비를 다시 시작합니다.
...
승인 단계는 캠페인과 연결된 템플릿으로 정의됩니다. 템플릿에서 승인할 요소를 선택하고 이러한 승인을 담당하는 Adobe Campaign 연산자를 지정해야 합니다. 캠페인 템플릿에 대한 자세한 내용은 다음을 참조하십시오. [이 섹션](../../campaign/using/marketing-campaign-templates.md#campaign-templates).
...
캠페인의 경우 **[!UICONTROL Edit > Properties]** 탭을 클릭한 다음 **[!UICONTROL Advanced campaign settings...]** 링크 및 마지막으로 **[!UICONTROL Approvals]** 하위 탭 을 클릭하여 승인 구성 페이지에 액세스합니다.

승인 프로세스를 선택하거나 선택 해제하여 Adobe Campaign 운영자가 승인을 담당하도록 지정할 수 있습니다. 개별 연산자, 연산자 그룹 또는 연산자 목록일 수 있습니다.

연산자 목록을 선택하려면 **[!UICONTROL Edit...]** 첫 번째 검토자를 지정하는 필드의 오른쪽에 있는 링크로 이동하여 아래와 같이 필요한 만큼 연산자를 추가합니다.
...
각 게재에 대해 **[!UICONTROL Audit]** 단추 및 **[!UICONTROL Approvals]** 탭으로 승인 날짜 및 자동 미리 알림을 보고 편집할 수 있습니다.
...
>[!CAUTION]
>
>콘텐츠를 승인하려면 증명 주기가 필수입니다. 증명을 통해 정보, 개인화 데이터 표시를 승인하고 링크가 작동하는지 확인할 수 있습니다. 에서 증명을 만드는 방법 알아보기 [이 섹션](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof).
>
...
1. 콘텐츠 승인 여부를 알리는 알림 이메일이 캠페인 책임자에게 전송됩니다.
   ...
   >[!NOTE]
   >
   >캠페인 담당자는 언제든지 콘텐츠 승인 주기를 다시 시작할 수 있습니다. 이렇게 하려면 **[!UICONTROL Content status]** 캠페인 대시보드의 라인(게재 수준)을 클릭한 다음 **[!UICONTROL Reset content approval to submit it again]**.
...
파일이 승인되면 라우터에 알림 이메일을 보낼 수 있습니다. 자세한 내용은 다음을 참조하십시오. [오프라인 게재 시작](../../campaign/using/marketing-campaign-deliveries.md#starting-an-offline-delivery).
