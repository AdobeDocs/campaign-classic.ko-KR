...
지정된 검토자인 [!DNL Adobe Campaign] 운영자에게 알림 메시지를 보내 승인 요청을 알립니다. 검토자에게 승인을 위한 **적절한 권한**&#x200B;이 있는지, 보안 영역이 올바르게 정의되어 있는지 확인하십시오. [검토자 선택에 대한 자세한 정보](#selecting-reviewers).

승인 프로시저는 [승인 프로시저 개요](#checking-and-approving-deliveries)에 표시됩니다.

>[!NOTE]
>
>게재 소유자만 게재를 시작할 수 있습니다. 다른 연산자(또는 연산자 그룹)가 게재를 시작하려면 **[!UICONTROL Delivery start:]** 필드에 검토자로 추가해야 합니다.\
>[검토자 선택에 대한 자세한 정보](#selecting-reviewers).

## 운영 원칙 {#operating-principle-}

예를 들어 예산 승인을 위한 표준 메시지는 다음과 같습니다.

![유효성 검사 링크가 있는 승인 알림 전자 메일](assets/s_user_validation_link_in_mail.png)

그러면 검토자 운영자는 예산을 승인할지 여부를 선택할 수 있습니다.

![수락 또는 거부 옵션이 있는 승인 확인 페이지](assets/s_user_validation_page_confirm.png)

운영자가 유효성을 검사하면 작업 승인 또는 거부가 게재 대시보드로 전달됩니다.

![작업에 대한 승인 링크를 표시하는 Campaign 대시보드](assets/s_user_validation_link_in_op_board.png)

이 정보는 캠페인의 승인 로그에서도 사용할 수 있습니다. 이러한 로그는 **[!UICONTROL Edit > Tracking > Approvals]** 탭을 통해 액세스할 수 있습니다.

![승인 로그를 표시하는 Campaign 편집 탭](assets/s_user_validation_log_in_op_edit_tab.png)

이러한 알림은 승인이 활성화된 각 프로세스에 영향을 받는 운영자에게 전송됩니다.

캠페인 템플릿, 각 캠페인에 대해 개별적으로 또는 게재에 대해 승인을 활성화할 수 있습니다.

승인이 필요한 모든 작업이 캠페인 템플릿에서 선택됩니다(**[!UICONTROL Properties]** > **[!UICONTROL Advanced campaign settings...]** > **[!UICONTROL Approvals]** 탭). 이 옵션이 비활성화되지 않는 한 승인을 담당하는 운영자도 여기에서 선택되고 알림을 받습니다. 자세한 내용은 [게재를 승인하는 단계](#approving-processes)를 참조하세요.
...
![프로세스 선택이 있는 게재 승인 설정](assets/s_user_validation_select_process_from_del.png)
...
![승인 연산자를 위한 검토자 추가 대화 상자](assets/s_user_validation_add_operator.png)

검토자를 지정하지 않으면 캠페인 관리자가 승인을 책임지고 알림을 받게 됩니다. 캠페인 관리자는 캠페인의 **[!UICONTROL Edit > Properties]** 탭에 지정됩니다.

![관리자 필드를 표시하는 캠페인 속성](assets/s_user_op_manager_field.png)

>[!NOTE]
>
>[!DNL Adobe Campaign] 권한이 있는 다른 모든 **[!UICONTROL Administrator]** 연산자도 작업을 승인할 수 있지만 알림을 받지 않습니다.\
...
![콘솔의 Campaign 대시보드 승인 작업](assets/s_user_validation_from_console.png)
...
![알림 링크에 대한 승인 로그인 페이지](assets/s_user_validation__log_in.png)

**[!UICONTROL Accept]** 또는 **[!UICONTROL Reject]**&#x200B;을(를) 선택하고 필요한 경우 댓글을 입력하십시오.

![수락 또는 거부와 의견이 있는 승인 페이지](assets/s_user_validation_save_target_validation.png)
...
* 캠페인 승인 로그에서 **[!UICONTROL Approvals]** 탭의 **[!UICONTROL Edit > Tracking]** 하위 탭:
  ...
  ![캠페인 승인 로그 목록](assets/s_user_validation_log_from_op.png)

* 캠페인 게재 로그에서 **[!UICONTROL Deliveries]** 탭의 **[!UICONTROL Edit > Tracking]** 하위 탭:

  ![승인 상태의 게재 로그 목록](assets/s_user_validation_log_from_delivery_list.png)

* 각 게재의 승인 상태는 **[!UICONTROL Hide/show log]** 탭의 **[!UICONTROL Summary]** 옵션을 클릭하여 볼 수 있습니다.

  ![승인 로그를 표시하는 게재 요약](assets/s_user_validation_log_delivery.png)

* 각 게재의 **[!UICONTROL Tracking > Approvals]** 탭을 통해서도 이 정보에 액세스할 수 있습니다.

  ![게재 추적 승인 탭](assets/s_user_validation_log_from_exe_tab.png)
...
타겟팅 워크플로우를 만들 때 승인이 자동(기본 모드)이면 [!DNL Adobe Campaign]에서 승인이 필요한 즉시 승인 링크를 표시하거나 알림을 보냅니다.
...
![수동 및 자동 모드의 승인 설정](assets/s_user_validation_select_mode.png)
...
![알림이 비활성화된 승인 설정](assets/s_user_validation_notif_desactivate.png)

### 알림 콘텐츠 {#notification-content}

알림 콘텐츠가 특정 템플릿에 정의되어 있습니다. **[!UICONTROL Notification of validations for the marketing campaign]**. 이 템플릿은 **[!UICONTROL Administration > Campaign management > Technical delivery templates]** 트리의 [!DNL Adobe Campaign] 폴더에 저장됩니다.

## 게재 검토 및 승인 {#checking-and-approving-deliveries}

[!DNL Adobe Campaign]을(를) 사용하면 공동 작업 모드에서 마케팅 캠페인의 주요 단계에 대한 승인 프로세스를 설정할 수 있습니다.

DM 게재의 경우 [!DNL Adobe Campaign] 운영자는 추출 파일을 라우터로 보내기 전에 볼 수 있으며, 필요한 경우 형식을 변경하고 추출을 다시 시작할 수 있습니다. [추출 파일 승인](#approving-an-extraction-file)을 참조하세요.

각 캠페인에 대해 게재 대상, 콘텐츠([콘텐츠 승인](#approving-content) 참조) 및 비용을 승인할 수 있습니다. 승인을 담당하는 [!DNL Adobe Campaign] 운영자는 전자 메일로 알림을 받을 수 있으며 콘솔 또는 웹 연결을 통해 승인을 수락하거나 거부할 수 있습니다. [게재를 승인하는 단계](#approving-processes)를 참조하세요.

이러한 유효성 검사 단계가 완료되면 게재를 시작할 수 있습니다. [게재 시작에 대해 자세히 알아보기](../../campaign/using/marketing-campaign-deliveries.md#starting-a-delivery).
...
![유효성을 검사할 게재 상태를 표시하는 Campaign 대시보드](assets/s_ncs_user_edit_del_to_validate.png)
...
![메시지 준비를 다시 시작하는 대시보드 링크](assets/s_user_validation_relaunch_message_preparation.png)
...
* **추출 승인(DM 게재)**
  ...
  **[!UICONTROL Approve content]** 링크는 아래와 같이 캠페인 대시보드에서 사용할 수 있습니다.
  ...
  ![콘텐츠 승인 링크를 표시하는 승인 대시보드](assets/s_ncs_user_edit_file_valid.png)

  추출 파일은 승인 상자를 통해 미리 본 다음 승인 또는 거부할 수 있습니다.

  ![승인 대화 상자의 추출 파일 미리 보기](assets/s_ncs_user_edit_file_valid_preview_file.png)
...
* **연계된 게재 승인**
  ...
  **[!UICONTROL Enable individual approval of each associated delivery]** 옵션은 보조 게재와 연결된 하나의 기본 게재에 사용됩니다. 기본적으로 이 옵션은 기본 게재의 전체 승인을 수행할 수 있도록 선택되어 있지 않습니다. 이 옵션을 선택한 경우 각 게재를 개별적으로 승인해야 합니다.
  ...
  ![연계된 게재에 대해 개별 승인을 활성화하는 옵션](assets/s_ncs_user_task_valid_associate.png)

### 승인 프로세스 선택 {#choosing-the-processes-to-be-approved}

승인 단계는 캠페인과 연결된 템플릿으로 정의됩니다. 템플릿에서 승인할 요소를 선택하고 이러한 승인을 담당하는 [!DNL Adobe Campaign] 연산자를 지정해야 합니다. 캠페인 템플릿에 대한 자세한 정보는 [캠페인 템플릿](../../campaign/using/marketing-campaign-templates.md#campaign-templates)을 참조하세요.
...
승인할 프로세스를 선택하고 선택을 취소하여 [!DNL Adobe Campaign] 연산자를 승인 담당자로 지정할 수 있습니다. 개별 연산자, 연산자 그룹 또는 연산자 목록일 수 있습니다.
...
![승인 일정 및 미리 알림 설정](assets/s_ncs_user_edit_op_valid_calendar.png)

각 게재에 대해 **[!UICONTROL Audit]** 단추와 **[!UICONTROL Approvals]** 탭을 클릭하여 승인 날짜와 자동 미리 알림을 보고 편집합니다.

![날짜 및 미리 알림이 있는 게재 승인 탭](assets/s_ncs_user_edit_del_valid.png)
...
>[!CAUTION]
>
>콘텐츠를 승인하려면 증명 주기가 필수입니다. 증명을 통해 정보, 개인화 데이터 표시를 승인하고 링크가 작동하는지 확인할 수 있습니다. [증명 만들기](../../delivery/using/steps-validating-the-delivery.md#sending-a-proof)에서 증명을 만드는 방법을 알아봅니다.
>
...
1. 새 게재를 만든 후 캠페인 관리자는 캠페인 대시보드의 **[!UICONTROL Submit content]** 링크를 클릭하여 콘텐츠 승인 주기를 시작합니다.
   ...
   ![승인을 위해 콘텐츠를 제출하는 Campaign 대시보드 링크](assets/s_ncs_user_validation_submit_content_validation.png)
...
1. 콘텐츠 책임자에게 알림 이메일이 전송되며 책임자는 승인 여부를 선택할 수 있습니다.
   ...
   * 알림 이메일을 통해:
     ...
     ![증명에 대한 콘텐츠 승인 알림 이메일](assets/s_ncs_user_del_content_valid_bat_notif.png)
   ...
   * 콘솔 또는 웹 인터페이스, 게재 추적, 게재 대시보드 또는 캠페인 대시보드를 통해 다음과 같은 작업을 수행할 수 있습니다.
     ...
     ![콘텐츠 증명 목록을 표시하는 게재 추적](assets/s_ncs_user_validation_content_bat_op.png)

     >[!NOTE]
     >
     >이 캠페인 대시보드를 사용하면 **[!UICONTROL Inbox rendering...]** 링크를 클릭하여 보낸 증명 목록을 볼 수 있습니다. 콘텐츠를 보려면 목록 오른쪽에 있는 **[!UICONTROL Detail]** 아이콘을 클릭합니다.

     ![콘텐츠 승인을 위한 증명 세부 정보 보기](assets/s_ncs_user_validation_content_BAT_details.png)
...
1. 콘텐츠 승인 여부를 알리는 알림 이메일이 캠페인 책임자에게 전송됩니다.
   ...
   ![콘텐츠 승인을 다시 시작하는 Campaign 대시보드 링크](assets/s_user_validation_relaunch_content_validation.png)
...
1. 새 게재를 만든 후 캠페인 담당자가 캠페인 대시보드의 **[!UICONTROL Submit content editing]** 링크를 클릭하여 콘텐츠 편집 주기를 시작합니다.
   ...
   ![콘텐츠 편집을 제출하는 Campaign 대시보드 링크](assets/s_ncs_user_validation_submit_content_edition.png)

1. 콘텐츠 편집 담당자는 콘텐츠를 사용할 수 있다는 이메일을 받게 됩니다.

   ![콘텐츠 편집 알림 전자 메일](assets/s_ncs_user_validation_submit_content_notif.png)

1. 그런 다음 콘솔에 로그온하여 게재를 열고 간소화된 도우미를 사용하여 편집하여 제목, HTML 및 텍스트 콘텐츠를 변경하고 증명을 보낼 수 있습니다.

   ![게재 콘텐츠 편집을 위한 간소화된 도우미](assets/s_user_validation_content_edition.png)
...
1. 콘텐츠 편집 담당자는 게재 콘텐츠를 변경하고 나면 콘텐츠를 사용 가능하게 만들 수 있습니다.
   ...
   * **[!UICONTROL Available content]** 콘솔을 통해 [!DNL Adobe Campaign] 링크를 클릭합니다.

     콘텐츠를 사용할 수 있도록 하는 ![콘솔 링크](assets/s_ncs_user_validation_submit_content_available.png)

   * 알림 메시지에서 링크를 클릭한 다음 콘텐츠 가용성을 승인합니다.

     ![콘텐츠 가용성을 승인하는 알림 링크](assets/s_ncs_user_validation_submit_content_available2.png)

     운영자는 콘텐츠를 캠페인 담당자에게 제출하기 전에 댓글을 추가할 수 있습니다.

     ![콘텐츠 가용성을 제출하기 전에 댓글 필드](assets/s_ncs_user_validation_submit_content_available3.png)

     알림 메시지를 통해 검토자는 컨텐츠를 승인하거나 거부할 수 있습니다.

     ![콘텐츠 가용성에 대한 승인 응답](assets/s_ncs_user_validation_submit_content_available4.png)
...
1. 알림 이메일에는 게재 렌더링을 볼 수 있는 전송된 증명에 대한 링크와 게재 콘텐츠를 승인 또는 거부하는 버튼이 포함되어 있습니다.
   ...
   ![증명 링크가 있는 외부 콘텐츠 승인 전자 메일](assets/s_user_validation_external_content.png)

### 추출 파일 승인 {#approving-an-extraction-file}

오프라인 게재의 경우 [!DNL Adobe Campaign]은(는) 설정 방법에 따라 라우터로 전송되는 추출 파일을 생성합니다. 콘텐츠는 사용된 내보내기 템플릿에 따라 다릅니다.

콘텐츠, 타겟팅 및 예산이 승인되면 캠페인에 대한 추출 워크플로우가 시작될 때까지 게재가 **[!UICONTROL Extraction pending]**(으)로 변경됩니다.

![추출 보류 중인 게재 상태 표시](assets/s_ncs_user_waiting_file_extraction.png)

추출 요청 날짜에 추출 파일이 만들어지고 게재 상태가 **[!UICONTROL File to approve]**(으)로 변경됩니다.

![승인할 파일을 표시하는 게재 상태](assets/s_ncs_user_file_extract_to_valid.png)
