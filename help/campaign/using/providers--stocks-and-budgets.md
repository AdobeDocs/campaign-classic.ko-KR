...
1. Adobe Campaign에서 서비스 공급자 생성
   ...
   자세한 내용은 [서비스 공급자 추가](#adding-a-service-provider).

1. 관련 서비스 템플릿의 비용 범주 및 구조 정의

   자세한 내용은 [원가 범주 정의](#defining-cost-categories) 및 [원가 구조 정의](#defining-the-cost-structure).

1. 프로세스 구성

   자세한 내용은 [서비스와 연결된 프로세스 구성](#configuring-processes-associated-with-a-service).

1. 캠페인 수준에서 서비스 공급자 참조

   자세한 내용은 [캠페인과 서비스 연결](#associating-a-service-with-a-campaign).
...
1. 서비스 공급자 목록을 마우스 오른쪽 단추로 클릭하고 **[!UICONTROL New]**&#x200B;를 클릭하거나 **[!UICONTROL New]** 서비스 공급자 목록 위의 단추.
...
>[!NOTE]
>
>자세한 내용은 [원가 구조 정의](#defining-the-cost-structure).
...
1. 에서 **[!UICONTROL Services]** 서비스 공급자 탭에서 **[!UICONTROL Add]** 버튼을 클릭하고 서비스 템플릿의 이름을 지정합니다.
...
1. 각 프로세스 유형에 대한 비용 범주를 만듭니다(DM/이메일 등으로 제공). 또는 작업)을 수행할 수 있습니다. 이렇게 하려면 **[!UICONTROL Cost categories]** 탭한 다음 **[!UICONTROL Add]** 버튼을 누르고 각 원가 범주의 매개변수를 입력합니다.
   ...
   * 이 원가 범주에 대한 레이블을 입력하고 관련 프로세스 유형을 선택합니다. 게재 기준 **[!UICONTROL Direct mail]**, **[!UICONTROL Email]**, **[!UICONTROL Mobile]**, **[!UICONTROL Telephone]** 또는 **[!UICONTROL Task]**.
   * 을(를) 클릭합니다. **[!UICONTROL Add]** 버튼을 눌러 이 범주와 연관된 원가 유형을 정의합니다.
   * 필요한 경우 사용된 수량이 기존 주식과 자동으로 관련되도록 재고 라인을 각 원가 유형과 연관시킵니다.

      >[!NOTE]
      >
      >스톡 라인은 **[!UICONTROL Stock management]** 노드 아래에 있어야 합니다.\
      >자세한 내용은 [주식 및 주문 관리](#stock-and-order-management).

1. 이 비용 범주의 값을 미리 선택할 수 있습니다. 이 값은 기본적으로 서비스 공급자 비용 범주에서 제공됩니다(공백 대신). 이렇게 하려면 **[!UICONTROL Selected]** 관련 범주 유형에 대한 열:
...
를 통해 서비스와 연관된 프로세스에 대한 정보를 연결할 수 있습니다 **[!UICONTROL Processes]** 탭.

이렇게 하려면 **[!UICONTROL Processes]** 라우터에 정보 전송을 구성하려면 탭을 클릭합니다.
...
게재에 대한 원가 범주 정의 중에 원가 유형에 대한 필수 선택을 부과할 수 있습니다. 이렇게 하려면 을(를) 선택합니다. **[!UICONTROL A cost type must be selected]**.
...
1. 관련 서비스 공급자의 주식 생성 및 참조
   ...
   자세한 내용은 [스톡 만들기](#creating-a-stock).

1. 재고 라인 추가

   자세한 내용은 [재고 라인 추가](#adding-stock-lines).

1. 경고 이벤트 시 연산자 알림

   자세한 내용은 [경고 운영자](#alerting-operators).

1. 주문 및 공급.

   을(를) 참조하십시오. [주문](#orders).
...
1. 스톡 레이블을 입력하고 드롭다운 목록에서 스톡 레이블이 연관된 서비스 공급자를 선택합니다.
   ...
   >[!NOTE]
   >
   >자세한 내용은 [서비스 공급자 및 비용 구조 생성](#creating-service-providers-and-their-cost-structures).
...
을(를) 클릭합니다. **[!UICONTROL Create]** 버튼을 클릭하여 스톡 매개 변수를 정의합니다.
...
캠페인에서 예산을 선택하는 경우 초기 금액을 입력해야 합니다. 계산된 비용은 입력된 금액의 약정 레벨(비용, 예상, 예약, 약정)에 따라 자동으로 갱신됩니다. 자세한 내용은 [금액 계산](../../mrm/using/controlling-costs.md#calculating-amounts).

>[!NOTE]
>
>예산 작성 절차는 다음과 같습니다. [예산 생성](../../mrm/using/controlling-costs.md#creating-a-budget).
