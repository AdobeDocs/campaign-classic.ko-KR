...
Adobe Campaign을 사용하면 작업을 생성하고 애플리케이션 내에서 직접 전체 라이프사이클을 관리할 수 있습니다. 프로그램 및 캠페인 구현은 Adobe Campaign 운영자 또는 외부 서비스 제공자에게 할당된 작업으로 분류할 수 있습니다. 이 작업 모드를 사용하면 모든 프로그램 참가자 및 외부 참가자를 포함하는 개방형 공동 작업 환경을 만들 수 있습니다.
...
기본적으로 Adobe Campaign 연산자가 작업을 만들면 자동으로 사용자에게 할당됩니다. 다른 연산자를 선택하려면 **[!UICONTROL Assigned to]** 필드를 사용합니다.
...
다른 연산자가 이미 이 작업을 하도록 지정된 경우에도 작업을 담당하는 사람이 승인 작업을 자신에게 할당할 수 있습니다. 정의된 검토자가 없는 경우, 알림이 작업을 담당하는 사람에게 전송됩니다. 권한이 있는 다른 모든 Adobe Campaign 운영자도 작업을 승인할 수 **[!UICONTROL Administrator]** 있습니다. 그러나 알림 수신은 받지 않습니다.
...
나중에 시작 날짜가 있는 작업을 만들거나 수정할 때 Adobe Campaign은 작업 담당자에게 이메일을 보내 시작 시기를 알리는 방법을 제공합니다.
...
알림을 예약한 경우 작업을 저장할 때 Adobe Campaign은 알림을 즉시 보내도록 보류 중입니다. 전송하기로 결정할 수 있으며 예약된 알림을 대체하지 않습니다.
...
하지만 그룹 작업에 연결할 작업을 이미 만든 경우 **[!UICONTROL Linked to]** 상자의 **[!UICONTROL Properties]** 필드를 통해 이 작업을 수행할 수 있습니다.
...
* 작업을 예약하려면 나중에 시작 날짜를 입력합니다. 그런 다음 작업 수행과 관련된 Adobe Campaign 운영자에게 첫 번째 알림을 보낼 수 있습니다. 작업 [수명 주기 완료를 참조하십시오](#complete-task-life-cycle).
