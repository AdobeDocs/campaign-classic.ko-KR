...
1. **[!UICONTROL New]** 을 클릭하거나 템플릿 목록에서 마우스 오른쪽 단추를 클릭하고 드롭다운 목록에서 **[!UICONTROL New]** 를 선택합니다.
1. 가설 레이블을 입력합니다.
1. **[!UICONTROL Hypothesis type]** 을 통해 오퍼나 게재에 대한 가설을 위해 템플릿을 지정할지 여부를 지정합니다.
1. **[!UICONTROL Delivery]** 유형 템플릿의 경우 컨트롤 그룹과 함께 측정을 수행해야 하는지 아니면 컨트롤 그룹 없이 측정을 수행해야 하는지 여부를 지정합니다(자세한 내용은 [가설 템플릿의 속성](#properties-of-a-hypothesis-template) 참조).
1. **[!UICONTROL Delivery]** 유형 템플릿의 경우 특정 채널을 선택하거나 **[!UICONTROL Channel]** 드롭다운 목록을 사용하여 Adobe Campaign에서 사용 가능한 모든 채널에 템플릿을 적용할 수 있습니다(자세한 내용은 [가설 템플릿의 속성](#properties-of-a-hypothesis-template) 참조).
...
1. **[!UICONTROL Transactions]** 탭에서 가설 계산에 필요한 테이블 및 필드를 지정합니다(자세한 내용은 [트랜잭션](#transactions) 참조).
1. 서식 파일이 **[!UICONTROL Offer]**&#x200B;에 대해 구성된 경우 **[!UICONTROL Update offer proposition status]** 옵션을 활성화할 수 있습니다.이 경우 변경할 오퍼 제안 상태를 선택합니다.
...
* **[!UICONTROL Measurement with control group]**:컨트롤 그룹이 게재에 대해 정의되었는지 아니면 캠페인에 정의되어 있는지 여부를 나타내며 측정 지표에 포함할 수 있습니다. 게재를 받지 않는 제어 그룹은 게재를 받은 대상 모집단과 비교하여 게재 후의 캠페인 영향을 측정할 수 있도록 해줍니다.
   ...
   컨트롤 그룹 정의 및 구성에 대한 자세한 내용은 [컨트롤 그룹 정의](../../campaign/using/marketing-campaign-deliveries.md#defining-a-control-group)를 참조하십시오.

* **[!UICONTROL Channel]**:드롭다운 목록에서 을(를) 선택하여 Adobe Campaign 콘솔의 모든 채널에 대해 특정 채널을 선택하거나 가설 템플릿을 사용할 수  **[!UICONTROL All channels]** 있도록 할 수 있습니다. 특정 채널에 대한 템플릿을 구성하면 가설을 만들 때 채널당 게재를 자동으로 필터링할 수 있습니다( [가설 만들기](../../campaign/using/creating-hypotheses.md) 참조).
...
* **[!UICONTROL Automatic execution]**:필요한 경우 가설 재계산을 예약할 수 있습니다(예를 들어 게재 종료 시까지 지표를 정기적으로 업데이트하려면).
   ...
   1. **[!UICONTROL Frequency of execution...]** 링크를 클릭한 다음 **[!UICONTROL Change...]** 버튼을 클릭합니다.
...
이 탭에는 거래의 관점에서 수신자 반응 내역을 저장할 수 있는 다양한 필드와 테이블이 포함되어 있습니다. 응답 관리 전용 테이블에 대한 자세한 내용은 [구성](../../configuration/using/about-schema-reference.md) 안내서를 참조하십시오.
...
* **[!UICONTROL Measurement period]**:가설이 실행되고 구매 라인이 복구되는 시작 및 종료 날짜를 구성할 수 있습니다.
   ...
   가설이 바로 시작된다면, 즉시 그것을 시작하려 한다면, 그것은 강요될 수 있습니다. 그렇지 않으면 가설 생성 날짜를 기반으로 하는 구성된 계산 종료 날짜를 기반으로 자동으로 트리거됩니다( [게재](../../campaign/using/creating-hypotheses.md#creating-a-hypothesis-on-the-fly-on-a-delivery) 시 가설 생성 참조).

* **[!UICONTROL Transaction/Margin amount]**:이러한 필드는 선택 사항이며 이직률 표시기를 자동으로 계산할 수  [있습니다(지표 참조](../../campaign/using/hypothesis-tracking.md#indicators)).
* **[!UICONTROL Unit amount]**:수입 계산을 위해 금액을 설정할 수 있도록 해줍니다(지표  [참조](../../campaign/using/hypothesis-tracking.md#indicators)).
...
* **트랜잭션 필터링**:탭 **[!UICONTROL Scope]** 에서 가설에 대한 필터를 구성할 수 있습니다. 방법은 다음과 같습니다.
...
* **수신자를 필터링합니다**.탭에서  **[!UICONTROL Scope]** 메시지에 연결된 정보(게재, 수신자, 이메일 주소, 서비스 등)로 가설을 제한할 수 있습니다.

   1. **[!UICONTROL Add a filter]** 링크를 클릭한 다음 **[!UICONTROL Edit query]** 을 클릭합니다.
...
이 예제에서는 DM 유형 게재에 대한 가설 템플릿을 만듭니다. 가설을 기반으로 하는 거래 테이블(**구매**)에는 문서 또는 제품에 연결된 구매 라인이 포함되어 있습니다. 구매 테이블의 문서 또는 제품에 대한 가설을 작성하도록 모델을 구성하려고 합니다.
...
1. **[!UICONTROL Scope]** 탭에서 **[!UICONTROL Edit query]** 를 클릭하여 가설 필터를 만듭니다.

   ![](assets/response_hypothesis_model_example_011.png)

   이렇게 생성된 템플릿을 사용하면 구매 테이블의 제품이나 문서에 대한 가설을 실행할 수 있습니다.

1. **[!UICONTROL Save]** 을 클릭하여 템플릿을 기록합니다.
