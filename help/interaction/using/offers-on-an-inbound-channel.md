...
이 사례 연구를 구현하려면 두 가지 오퍼 환경이 필요합니다.하나는 익명의 상호 작용과, 하나는 식별된 연락처에 대해 특별히 구성된 오퍼가 있는 것입니다. 식별된 오퍼 환경은 연락처가 로그인되어 있지 않아 식별되지 않는 경우 자동으로 익명 오퍼 환경으로 전환되도록 구성됩니다.
...
1. 이 **[!UICONTROL Eligibility]** 탭으로 이동하여 오퍼가 웹 사이트에 연속적으로 표시되도록 이 카테고리의 첫 번째 오퍼에 적용한 것과 동일한 두께를 적용합니다.
1. 각 오퍼에 대한 승인 주기를 실행하여 업데이트를 시작합니다. 컨텐츠 승인 중에 파트너 또는 **우수** 오퍼 **장소를** 활성화합니다.
