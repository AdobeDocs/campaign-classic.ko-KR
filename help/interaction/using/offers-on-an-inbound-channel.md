...
네이뱅크 사이트는 해당 페이지를 탐색하는 익명의 방문자를 겨냥한 제안을 웹 사이트에 표시하려고 합니다.
...
오퍼 환경 [](../../interaction/using/live-design-environments.md#creating-an-offer-environment) 만들기에 설명된 절차에 따라 방문자 **의 차원을 기반으로 익명 환경을**&#x200B;만드십시오.

새 환경이 들어 있는 트리 구조가 표시됩니다.
...
1. 클릭 **[!UICONTROL New]** 을 클릭하여 통화 채널을 만듭니다.
...
1. 레이블을 변경하고 채널을 **[!UICONTROL Inbound Web]** 선택합니다. 또한 **[!UICONTROL Enable unitary mode]** 상자를 체크해야 합니다

   ![](assets/offer_inbound_anonymous_example_006.png)

1. 공간에 사용된 오퍼 컨텐츠 필드를 선택하고 관련 상자를 선택하여 필요에 따라 지정합니다.

   이렇게 하면 다음 요소 중 하나가 없는 오퍼가 이 공간에 적용되지 않습니다.
...
1. 노드를 마우스 오른쪽 단추로 **[!UICONTROL Offer catalog]** 클릭하고 선택합니다 **[!UICONTROL Create a new 'Offer category' folder]**.

   예를 들어 새 카테고리, **금융 제품** 이름을 지정합니다.

1. 카테고리 **[!UICONTROL Eligibility]** 탭으로 이동한 다음 **파이낸싱** (fanching)을 테마로 입력한 다음 변경 사항을 저장합니다.
...
1. 곧바로 사용할 수 있는 익명 오퍼 템플릿 또는 이전에 만든 템플릿을 선택합니다.
...
1. 응용 프로그램 컨텍스트에 따라 **[!UICONTROL Eligibility]** 탭으로 이동하여 오퍼의 가중치를 지정합니다.

   이 예에서 오퍼는 연말에 대한 우선 순위로 사이트의 홈 페이지에 표시되도록 구성됩니다.

   ![](assets/offer_inbound_anonymous_example_016.png)

1. 탭으로 **[!UICONTROL Content]** 이동하여 오퍼의 컨텐츠를 정의합니다.

   >[!NOTE]
   >
   >웹 공간에 필요한 요소 목록 **[!UICONTROL Content definitions]** 을 표시하도록 선택할 수 있습니다.
...
1. 탭으로 **[!UICONTROL Eligibility]** 가서 첫 번째 오퍼와 동일한 두께를 적용합니다.
1. 온라인 환경에서 승인된 오퍼 공간과 오퍼를 이용할 수 있도록 각 오퍼에 대한 승인 주기를 실행합니다.

### 웹 사이트에서 웹 오퍼 공간 구성 {#configure-the-web-offer-space-on-the-website}

방금 구성한 오퍼를 웹 사이트에 표시하려면, 상호 작용 엔진을 호출하기 위해 사이트의 HTML 페이지에 JavaScript 코드를 삽입하십시오(여기에 대한 자세한 내용은 인바운드 채널 [정보 참조](../../interaction/using/about-inbound-channels.md)).

1. HTML 페이지로 이동하여 이전에 만든 익명 오퍼 공간의 내부 이름과 일치하는 값이 있는 @id 속성을 [i_](#creating-anonymous-offer-spaces)****&#x200B;앞에 삽입합니다(익명 오퍼 공간 만들기 참조).
...
1. 호출 URL을 삽입합니다.
   ...
   위의 파란색 URL 상자는 인스턴스 이름, 환경의 내부 이름(익명 환경 [만들기 참조) 및 카테고리에 연결된 테마(](#creating-an-anonymous-environment)오퍼 카테고리 및 테마[](#creating-an-offer-category-and-a-theme)만들기)에 해당합니다. 후자는 선택 사항입니다.

방문자가 웹 사이트의 홈 페이지에 액세스하면 **파이낸싱** 테마가 있는 오퍼가 HTML 페이지에 구성된 대로 표시됩니다.

![](assets/offer_inbound_anonymous_example_022.png)

페이지를 여러 번 방문하는 사용자는 동일한 가중치가 할당되었으므로 카테고리의 하나 또는 다른 오퍼를 보게 됩니다.

## 확인되지 않은 연락처의 경우 익명 환경으로 전환 {#switching-to-an-anonymous-environment-in-case-of-unidentified-contacts}

Neobank 회사는 두 개의 다른 대상에 대한 마케팅 오퍼를 만들고 싶습니다. 익명의 웹 사이트 브라우저에 대한 일반 오퍼를 표시하려고 합니다. 이 사용자 중 한 명이 Neobank에서 제공하는 식별자를 가진 고객인 것으로 밝혀지면 Dell은 고객이 로그인하는 즉시 개인화된 제안을 받게 됩니다.

이 케이스 스터디는 다음 시나리오를 기반으로 합니다.

1. 방문자는 로그온하지 않고 Neobank 웹 사이트를 탐색합니다.

   ![](assets/offer_inbound_fallback_example_050.png)

   페이지에 세 개의 익명 오퍼가 표시됩니다.Neobank **제품에 대한 두 가지** 최고의오퍼와 Neobank 파트너로부터 한 가지 오퍼를 제공합니다.
...
이 사례 연구를 구현하려면 두 가지 제안 환경이 필요합니다.하나는 익명의 상호 작용과, 다른 하나는 식별된 연락처에 대해 특별히 구성된 오퍼입니다. 연락처가 로그인되어 있지 않아 식별되지 않는 경우 식별된 오퍼 환경이 익명 오퍼 환경으로 자동 전환되도록 구성됩니다.
...
1. 전달 매핑 마법사(**방문자** 매핑)를 통해 익명 인바운드 상호 작용에 대한 오퍼 환경을 만듭니다. 자세한 내용은 오퍼 환경 [만들기를 참조하십시오](../../interaction/using/live-design-environments.md#creating-an-offer-environment).

   ![](assets/offer_env_anonymous_003.png)

### 익명 환경을 위한 오퍼 공간 구성 {#configuring-offer-spaces-for-the-anonymous-environment}

웹 사이트에 표시되어야 하는 오퍼는 두 가지 카테고리에 속합니다. **최고의 제안** 및 **파트너**. 이 예에서는 각 카테고리에 대한 특정 오퍼 공간을 만듭니다.
...
1. 새 문자 공간을 **[!UICONTROL Inbound web]** 만듭니다.

   ![](assets/offer_inbound_fallback_example_024.png)

1. 레이블 입력: **예를 들어 웹 우수 익명** 오퍼를 참조하십시오.
...
먼저 두 가지 오퍼 카테고리를 만듭니다.가장 **좋은 오퍼** 카테고리 및 **파트너** 카테고리. 각 부문에는 익명 연락처에 대한 두 가지 오퍼가 포함됩니다.

1. 방금 만든 익명 환경 **[!UICONTROL Offer catalog]** 으로 이동합니다.
1. 최고 오퍼 **[!UICONTROL Offer category]** 가 있는 **** 폴더를 레이블로 추가합니다.

   ![](assets/offer_inbound_fallback_example_027.png)

1. 두 번째 카테고리를 **레이블로** 만듭니다.
...
1. 이 카테고리의 첫 번째 오퍼에 적용한 것과 동일한 두께를 **[!UICONTROL Eligibility]** 적용하여 오퍼가 웹 사이트에 연속 표시되도록 합니다.

   ![](assets/offer_inbound_fallback_example_036.png)

1. 각 오퍼에 대한 승인 주기를 실행하여 라이브로 만듭니다. 컨텐츠를 승인할 때는 오퍼에 따라 **파트너** 또는 **최고 오퍼** 공간을활성화합니다.

### 식별된 환경에서 오퍼 공간 구성 {#configure-the-offer-spaces-in-the-identified-environment}

웹 사이트에서 제공할 오퍼는 다음 두 가지 카테고리에서 가져옵니다. **최고의 제안** 및 **파트너**. 이 예에서는 각 카테고리에 대한 특정 공간을 만들고 싶습니다.

두 오퍼 공백을 만들려면 익명 오퍼 공간에 대해 동일한 절차를 적용합니다. 익명 [환경을 위한 오퍼 공간 구성을 참조하십시오](#configuring-offer-spaces-for-the-anonymous-environment).

1. Adobe Campaign 트리에서 방금 만든 환경으로 이동하여 **최고 오퍼** 및 **파트너** 오퍼 공간을추가합니다.
1. 익명 환경에 대한 오퍼 공간 [구성에 설명된 프로세스를 적용합니다](#configuring-offer-spaces-for-the-anonymous-environment).
...
1. 드롭다운 목록을 사용하여 이전에 만든 익명의 웹 오퍼 공간을 선택합니다(익명 환경을 [위한 오퍼 공간 구성 참조](#configuring-offer-spaces-for-the-anonymous-environment)).
...
이 예에서 연락처 식별은 Adobe Campaign 데이터베이스의 이메일 주소 덕분에 이루어집니다. 받는 사람 이메일을 스페이스에 추가하려면 다음 프로세스를 적용합니다.

1. 식별된 환경에서 오퍼 공간 폴더로 이동합니다.
1. 최상의 **오퍼** 공간을 선택하고 을 클릭합니다 **[!UICONTROL Advanced parameters]**.
...
1. 을 **[!UICONTROL OK]** 클릭하여 **[!UICONTROL Advanced parameters]** 창을 닫고 **최상의 오퍼** 공간 구성을 마칩니다.
...
두 개의 개별 카테고리를 만들 예정입니다.각각 **2개의 맞춤형 오퍼가** 포함된 **최고 오퍼** 카테고리 및파트너카테고리.

1. 식별된 환경의 **[!UICONTROL Offer catalogs]** 노드로 이동합니다.
1. 익명 환경에서와 마찬가지로, **[!UICONTROL Offer category]** 최고의 오퍼 **및** 파트너 **를 레이블로 두 개** 폴더를 추가합니다.
...
1. 최상의 **오퍼** 카테고리로 이동하여 첫 번째 맞춤형 오퍼를 만듭니다.
...
1. 이 카테고리의 첫 번째 오퍼에 적용한 것과 동일한 두께를 **[!UICONTROL Eligibility]** 적용하여 오퍼가 웹 사이트에 연속 표시되도록 합니다.
1. 각 오퍼에 대한 승인 주기를 실행하여 업데이트를 시작합니다. 컨텐츠 승인 중에 **파트너** 또는 **베스트 오퍼** 공간을활성화합니다.

### 웹 페이지에서 오퍼 공간 구성 {#configuring-offer-spaces-on-the-web-page}

Neobank 회사의 웹 사이트에는 다음과 같은 세 가지 오퍼가 있습니다.은행 관련 오퍼의 경우 **최고 오퍼** 카테고리의 2개, **파트너** 카테고리의 오퍼에 1개.
...
1. 그런 다음 속성 값 정의에 사용할 스크립트를 삽입합니다.

   ![](assets/offer_inbound_fallback_example_040.png)

   이 예에서 **ContBO1****및** ContBO2 **는 이전에**&#x200B;식별된 환경에서 생성된 Best OfferOfferIdentified **값(예:** Best OfferOfferIdentified)을받습니다. CatBestOffer **및** CatBestOfferAnient **값은 익명 및 식별된 환경을 위한** Best Offer **범주의 내부 이름과** 일치합니다.

   ![](assets/offer_inbound_fallback_example_041.png)

   마찬가지로, **ContPtn** 은 식별된 환경에서 생성된 **파트너** 오퍼 공간의 내부 이름과 일치하는 OSWebPartnerIdentified **** 값을받습니다. **CatPartner** 및 **CatPartnerAnuniar** 은 익명 및 식별된 환경의 **Partner** 카테고리의 내부 이름과 일치합니다.
...
1. 호출 URL을 삽입합니다.
   ...
   URL에는 식별된 **환경의 내부 이름인 EnvNeobankRecipe가**&#x200B;포함됩니다.
...
이 예에서 엔진은 식별된 환경(**EnvNeobankIdnRecipe)을 인식합니다**. 웹 페이지에 있는 첫 번째 및 두 번째 오퍼의&#x200B;**공간과 (** OSWebBestOfferIdentified **** ) 웹 페이지에 있는 두 번째 오퍼에 대한 오퍼 공간(**CatBest** Offer **)과 (********** OSWebPartnerIdentifiedPartnerSpace) 오퍼와 The Space(CowaBestBestBest OfferOfferCategory) 사이트에서 세 번째 제공 공간을 위한 CatPartnerTelevision.

엔진에서 수신자를 식별할 수 없는 경우, 엔진은 식별된 오퍼 공간에서 참조되는 익명 오퍼 공간과 스크립트에서 지정한 익명 범주(**CatPartner** 및 **CatPartnerAnuniar**)로 전환됩니다.
