...
확장성을 지원하고 인바운드 채널에 24/7 서비스를 제공하기 위해 분산 아키텍처와 상호 작용 을 사용할 수 있습니다. 이 유형의 아키텍처는 메시지 센터에서 이미 사용되고 있으며, 다음과 같은 몇 가지 인스턴스로 구성됩니다.
...
>[!NOTE]
>
>패키지를 설치할 때 **long** 유형 필드는 제안 ID와 같이 **nms:proposition** 테이블의 **int64** 유형 필드가 됩니다. 이 유형의 데이터는 [이 섹션에 자세히 설명되어 있습니다.](../../configuration/using/schema-structure.md#mapping-the-types-of-adobe-campaign-dbms-data)

데이터 보존 기간은 각 인스턴스에 구성해야 합니다(배포 마법사의 **[!UICONTROL Data purge]** 창을 통해). 실행 인스턴스에서 이 기간은 유형화 규칙(슬라이딩 기간) 및 자격 규칙을 계산하는 데 필요한 이전 깊이에 해당해야 합니다.
...
이전에 인스턴스에 상호 작용 패키지가 없는 경우 마이그레이션이 필요하지 않습니다. 기본적으로, 제안 테이블은 패키지가 설치된 후 64비트 이내입니다.
...
**Number** 유형의 크기를 편집해도 값이 반환되거나 인덱스가 다시 작성되지 않습니다. 그러므로 그것은 즉시 입니다.
