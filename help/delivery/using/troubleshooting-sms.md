...
제공자 측에서 발생할 수 있는 충돌을 진단하려면 제공자에게 연락해야 합니다.

   * 일부 외부 계정은 동일한 로그인/암호 조합을 공유합니다.
공급자는 어떤 외부 계정 `BIND PDU` 에서 오는지 알 수 있는 방법이 없으므로 여러 계정의 모든 연결을 단일 연결로 취급합니다. 두 계정에서 MO 및 SR을 임의로 라우팅하여 문제가 발생했을 수 있습니다.
...
* (/postupgrade 디렉토리에서) 시스템이 업그레이드되었는지 여부와 시기를 조사합니다.
* SMS에 영향을 주는 패키지가 최근에 업그레이드되었는지 여부를 조사합니다(/var/log/dpkg.log).
...
* `BIND PDU` 에서 0 `command_status` 이 아닌 코드를 반환하는 경우 공급자에게 자세한 정보를 요청합니다.

* TCP 연결이 공급자에게 이루어질 수 있도록 네트워크가 올바르게 구성되었는지 확인합니다.

* 공급자에게 Adobe Campaign 인스턴스의 허용 목록에 IP를 제대로 추가했는지 확인하도록 요청합니다.

* 외부 계정&#x200B;**설정을 확인합니다**. 공급자에게 필드의 값을 물어보십시오.
...
* 공급자가 다음 중 하나를 전송하여 연결을 닫는 경우: `TCP FIN` 또는 `TCP RST packet`공급자에게 자세한 정보를 문의하십시오.
...
* 연결이 안정적인지 확인합니다. SMPP 연결은 Adobe Campaign Classic의 송신기를 제외하고 최소 1시간 동안 지속적으로 유지되어야 합니다. 섹션 보기 [불안정한 연결 문제](sms-protocol.md#issues-unstable-connection).

* MTA를 다시 시작하면 MT를 다시 보내는 데 시간이 조금 걸리는 경우 연결이 불안정하여 전송률이 저하될 수 있습니다. 섹션 보기 [불안정한 연결 문제](troubleshooting-sms.md#issues-unstable-connection).
...
* 전송 창을 낮춥니다. 전송 기간은 대기 시간을 감당 `SUBMIT_SM_RESP` 할 수 있을 만큼 커야 합니다. 이 값은 창이 꽉 찼을 때 오류가 발생할 경우 복제할 수 있는 최대 메시지 수를 나타냅니다.

## SR(배달 영수증) 처리 시 문제 {#issue-process-SR}
...
* 에서 ID 추출 및 오류 처리와 관련된 정규 표현식 확인 **외부 계정**. 의 `DELIVER_SM PDU`컨텐츠에 대해 유효성을 검사해야 할 수도 있습니다.

* 테이블에 오류가 제대로 프로비저닝되어 `broadLogMsg` 있는지 확인합니다.

다음과 같은 경우 `DELIVER_SM PDU` Adobe Campaign Classic 확장 SMPP 커넥터가 을 확인했지만 broadLog가 제대로 업데이트되지 않았습니다. 섹션에 설명된 ID 조정 프로세스를 확인하십시오 [일치하는 MT, SR 및 broadlog 항목](sms-protocol.md#matching-mt).
...
* 테스트 중에 SMPP 추적을 활성화합니다. TLS를 사용하도록 설정하지 않으면 MO 문제를 해결할 때 네트워크 캡처를 수행하여 PDU에 올바른 정보가 포함되어 있고 형식이 올바른지 확인해야 합니다.
...
* 다음과 같은 경우 `DELIVER_SM PDU` 가 나타나면 Adobe Campaign에서 정상적으로 승인되었는지 확인합니다. `DELIVER_SM_RESP PDU` (코드 0). 이 RESP는 모든 처리 논리가 Adobe Campaign(자동 회신 및 허용/차단 목록)에 의해 적용되었음을 보장합니다. 그렇지 않은 경우 SMS 프로세스 로그에서 오류 메시지를 검색합니다.

* 자동 회신이 활성화되어 있으면 `SUBMIT_SM` 이(가) 공급자에게 전송되었습니다. 그렇지 않은 경우 SMS 프로세스 로그에서 오류 메시지를 찾을 수 있습니다.
...
* 검역표와 배송 로그에서 전화번호 형식이 정확히 동일한지 확인합니다. 그렇지 않은 경우 다음을 참조하십시오. [섹션](sms-protocol.md#automatic-reply) 국제 전화 번호 형식의 더하기 접두사에 문제가 있는 경우.
...
인코딩 문제에 대한 티켓을 열 때 공급자든 아니든 [Adobe 고객 지원 센터](https://helpx.adobe.com/kr/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html)는 항상 사용자가 입력하는 내용과 표시되는 내용의 16진수 버전을 포함합니다.
...
* 플랫폼에서 변경된 사항이나 조정을 포함합니다. 또한 공급자가 자신의 측에서 수행했을 수 있는 변경 사항을 포함합니다.
...
1. 다음에서 **외부 계정**, 확인 **로그 파일에서 자세한 SMPP 추적 활성화**.
