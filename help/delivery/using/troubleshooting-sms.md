...
`BIND PDU`에서 사용할 수 있는 필드(일반적으로 `address_range`에 짧은 코드를 추가하지만 공급자의 특별한 지원이 필요함)를 알려면 위의 [각 PDU 종류의 정보](sms-protocol.md#information-pdu) 섹션을 참조하십시오. 여러 단축 코드를 독립적으로 라우팅하는 방법을 알려면 해당 업체에 문의하십시오.
