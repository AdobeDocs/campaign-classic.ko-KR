...
* Adobe Campaign이 적시에 성공적인 `DELIVER_SM_RESP PDU`(으)로 회신하는지 확인하십시오. Adobe Campaign Classic에서는 SMS 프로세스에서 지연된 처리를 위해 SR이 `providerMsgId` 테이블에 삽입되도록 보장합니다.
...
Adobe Campaign Classic 확장 SMPP 커넥터가 `DELIVER_SM PDU`을(를) 확인했지만 broadLog가 제대로 업데이트되지 않은 경우 [MT, SR 및 broadlog 항목 일치](sms-protocol.md#matching-mt) 섹션에 설명된 ID 조정 프로세스를 확인하십시오.
...
네트워크 캡처가 항상 필요한 것은 아닙니다. 일반적으로 자세한 SMPP 메시지만 있으면 됩니다. 다음은 네트워크 캡처가 필요한지 여부를 결정하는 데 도움이 되는 몇 가지 지침입니다.
...
다른 모든 상황에서는 먼저 자세한 정보 SMPP 메시지를 분석하고 자세한 정보 로그에 정보가 누락된 경우에만 네트워크 캡처를 요청하십시오.
