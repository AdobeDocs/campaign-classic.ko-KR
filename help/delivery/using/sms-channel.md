...
>[!NOTE]
>
>또한 Adobe Campaign은 **Adobe Campaign 모바일 앱 채널(NMAC)** 옵션을 통해 모바일 터미널에 알림을 제출할 수도 있습니다.
> 
>자세한 내용은 [모바일 앱 채널](../../delivery/using/about-mobile-app-channel.md) 정보 섹션을 참조하십시오.

아래 섹션에서는 SMS 채널과 관련된 정보를 제공합니다. 배달을 만드는 방법에 대한 자세한 내용은 [이 섹션](../../delivery/using/steps-about-delivery-creation-steps.md)을 참조하십시오.
...
1. 계정 유형을 **라우팅**&#x200B;으로, 채널은 **모바일(SMS)**&#x200B;으로, 배달 모드는 **벌크 배달**&#x200B;으로 정의합니다.
...
1. **[!UICONTROL Mobile]** 탭의 **[!UICONTROL Connector]** 드롭다운 목록에서 **[!UICONTROL Extended generic SMPP]**&#x200B;을 선택합니다.
   ...
   >[!CAUTION]
   >
   > 릴리스 20.2부터는 레거시 커넥터가 더 이상 지원되지 않습니다. **[!UICONTROL Extended generic SMPP]** 커넥터를 사용하는 것이 좋습니다. 권장 커넥터로 마이그레이션하는 방법에 대한 자세한 내용은 이 [page](https://helpx.adobe.com/kr/campaign/kb/sms-connector.html)를 참조하십시오.
...
1. **[!UICONTROL Connection settings]** 탭에서 다른 외부 계정 필드를 완료하는 방법을 설명하는 SMS 서비스 공급업체에 문의하십시오.
...
1. **[!UICONTROL Throughput and delays]** 탭에서 초당 MT로 아웃바운드 메시지(&quot;MT&quot;, Mobile Terminated)의 최대 처리량을 지정할 수 있습니다. 해당 필드에 &quot;0&quot;을 입력하면 처리량이 무제한이 됩니다.
...
1. **[!UICONTROL SMSC specificities]** 탭에서 기본적으로 **[!UICONTROL Send full phone number]** 옵션이 비활성화됩니다. SMPP 프로토콜을 존중하고 SMSC(SMS 공급자)의 서버에 숫자만 전송하려는 경우 활성화하지 마십시오.
...
SMPP 모바일 전달 외부 계정의 **[!UICONTROL Mobile]** 탭에서 문자 변환 기능을 설정할 수 있습니다.

변환은 GSM 표준에서 고려하지 않는 SMS 문자를 다른 문자로 바꾸는 작업입니다.

* 음역 자격이 **[!UICONTROL authorized]**&#x200B;인 경우 메시지를 보낼 때 고려되지 않은 각 문자가 GSM 문자로 대체됩니다. 예를 들어 &quot;ë&quot;라는 글자는 &quot;e&quot;로 대체됩니다. 따라서 메시지는 약간 변경되지만 글자 수 제한은 그대로 유지됩니다.
* 음역설이 **[!UICONTROL not authorized]**&#x200B;이면, 계산되지 않은 문자가 포함된 각 메시지는 바이너리 형식(유니코드)으로 전송됩니다.따라서 모든 문자가 있는 그대로 전송됩니다. 그러나 유니코드를 사용하는 SMS 메시지는 70자(또는 여러 부분으로 나누어 보내는 SMS의 경우 67자)로 제한됩니다. 최대 글자 수를 초과하면 메시지가 여러 개로 보내져 추가 비용이 발생할 수 있습니다.
...
새 SMPP 모바일 배달 외부 계정을 구성할 때 **[!UICONTROL Mobile]** 탭에서 **[!UICONTROL Mapping of encodings]**&#x200B;을 정의할 수 있습니다.**[!UICONTROL data_coding]** 필드에서 Adobe Campaign은 SMSC에 사용되는 인코딩을 통신할 수 있습니다.
...
* 연결된 **[!UICONTROL data_coding]** 필드 값과 함께 사용할 인코딩을 정의하면 Facebook에서 목록의 첫 번째 인코딩을 사용하려고 하면 첫 번째 인코딩이 불가능할 경우 다음 인코딩이 사용됩니다.
...
**[!UICONTROL Remove from quarantine]** 동작을 자동 응답에 연결하면 해당 키워드를 전송하는 수신자는 자동으로 격리된 상태로 제거됩니다.

받는 사람은 **[!UICONTROL Administration]** > **[!UICONTROL Campaign Management]** > **[!UICONTROL Non deliverables Management]** 메뉴를 통해 사용할 수 있는 **[!UICONTROL Non deliverables and addresses]** 테이블에 나열됩니다.

* 짧은 코드가 무엇이든 동일한 응답을 보내려면 **[!UICONTROL Short code]** 열을 비워 두십시오.
* 키워드에 상관없이 동일한 응답을 보내려면 **[!UICONTROL Keyword]** 열을 비워 두십시오.
* 응답을 보내지 않고 작업을 수행하려면 **[!UICONTROL Response]** 열을 비워 두십시오. 예를 들어 &quot;STOP&quot; 이외의 메시지로 답글을 보낸 사용자를 격리하지 않아도 됩니다.
...
>[!NOTE]
>
>배달 만들기에 대한 글로벌 개념은 [이 섹션](../../delivery/using/steps-about-delivery-creation-steps.md)에서 제공됩니다.
...
1. 페이지 하단에 있는 **[!UICONTROL Preview]**&#x200B;을 클릭하여 개인화된 메시지와 함께 메시지 렌더링을 확인합니다. 미리 보기를 시작하려면 도구 모음에서 **[!UICONTROL Test personalization]** 단추를 사용하여 수신자를 선택합니다. 정의된 대상에서 수신자를 선택하거나 다른 수신자를 선택할 수 있습니다.
...
메시지를 승인하고 만드는 배달의 받는 사람에게 보내려면 **[!UICONTROL Send]**&#x200B;을 클릭합니다.
...
* **응용 프로그램 유형**:SMS 전달에 할당할 애플리케이션을 선택합니다. **[!UICONTROL Direct Marketing]** 옵션은 기본적으로 선택되어 있으며 가장 일반적으로 사용됩니다.
...
**nlserver sms** 모듈은 정기적으로 SMS 라우터를 쿼리합니다. 이를 통해 Adobe Campaign은 배송 진행 상황을 추적하고 상태 보고서와 수신자 구독 취소 요청을 처리할 수 있습니다.
...
* **구독 취소**:SMS 전달 수신을 중지하려는 수신자는 STOP이라는 단어가 포함된 메시지를 반환할 수 있습니다. 공급자가 계약 약관에 따라 메시지를 허용하는 경우, **인바운드 SMS** 워크플로우 활동을 통해 메시지를 검색한 다음 쿼리를 만들어 해당 수신자에게 더 이상 연락하지 않음&#x200B;**옵션을 활성화할 수 있습니다.**
...
>[!IMPORTANT]
>
>다음 세부 절차는 확장된 일반 SMPP 커넥터를 제외한 SMPP 커넥터에만 유효합니다. 자세한 내용은 [SMPP 외부 계정 만들기](#creating-an-smpp-external-account) 섹션을 참조하십시오.
>
...
1. **`<shortcode>`** 태그의 **name** 특성에 대해 메시지 보낸 사람 이름 대신 표시할 짧은 코드를 지정합니다.
   ...
   각 **`<reply>`** 태그에 키워드와 함께 **키워드** 속성을 입력하고 이 키워드에 대해 보내려는 메시지와 함께 **text** 속성을 입력합니다.
...
>
>이러한 메시지는 [상용 압력 규칙](../../campaign/using/pressure-rules.md)의 일부로 간주되지 않습니다.
