...
이메일이 실패하면 원격 서버는 플랫폼 매개 변수에 지정된 주소로 바운스 메시지를 반환합니다. Adobe Campaign은 각 바운스 메일의 콘텐츠를 규칙 목록의 문자열과 비교한 다음 세 가지 중 하나를 지정합니다 [오류 유형](#delivery-failure-types-and-reasons).
