...
* **[!UICONTROL Ignore]** :캠페인 MTA에서 바운스 메일을 무시합니다. 즉, 이 바운스로 인해 받는 사람 주소가 격리되지 않습니다. 전달 가능 **** 워크플로우를 위해 새로 고침에서 사용되지 않으며 클라이언트 인스턴스로 전송되지 않습니다.
...
이메일이 실패하면 원격 서버는 플랫폼 매개 변수에 지정된 주소로 바운스 메시지를 반환합니다. Adobe Campaign은 각 바운스 메일의 컨텐츠를 규칙 목록의 문자열에 대해 비교한 다음 세 가지 [오류 유형](#delivery-failure-types-and-reasons)중 하나를 할당합니다.

>[!NOTE]
>
>사용자는 자신의 규칙을 만들 수 있습니다. 패키지를 가져올 때 전달 가능 **** 워크플로우를 위해 새로 고침을 통해 데이터를 업데이트할 때 사용자가 만든 규칙을 덮어씁니다.
...
* SMTP **릴레이** 매개 변수를 사용하여 특정 도메인에 대한 IP 주소와 릴레이 서버 포트를 구성할 수 있습니다. 자세한 내용은 [이 섹션을](../../installation/using/configuring-campaign-server.md#smtp-relay)참조하십시오.
