...
워크플로우 **다이어그램의 도구 모음을 통해** 컨텐츠 관리 **[!UICONTROL Tools]** 활동에 액세스합니다.
...
* **전환으로 지정됨**
   ...
   사용할 컨텐츠는 이전에 만들어졌습니다. 프로세스는 들어오는 이벤트로 전파된 컨텐트 인스턴스에 대해 염려합니다. 컨텐츠 식별자는 이벤트의 &quot;contentId&quot; 변수를 통해 액세스합니다.
...
* **스크립트로 계산됨**
   ...
   JavaScript 템플릿을 기반으로 컨텐츠 인스턴스를 선택합니다. 평가할 코드를 사용하면 컨텐츠 식별자를 검색할 수 있습니다.
...
* **생성**
   ...
   &quot;파일&quot; 유형 게시가 있는 각 변형 템플릿의 출력 파일을 생성합니다. 나가는 전환이 생성된 각 파일에 대해 다음 매개 변수를 사용하여 활성화됩니다.&quot;contentId&quot; 변수에 저장된 컨텐츠의 식별자와 &quot;filename&quot; 변수의 파일 이름입니다.

### 전환 {#transition}

출력 전환 **생성** **[!UICONTROL Content management]** 옵션을 사용하면 활동에 출력 전환을 추가하여 새 활동을 워크플로우 실행에 연결할 수 있습니다. 이 옵션을 선택한 후 전환 레이블을 입력합니다.

## 예제 {#examples}

### 컨텐츠 제작 및 전달 자동화 {#automating-content-creation-and-delivery}

다음 예에서는 컨텐츠 블록의 작성 및 배달을 자동화합니다.

![](assets/d_ncs_content_workflow2.png)

컨텐츠는 &quot;컨텐트 관리&quot; 활동을 통해 구성됩니다.

![](assets/d_ncs_content_workflow3.png)

새 콘텐츠 인스턴스는 게시 모델 및 콘텐츠 문자열 폴더를 통해 만들어집니다.

우리의 예에서, 우리는 배달 주제에 과부하가 있었습니다. 템플릿에 입력된 것 대신 고려됩니다 **[!UICONTROL Delivery]** .
...
데이터 형식이 발행물 템플릿에 입력한 데이터 스키마(예&#x200B;**의 cus:book** )와 일치하지 않습니다.요소를 **`<section>`** 요소로 **`<chapter>`** 바꿔야 합니다. &quot;cus:book-workflow.xsl&quot; 스타일시트를 적용하여 필요한 변경을 수행해야 합니다.
...
활동의 최종 동작은 컨텐츠 인스턴스를 저장하고 다음 작업으로 진행하는 것입니다.

타깃팅은 **쿼리 활동을 통해** 수행됩니다.

AND **조인** 활동이 추가되어 대상 쿼리 및 컨텐츠 업데이트가 완료된 경우에만 배달이 시작되도록 했습니다.
...
활동의 배달 템플릿은 게시 템플릿의 변형 템플릿을 선택하는 데 사용됩니다. 컨텐츠 생성은 배달 템플릿 또는 활동과 동일한 템플릿으로 참조되는 템플릿 없이 모든 HTML 및 텍스트 템플릿을 고려합니다.

배달될 대상은 들어오는 이벤트를 통해 입력됩니다.

배달 컨텐츠는 들어오는 이벤트를 통해 채워집니다.

활동을 완료하기 위한 마지막 단계는 배달을 준비하고 시작하는 것입니다.

### 컨텐츠 제작 및 나중에 게시 {#creating-content-and-publishing-it-later}

이 예에서는 컨텐츠 블록을 만들고 특정 시간 지연 후에 파일 게시를 시작합니다.
...
마지막 작업은 게시 디렉토리를 강제 실행하여 이 컨텐츠를 생성하는 것입니다.
...
이 예에서는 첫 번째 예와 동일한 개념을 사용하므로 첫 번째 단계에서만 배달 작업을 생성합니다.

![](assets/d_ncs_content_workflow10.png)

첫 번째 **배달** 만들기 작업은 배달 작업을 만듭니다.

포크 활동을 사용하면 타겟 계산 및 컨텐츠 인스턴스 생성을 동시에 실행할 수 있습니다.

작업이 실행되면, AND-join 상자는 **배달** 작업을 활성화하여 이전에 컨텐츠 및 타깃팅에 생성된 배달을 시작합니다.
...
배달 컨텐츠가 FTP 또는 SFTP 서버에 있는 HTML 파일에서 사용 가능한 경우 이 컨텐츠를 Adobe Campaign 배달으로 쉽게 로드할 수 있습니다. Refer to [this example](../../workflow/using/loading-delivery-content.md).

### Amazon Simple Storage Service (S3) 커넥터에서 컨텐츠 가져오기 {#importing-content-from-amazon-simple-storage-service--s3--connector}

배달 컨텐츠가 Amazon Simple Storage Service (S3) 버킷에 있는 경우 이 컨텐츠를 Adobe Campaign 배달에 쉽게 로드할 수 있습니다. Refer to [this example](../../workflow/using/loading-delivery-content.md).
...
양식에서 **editBtn** 유형 **`<input>`** 필드를 선언하는 것이 목적입니다. 이 컨트롤은 편집 영역과 처리를 시작하는 단추로 구성됩니다.

편집 영역을 사용하면 검색할 데이터의 XML 피드의 URL을 구성하는 데 사용되는 변수 데이터를 채울 수 있습니다.

이 단추는 태그 아래에 채워진 **GetAndTransform** SOAP 메서드를 **`<input>`** 실행합니다.
...
GetAndTransform **** 메서드는 **`<enter>`** 태그의 **`<input>`** 요소 아래에 선언되어야 합니다. 이 태그는 동적으로 구성된 표현식에서 XML 데이터 복구 URL의 매개 변수로 사용됩니다. 함수의 두 번째 매개 변수는 선택 사항이며, 들어오는 XML 데이터가 내용과 같은 형식이 아닐 때 중간 변환에 사용되는 스타일시트를 참조합니다.
...
편집 영역을 사용하면 검색할 파일의 이름을 입력할 수 있습니다. URL은 다음과 같이 이 이름을 기반으로 구성됩니다.https://server/incomin/data.xml

검색할 데이터의 형식은 워크플로우 자동화 예 1과 동일합니다. 이 예제에서 볼 수 있는 &quot;cus:book-workflow.xsl&quot; 스타일시트를 사용합니다.
