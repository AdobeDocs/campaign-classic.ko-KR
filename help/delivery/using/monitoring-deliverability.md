...
* 컨텐츠 및 평판을 스캔하기 위해 주요 이메일 클라이언트에서 메시지를 미리 볼 수 있는 [받은 편지함 렌더링 보고서](../../delivery/using/inbox-rendering.md)입니다.
* 메시지 품질 개요(받은 편지함, 스팸).
...
* 일반적으로 [게재 대시보드](../../delivery/using/about-delivery-monitoring.md)는 다음 항목에 액세스할 수 있도록 합니다.
   * [게재 요약](../../delivery/using/delivery-dashboard.md#delivery-summary)을(를) 참조하십시오.
   * 제외된 타겟 및 그 이유를 보여주는 [게재 로그 및 기록](../../delivery/using/delivery-dashboard.md#delivery-logs-and-history);
...
* [다시 시도](../../delivery/using/understanding-delivery-failures.md#retries-after-a-delivery-temporary-failure)가 게재 템플릿에서 올바르게 설정되었는지(다시 시도 기간은 30분, 20번 이상 다시 시도) 확인합니다.
* [바운스](../../delivery/using/understanding-delivery-failures.md#bounce-mail-management) 사서함이 액세스할 수 있고 계정이 만료되지 않았는지 정기적으로 확인합니다.
* [게재 대시보드](../../delivery/using/delivery-dashboard.md)에서 액세스할 수 있는 각 게재 처리량을 확인하여 게재 컨텐츠의 유효성(예:&#39;플래시 판매&#39;는 며칠이 아니라 몇 분 안에 배달되어야 합니다.
* [waves](../../delivery/using/steps-sending-the-delivery.md#sending-using-multiple-waves)를 사용할 때 다음 웨이브가 트리거되기 전에 각 웨이브를 완료할 시간이 충분한지 확인합니다.
* 오류 수 및 새 [격리](../../delivery/using/understanding-quarantine-management.md)가 다른 게재와 일치하는지 확인합니다.
* 강조 표시된 오류 유형(차단 목록, DNS 문제, 스팸 방지 규칙 등)을 확인하려면 [게재 로그](../../delivery/using/delivery-dashboard.md#delivery-logs-and-history)를 자세히 참조하십시오.
