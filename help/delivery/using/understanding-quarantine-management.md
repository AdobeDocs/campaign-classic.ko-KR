...
Adobe Campaign은 격리된 주소 목록을 관리합니다. 주소가 격리된 수신자는 배달 분석 중에 기본적으로 제외되며 타깃팅되지 않습니다. 예를 들어, 사서함이 가득 찼거나 주소가 없는 경우 이메일 주소를 격리할 수 있습니다. 어떠한 경우든, 격리 절차는 아래에 설명된 특정 규칙을 준수합니다.
...
게다가, 검역원은 잘못된 전화 번호를 배달에서 제외함으로써 SMS를 보내는 비용을 줄이는 데 도움을 주었다. 배달 보안 및 최적화를 위한 우수 사례에 대한 자세한 내용은 [이 페이지를 참조하십시오](https://docs.campaign.adobe.com/doc/AC/getting_started/EN/deliveryBestPractices.html).
...
반면에 **블록 목록에**&#x200B;있으면 더 이상 프로필이 전달의 대상이 되지 않습니다(예: 가입 해지).

>[!NOTE]
>
>사용자가 SMS 전달에서 옵트아웃하기 위해 &quot;STOP&quot;과 같은 키워드로 SMS 메시지에 댓글을 달면 이메일 옵트아웃 과정과 같이 차단 목록에 프로필이 추가되지 않습니다. 프로필 전화 번호는 격리되도록 보내져 사용자가 계속 이메일 메시지를 수신하게 됩니다.
...
배달 준비 단계 동안 지정된 배달의 격리된 주소가 배달 대시보드의 배달 로그에 나열됩니다( [배달 로그 및 내역](../../delivery/using/monitoring-a-delivery.md#delivery-logs-and-history)참조).

### 전체 플랫폼의 격리된 주소 식별 {#identifying-quarantined-addresses-for-the-entire-platform}

관리자는 노드의 전체 플랫폼에 대해 격리된 주소를 나열할 수 **[!UICONTROL Administration > Campaign Management > Non deliverables Management > Non deliverables and addresses]** 있습니다.
...
플랫폼의 모든 배달(**홈 페이지>보고서**)이나 특정 배달에 대해 이 정보를 조회할 수 있습니다. 사용자 지정된 보고서를 만들고 표시할 정보를 선택할 수도 있습니다.

### 받는 사람의 격리된 주소 식별 {#identifying-quarantined-addresses-for-a-recipient}
...
격리 시 주소를 제거해야 하는 경우 상태를 수동으로 로 변경하십시오 **[!UICONTROL Valid]**.

![](assets/tech_quarant_error_status.png)

상태를 로 변경하는 경우 오류가 발생하더라도 매번 주소가 체계적으로 타깃팅됩니다 **[!UICONTROL On allow list]**.

>[!CAUTION]
차단 목록의 주소는 격리 시스템에 영향을 받지 않으며, 주소 상태를 변경해도 타깃팅되지 않습니다.

오류 수와 오류 사이의 기간을 변경할 수도 있습니다. 이렇게 하려면 배포 마법사의 설정을 변경합니다(이메일 채널/고급 설정). 배포 마법사에 대한 자세한 내용은 [이 섹션을 참조하십시오](../../installation/using/deploying-an-instance.md).

## 격리에 주소 보내기 조건 {#conditions-for-sending-an-address-to-quarantine}

Adobe Campaign은 배달 실패 유형 및 오류 메시지 자격 조건( [바운스 메일 자격](../../delivery/using/understanding-delivery-failures.md#bounce-mail-qualification)참조) 및 배달 실패 유형 및 이유에 따라 [격리를 관리합니다](../../delivery/using/understanding-delivery-failures.md#delivery-failure-types-and-reasons).

* **무시된 오류**: 무시된 오류는 격리된 주소를 보내지 않습니다.
* **오류**: 해당 이메일 주소는 즉시 격리하도록 전송됩니다.
* **소프트 오류**: 소프트 오류는 즉시 격리된 주소를 보내지 않지만 오류 카운터를 증가시킵니다. 자세한 내용은 [소프트 오류 관리를 참조하십시오](#soft-error-management).

사용자가 이메일을 스팸(**피드백 루프**)으로 자격이 되는 경우, 메시지는 Adobe에서 관리하는 기술 사서함으로 자동 리디렉션됩니다. 그러면 사용자의 이메일 주소가 자동으로 격리되도록 전송됩니다.

격리된 주소 목록에서 선택한 주소가 격리된 **[!UICONTROL Error reason]** 이유를 나타내는 필드가 표시됩니다. Adobe Campaign에서의 검역은 대/소문자를 구분합니다. 이메일 주소가 나중에 다시 타깃팅되지 않도록 이메일 주소를 낮은 대소문자 로 가져와야 합니다.
...
* 기본 구성에서 임계값은 5개의 오류로 설정되며, 최소 24시간 간격으로 발생하는 경우 두 개의 오류가 중요합니다. 다섯 번째 오류 발생 시 주소가 격리되어 있다.
* 오류 카운터 임계값을 수정할 수 있습니다. 이 방법에 대한 자세한 내용은 배달 임시 실패 후 [재시도를 참조하십시오](../../delivery/using/understanding-delivery-failures.md#retries-after-a-delivery-temporary-failure).
...
Adobe Campaign은 Baidu 서버에 10분마다 연결하여 보낸 메시지의 상태를 검색하고 브로드로그를 업데이트합니다. 메시지가 전송됨으로 선언되면 브로드로그에 있는 메시지 상태가 로 설정됩니다 **[!UICONTROL Received]**. Baidu가 오류를 선언하면 상태가 로 설정됩니다 **[!UICONTROL Failed]**.

**Android V2용**

Android V2 격리 메커니즘은 Android V1과 동일한 프로세스를 사용하며, 구독 및 제외 업데이트에 대해서도 마찬가지입니다. 자세한 내용은 [Android V1](#android-quarantine) 섹션을 참조하십시오.
...
>[!NOTE]
실패 유형 및 실패 이유는 이메일과 동일합니다. 배달 [실패 유형 및 이유를 참조하십시오](../../delivery/using/understanding-delivery-failures.md#delivery-failure-types-and-reasons).
...
* 오류 메시지의 두 번째 부분(**이 예에서 일반** )은 SMS 외부 계정 필드에 정의된 것과 같은 SMSC 구현의 **[!UICONTROL SMSC implementation name]** 이름을 나타냅니다. 이 페이지 [를 참조하십시오](../../delivery/using/sms-channel.md#creating-an-smpp-external-account).
...
* 오류 메시지의 세 번째 부분(**이** 예제의 DELIVRD)은 SMS 외부 계정에 정의된 상태 추출 규칙을 사용하여 SR에서 검색한 상태 코드에 해당합니다.
   ...
   이 정규식은 외부 계정의 **[!UICONTROL SMSC specificities]** 탭에 지정됩니다. 이 페이지 [를 참조하십시오](../../delivery/using/sms-channel.md#creating-an-smpp-external-account).
