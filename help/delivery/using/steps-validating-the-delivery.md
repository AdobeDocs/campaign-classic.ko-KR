...
1. **게재 분석**: 이 단계에서는 전달할 메시지를 준비할 수 있습니다. [자세히 알아보기](#analyzing-the-delivery)
   ...
   분석 중에 적용된 규칙은 [이 섹션](#validation-process-with-typologies). 사용 가능한 유효성 검사 모드는 [승인 모드 변경](#changing-the-approval-mode) 섹션을 참조하십시오.

1. **증명 보내기**: 이 단계에서는 컨텐츠, URL, 개인화 등을 제어할 수 있습니다. 추가 정보 [증명 보내기](steps-validating-the-delivery.md#sending-a-proof) 및 [특정 증명 대상 정의](steps-defining-the-target-population.md#defining-a-specific-proof-target).
...
1. 게재 분석을 시작하려면 **[!UICONTROL Send]**.
1. **[!UICONTROL Deliver as soon as possible]**&#x200B;을(를) 선택합니다.

   ![](assets/s_ncs_user_email_del_send.png)

1. 클릭 **[!UICONTROL Analyze]** 분석을 수동으로 시작하려면 다음을 수행하십시오.
   ...
   >[!NOTE]
   >
   >분석 중에 사용되는 검증 규칙은 [유형화를 사용한 유효성 검사 프로세스](steps-validating-the-delivery.md#validation-process-with-typologies) 섹션을 참조하십시오.

1. 를 클릭하면 언제든지 분석을 중지할 수 있습니다 **[!UICONTROL Stop]**.
...
1. 클릭 **[!UICONTROL Close]** 오류가 있을 경우 오류를 수정했습니다.

1. 변경한 후 분석을 다시 시작한 다음 **[!UICONTROL Analyze]**.

분석 결과를 확인한 후 **[!UICONTROL Confirm delivery]** 메시지를 지정된 타겟에게 보내려면 확인 메시지를 통해 게재를 시작할 수 있습니다.

![](assets/s_ncs_user_email_del_analyze_ok.png)

>[!NOTE]
>
>을(를) 클릭합니다. **[!UICONTROL Change the main delivery target]** 전송할 메시지 수가 구성과 일치하지 않는 경우 연결합니다. 이를 통해 대상 모집단의 정의를 변경하고 분석을 다시 시작할 수 있습니다.

### 분석 설정 {#analysis-parameters}

다음 **[!UICONTROL Analysis]** 게재 속성의 탭에서는 분석 단계 동안 메시지 준비에 대한 정보 세트를 정의할 수 있습니다.
...
* **[!UICONTROL Approval mode]** : 이 필드를 사용하면 분석이 완료되면 수동 또는 자동 배달을 정의할 수 있습니다. 유효성 검사 모드는 [승인 모드 변경](#changing-the-approval-mode) 섹션을 참조하십시오.
...
* **[!UICONTROL Prepare the personalization data with a workflow]** : 이 옵션을 사용하면 자동 워크플로우에서 게재에 포함된 개인화 데이터를 준비할 수 있으므로 개인화 실행을 위한 상당한 성능 향상을 달성할 수 있습니다. 자세한 내용은 [개인화 최적화](personalization-fields.md#optimizing-personalization).
...
* **[!UICONTROL Ignore personalization scripts during sending]** : 이 옵션을 사용하면 HTML 컨텐츠에 있는 JavaScript 지시문의 해석을 무시할 수 있습니다. 전달된 컨텐츠에 그대로 표시됩니다. 이러한 지시문은 **&lt;%=** 태그 내에만 게시합니다.

### 게재 분석 성능 개선 {#improving-delivery-analysis}

게재 준비 속도를 높이기 위해 다음을 확인할 수 있습니다. **[!UICONTROL Prepare the delivery parts in the database]** 옵션을 선택한 후에 분석을 시작합니다.
...
* 중간 소싱 또는 외부 공정순서를 사용해서는 안 되며 대량 납품 공정순서 유형만 사용할 수 있습니다. 에서 사용되는 공정순서를 확인할 수 있습니다 **[!UICONTROL General]** 의 탭 **[!UICONTROL Delivery properties]**.
* 외부 파일에서 만든 모집단은 타겟팅할 수 없습니다. 단일 게재의 경우 **[!UICONTROL To]** 링크 위치 **[!UICONTROL Email parameters]** 그리고 그것을 확인합니다 **[!UICONTROL Defined in the database]** 옵션이 선택되어 있습니다. 워크플로우에 사용된 게재의 경우 수신자가 **[!UICONTROL Specified by the inbound event(s)]** 에서 **[!UICONTROL Delivery]** 탭.
...
옵션 **[!UICONTROL Message preparation by the scheduler]** campaign 워크플로우에서 게재 분석에 우선 순위를 지정할 수 있습니다.
...
>[!NOTE]
>
>더 큰 게재 분석으로 워크플로우의 진행 속도가 느려지지 않도록 하려면, **[!UICONTROL Schedule execution for a time of low activity]**.
...
1. 증명 타겟이 [특정 증명 대상 정의](steps-defining-the-target-population.md#defining-a-specific-proof-target).
1. 클릭 **[!UICONTROL Send a proof]** 을 클릭합니다.

   ![](assets/s_ncs_user_email_del_send_proof.png)

1. 메시지 분석을 시작합니다. 자세한 내용은 [게재 분석](steps-validating-the-delivery.md#analyzing-the-delivery).
1. 이제 게재를 보낼 수 있습니다( [게재 보내기](steps-sending-the-delivery.md)).
...
게재를 분석하면 을(를) 통해 보낸 다양한 증명을 볼 수 있습니다 **[!UICONTROL Proofs]** 로그의 하위 탭(**[!UICONTROL Audit]** 탭).

![](assets/s_ncs_user_delivery_validation_cycle_03.png)

게재의 컨텐츠가 완료될 때까지 필요한 만큼 증명을 보내야 합니다. 그런 다음 게재를 기본 타겟으로 보내고 유효성 검사 주기를 닫을 수 있습니다.

다음 **[!UICONTROL Advanced]** 게재 속성의 탭에서는 증명의 속성을 정의할 수 있습니다. 필요한 경우 수신자 제외 규칙을 무시할 수 있습니다.
...
* 다음 두 옵션을 모두 사용하면 중인 수신자와 주소를 격리할 차단 목록 수 있습니다. 에서 기본 대상에 대해 이러한 옵션에 대한 설명을 참조하십시오. [제외 설정 사용자 지정](steps-defining-the-target-population.md#customizing-exclusion-settings). 이러한 주소는 기본적으로 제외되는 게재 타겟과 달리, 증명 대상에 대해서는 기본적으로 유지됩니다.
* 다음 **[!UICONTROL Keep the delivery code for the proof]** 옵션을 사용하면 증명 표시와 관련된 게재에 대해 정의된 배달 코드와 동일한 배달 코드를 제공할 수 있습니다. 이 코드는 게재 마법사의 첫 번째 단계에서 지정됩니다.
* 기본적으로 증명의 제목에는 &#39;Proof #&#39; 접두사가 붙습니다. 여기서 #은 증명의 번호입니다. 이 접두사는 **[!UICONTROL Label prefix]** 필드.

## 유형화를 사용한 유효성 검사 프로세스 {#validation-process-with-typologies}

메시지를 보내기 전에 캠페인을 분석하여 해당 콘텐츠와 구성을 승인해야 합니다. 분석 단계 동안 적용된 확인 규칙은 **유형화**. 기본적으로 이메일의 경우 분석은 다음 사항을 다룹니다.
...
각 게재에 적용할 유형화는 **[!UICONTROL Typologies]** 탭에서 다음 매개 변수를 사용할 수 있습니다.

을 통해 승인 규칙, 해당 컨텐츠, 실행 순서 및 전체 설명을 보고 편집할 수 있습니다. **[!UICONTROL Administration > Campaign execution > Typology management > Typology rules]** 노드 아래에 있어야 합니다.
...
현재 유형화를 편집하려면 **[!UICONTROL Edit link]** 아이콘 을 클릭합니다. **[!UICONTROL Typology]** 필드.

![](assets/s_ncs_user_email_del_typo_tab.png)

다음 **[!UICONTROL Rule]** 탭에는 적용할 유형화 규칙 목록이 있습니다. 규칙을 선택하고 **[!UICONTROL Detail...]** 아이콘을 사용하여 해당 구성을 확인합니다.
