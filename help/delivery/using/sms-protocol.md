...
>[!NOTE]
>
>**SMS 커넥터 프로토콜 및 Adobe Campaign Standard용 설정**&#x200B;은 이 [페이지](https://experienceleague.adobe.com/docs/campaign-standard/using/administrating/configuring-sms/sms-protocol.html#administrating)에서 찾을 수 있습니다.
>
...
이 문서에서는 Adobe Campaign과 SMPP 공급자 간에 설정된 연결을 안내합니다.
...
* 140바이트로 제한된 텍스트로서 인코딩에 따라 70-160자 사이의 텍스트입니다. 자세한 내용 및 제한 사항은 아래의 [SMS 텍스트 인코딩](../../delivery/using/sms-protocol.md#sms-text-encoding)을 참조하십시오.
...
Adobe Campaign Classic은 SMPP 프로토콜 버전 3.4를 지원합니다. 이는 SMSC(제공자)에 SMS를 전송하고 SMS와 영수증을 수신할 수 있도록 하는 광범위한 프로토콜입니다. 자세한 내용은 [SMPP 설명서](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)를 참조하십시오.
...
* **트랜시버(또는 TRX)**:단일 TCP 연결은 메시지를 전송하고 받는 데 사용됩니다.

>[!NOTE]
>
> Adobe Campaign Classic은 TX+RX 모드만 지원합니다. 이 제한은 기술 아키텍처 때문입니다.
...
**BIND_TRANSMITTER / BIND_RECEIVER / BIND_TRISVER**
...
이 PDU는 연결을 끊기 전에 시스템에 의해 전송되어야 합니다. 연결을 닫기 전에 일치하는 `UNBIND_RESP` PDU를 기다려야 합니다.
...
* **data_coding**:텍스트 필드에 사용된 인코딩을 나타냅니다. 자세한 내용은 [SMS 텍스트 인코딩](../../delivery/using/sms-protocol.md#sms-text-encoding) 섹션을 참조하십시오.
...
Adobe Campaign은 일부 구성의 `receipted_message_id` 선택 필드에서 메시지 ID를 읽을 수 있습니다.

#### DELIVER_SM_RESP {#deliver-sm-resp}

이 PDU는 Adobe Campaign이 SR 및 MO를 확인하도록 전송됩니다.

Adobe Campaign Classic은 SR 및 MO가 데이터베이스에 삽입되면 이를 확인합니다. 성공한 `DELIVER_SM_RESP` PDU를 보냈더라도 일부 처리 오류가 발생할 수 있습니다. 이 제한은 Adobe Campaign Classic의 소프트웨어 아키텍처로 인해 발생합니다.
...
여러 부분으로 이루어진 SMS 또는 긴 SMS는 여러 부분으로 보내지는 SMS입니다. 모바일 네트워크 프로토콜의 기술적 제한으로 인해 SMS는 140바이트보다 크지 않거나 분할해야 합니다. SMS에 맞는 문자 수에 대한 자세한 내용은 [SMS 텍스트 인코딩](../../delivery/using/sms-protocol.md#sms-text-encoding) 섹션을 참조하십시오.
...
대부분의 제공업체는 각 SMPP 연결에 대해 처리량 제한을 요구합니다. 외부 계정에 SMS 수를 설정하여 이를 수행할 수 있습니다. 처리량 조절은 연결당 발생하며, 총 유효 처리량은 연결당 제한과 총 연결 수를 곱한 것입니다. 이것은 [동시 연결](../../delivery/using/sms-protocol.md#connection-settings) 섹션에 자세히 설명되어 있습니다.

가능한 처리량을 최대화하려면 최대 전송 창을 미세 조정해야 합니다. 전송 창은 `SUBMIT_SM_RESP`을(를) 기다리지 않고 보낼 수 있는 `SUBMIT_SM PDU`의 수입니다. 자세한 내용은 [보내기 창 설정](../../delivery/using/sms-protocol.md#throughput-timeouts) 섹션을 참조하십시오.
...
Adobe Campaign Classic 커넥터(확장 SMPP 제외)에서는 선택한 공급자에 따라 달라지는 하드 코딩된 동작을 사용합니다. 일반 SMPP는 세부 사항 없이 성공 및 오류만 구분합니다. 배달 로그에는 보장되지 않는 일부 정보가 포함될 수 있습니다.
...
### Adobe Campaign Classic Extended 일반 SMPP {#sr-processing}의 SR 처리 예
...
인코딩 문제가 발생하면 **항상 SMSC 공급자에게 문의하십시오**. SMSC 제공업체만이 지원하는 인코딩과 기술 플랫폼의 한계로 인해 적용될 수 있는 특수 규칙에 대해 알고 있습니다.
...
* 인코딩에 속한 문자를 알고 있어야 합니다. GSM7에서는 발음 부호 또는 억양을 완벽하게 지원하지 않습니다. 특히 프랑스어로, 엑시와 프랑스가 GSM7의 일부지만, 혹은 그렇지 않다. 스페인어도 마찬가지다.
...
이 옵션을 사용하면 연결 수를 보다 정밀하게 제어할 수 있습니다. [동시 연결](../../delivery/using/sms-protocol.md#connection-settings)을 참조하십시오.
...
SMSC 구현의 이름을 설정합니다. 공급자 이름으로 설정해야 합니다. 이 필드에 추가할 내용을 알려면 관리자 또는 제공 팀에 문의하십시오. 이 필드의 역할은 [SR 오류 관리](../../delivery/using/sms-protocol.md#sr-error-management) 섹션에 설명되어 있습니다.
...
#### MTA 자식 연결 수 {#number-mta-child}

Adobe Campaign Classic에서는 MTA 자식에 대한 연결 수를 정의합니다.

Adobe Campaign Classic Extended SMPP 커넥터는 MTA 자식당 연결 수를 제어할 수 있습니다. 연결의 글로벌 제한을 제어하려면 MTA 하위 프로세스의 수를 제한해야 합니다. 이 프로세스는 종종 SMS용 전용 중간 소싱 플랫폼을 의미합니다.

Adobe Campaign Classic의 경우 다음과 같이 서로 다른 수의 수신기와 전송기 연결이 있을 수 있습니다.

* **전송자 연결 = MTA 하위 연결 수 * MTA 하위 프로세스 수 * MTA 수(자동 회신이 설정된 경우) + MTA 하위 연결 수**

위에서 설명한 것처럼 자동 회신이 활성화된 경우 Adobe Campaign Classic SMS 프로세스에서 더 많은 전송기 연결을 엽니다. 이러한 추가 연결은 자동 답글을 보내는 데 사용됩니다.

* **받는 사람 연결 = MTA 자식 연결 수**

자동 회신을 설정하면 SMS 프로세스에서 송신기/수신기 쌍을 열어 송신기 연결 수가 증가합니다. 자동 응답을 설정하지 않은 경우 수신자 연결만 열립니다.
...
Adobe Campaign Classic에서 로그 출력은 MT 관련 트래픽에 대한 MTA 로그와 MO 및 SR 관련 트래픽에 대한 SMS 로그에 있습니다.
...
이러한 설정은 송신기+수신기 모드에서 수신기에 적용됩니다. 그것들은 송신기 부분처럼 작동하며, 더 자세한 내용은 상기를 참조하십시오.
...
인코딩 프로세스에 대한 보다 일반적인 설명은 [인코딩 설정](../../delivery/using/sms-protocol.md#SMSC-specifics)의 특정 매핑 정의를 참조하십시오.
...
이 확인란을 선택하면 긴 SMS가 하나의 SUBMIT_SM PDU에 전송되어 message_payload 옵션 필드에 텍스트를 입력합니다. 자세한 내용은 [SMPP 사양](../../delivery/using/sms-protocol.md#ACS-SMPP-connector)을 참조하십시오.
...
SR 형식은 SMPP 프로토콜 사양에 의해 엄격하게 적용되지 않습니다. 이것은 사양 [부록 B](../../delivery/using/sms-protocol.md#sr-error-management)(167페이지)에 설명된 권장 사항일 뿐입니다. 일부 SMPP 구현자는 이 필드의 형식을 다르게 지정하므로 Adobe Campaign에서 올바른 필드를 추출하는 방법이 필요합니다.
...
기본적으로 `DELIV`로 시작하는 시작 값(예:[부록 B](../../delivery/using/sms-protocol.md#sr-error-management)의 `DELIVRD`은 성공적으로 배달된 것으로 간주되며 오류가 일치하는 모든 통계 값(예: )으로 간주됩니다.`REJECTED`, `UNDELIV`는 오류로 간주됩니다.
...
>[!IMPORTANT]
>
>Adobe Campaign Classic 및 하이브리드 아키텍처에서 확장 SMPP 커넥터에 대해 자동 응답을 적용하려면 **외부 계정** 폴더에 있는 mid 연산자에 대한 쓰기 액세스를 추가해야 합니다.
...
이 설정은 **메시지 페이로드** 설정이 비활성화된 경우에만 작동합니다. 메시지에 이 값보다 더 많은 SMS가 필요한 경우 오류가 트리거됩니다.
...
배달 부품을 전송할 때 MTA는 MTA 하위 항목을 생성합니다. MTA 하위 프로세스 수는 동적이며 serverConf.xml의 구성에 따라 달라집니다. 각 MTA 자식은 SMPP 공급자에 연결하는 커넥터 `CSmppConnectorWorker`을 인스턴스화합니다. MTA 하위 항목이 살아 있는 한 연결은 활성 상태로 유지되며 serverConf.xml에서도 구성할 수 있습니다.

SMS 프로세스는 SR만 처리하며, 제공자와 연결되고 연결이 열린 상태로 유지됩니다. 프로세스는 10분마다 다시 연결하여 새 설정을 다시 불러옵니다. 정상적인 작업입니다.

### MT, SR 및 방송 로그 항목 일치 {#matching-mt}

중간 테이블 `nmsProviderMsgId`은 브로드로그에 비동기식으로 커밋되기 전에 MT 및 SR 데이터를 일시적으로 저장하는 데 사용됩니다.

`nmsProviderMsgId` 표에는 3개의 열 그룹이 있습니다.

* MT가 전송되고 확인될 때 업데이트된 열:`iBroadLogId`, `iDeliveryId`

* SR이 수신될 때 업데이트된 열:`iMsgId`, `iStatus`

* 항상 업데이트되는 열:`tsCreated`, `sProviderId`

MT와 SR 모두 처리가 끝나면, 광대역 정보와 상태 정보가 모두 포함된 완료 라인이 있어야 합니다.

여기서 `iMsgId`은(는) 전체 상태/오류 메시지를 나타내는 `nmsBroadLogMsg` 테이블에 연결되어 있습니다.

SMS 프로세스는 매 분마다 전체 줄을 확인한 다음 비동기적으로 처리합니다.

* 전체 줄을 읽습니다.
* SMS 프로세스는 배달 매핑을 기반으로 브로드로그 테이블의 이름을 계산합니다.
* SMS 프로세스는 메시지 ID 및 상태로 브로드로그 테이블을 업데이트합니다.

**처리량 및 병렬 연결**

각 MTA 하위는 구성 가능한 연결 양을 생성하므로 MTA 하위 항목의 수를 제한하면 연결 수가 제한됩니다. MTA 하위 프로세스와 트래픽 간의 상관관계가 있으므로 다소 제어될 수 있지만 다소 예측할 수 없습니다.
...
`BIND_* PDU`이(가) 너무 많지 않은지 확인하십시오.너무 많은 경우 연결이 불안정함을 나타낼 수 있습니다. 자세한 내용은 [불안정한 연결 문제](../../delivery/using/sms-protocol.md#issues-unstable-connection) 섹션을 참조하십시오.
