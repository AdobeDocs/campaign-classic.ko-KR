...
Adobe Campaign Classic은 SMPP 프로토콜 버전 3.4를 지원합니다. SMSC(공급자)로 SMS를 전송하고 SMS와 영수증을 받을 수 있는 광범위한 프로토콜입니다. 자세한 내용은 [SMPP 설명서](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)를 참조하십시오.
...
Adobe Campaign Classic에서 SR을 해당 MT와 연결하려면 SMSC가 `SUBMIT_SM_RESP` 및 `DELIVER_SM` 단계를 사용하여 ID를 반환합니다. 식별자는 `nms::providerMsgId` 테이블의 `providerId` 필드에 저장되며 `broadLogId` 및 `deliveryId`에 연결됩니다. 이 일치 작업은 데이터베이스에 쓸 때 SMS 프로세스에서 수행합니다.
...
이 PDU는 연결을 끊기 전에 시스템에서 전송해야 합니다. 연결을 닫기 전에 일치하는 `UNBIND_RESP` PDU를 기다려야 합니다.
...
Adobe Campaign Classic은 SR 및 MO가 데이터베이스에 삽입되면 이를 승인합니다. 성공한 `DELIVER_SM_RESP` PDU를 보낸 경우에도 일부 처리 오류가 발생할 수 있습니다. 이 제한은 Adobe Campaign Classic의 소프트웨어 아키텍처로 인해 발생합니다.
...
[SMPP 프로토콜 사양](https://smpp.org/SMPP_v3_4_Issue1_2.pdf)(167페이지)의 부록 B 섹션에 권장 사항이 있지만, 실제 오류 코드나 그 의미가 나열되지는 않습니다.
...
인코딩 문제가 발생하면 **항상 SMSC 공급자에게 문의해야 합니다**. SMSC 공급자만 지원하는 인코딩과 기술 플랫폼의 제한 사항으로 인해 적용될 수 있는 특수 규칙에 대해 알고 있습니다.
...
이 창은 네트워크 링크가 지연 시간이 높을 때 처리량을 늘리는 데 도움이 됩니다.  창 값은 SMS 수와 링크 지연을 곱한 값 이상이어야 합니다
초 단위 커넥터는 다음 메시지를 보내기 전에 `SUBMIT_SM_RESP`을 기다리지 않습니다.
...
>[!IMPORTANT]
>
>Adobe Campaign Classic 및 하이브리드 아키텍처에서 확장 SMPP 커넥터에 대한 자동 답장을 적용하려면 **외부 계정** 폴더에서 mid 연산자에 대한 쓰기 액세스를 추가해야 합니다.
...
이 설정은 **메시지 페이로드** 설정이 비활성화된 경우에만 작동합니다. 메시지에 이 값보다 SMS가 더 많은 경우 오류가 트리거됩니다.
...
유효 기간은 `SUBMIT_SM PDU` 의 `validity_period` 필드에 전송됩니다. 날짜 형식은 항상 절대 UTC 시간 형식으로 지정되며 날짜 필드는 &quot;00+&quot;로 끝납니다.
...
게재 부품을 전송할 때 MTA에서 MTA 하위 항목을 생성합니다. MTA 하위 프로세스 수는 동적이며 serverConf.xml의 구성에 따라 달라집니다. 각 MTA 자식은 SMPP 공급자에 연결하는 커넥터 `CSmppConnectorWorker`을 인스턴스화합니다. MTA 하위가 살아남아 있는 한 연결은 그대로 유지되며 serverConf.xml에서도 구성할 수 있습니다.
...
중간 테이블 `nmsProviderMsgId`은 브로드로그에 비동기식으로 커밋되기 전에 일시적으로 MT 및 SR 데이터를 저장하는 데 사용됩니다.
...
여기서 `iMsgId`은(는) `nmsBroadLogMsg` 테이블에 연결되어 있으며, 전체 상태/오류 메시지를 나타냅니다.
