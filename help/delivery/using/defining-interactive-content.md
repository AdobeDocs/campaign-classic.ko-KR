...
Adobe Campaign을 사용하면 특정 조건에서 동적 이메일을 보낼 수 있는 새로운 대화형 [이메일](https://amp.dev/about/email/) 형식(AMP for Email)을 사용할 수 있습니다.

>[!IMPORTANT]
>
>* 이 기능은 Adobe Campaign의 베타 기능입니다.
>* AMP for Email은 개발자가 다이내믹하고 인터랙티브한 이메일을 만들 수 있는 새로운 오픈 소스 포맷입니다. 현재 일부 이메일 제공업체에서 지원합니다. Gmail, Outlook 및 Mail.ru.
...
## Adobe Campaign이 있는 이메일에 AMP를 사용하는 주요 단계 {#key-steps-to-use-amp}

Adobe Campaign이 포함된 AMP 이메일을 성공적으로 테스트하고 전송하려면 아래 단계를 따르십시오.
...
1. Adobe Campaign에서 이메일을 만들고 AMP 콘텐츠를 빌드합니다. Adobe Campaign [를 사용하여 AMP 이메일 콘텐츠 제작을 참조하십시오](#build-amp-email-content).
...
1. 요구 사항에 맞게 AMP 컨텐츠를 편집할 수 있습니다.
   ...
   예를 들어 AMP 템플릿의 제품 목록 구성 요소를 사용하고 타사 시스템 또는 Adobe Campaign의 제품 목록을 유지할 수 있습니다. 가격이나 다른 요소를 조정할 때마다 받는 사람이 사서함에서 이메일을 다시 열면 자동으로 반영됩니다.
...
Adobe Campaign에서 AMP 컨텐츠를 작성할 때는 수신자의 이메일 공급자에 따라 제공되는 동적 이메일의 조건을 준수해야 합니다.
...
1. Adobe Campaign을 사용하면 컨텐츠와 동작을 확인하기 위해 AMP 기반 동적 이메일을 선택한 이메일 주소로 적절하게 구성된 것을 테스트할 수 있습니다. 선택한 [주소에 대한 AMP 이메일 배달 테스트를 참조하십시오](#testing-amp-delivery-for-selected-addresses).
1. 테스트를 거친 후에는 관련 이메일 공급자에 등록하여 수신 도메인을 허용 목록에 추가하도록 함으로써 이메일 베타 프로그램의 일부로 배달 또는 캠페인을 보낼 수 있습니다. 이메일 [공급자에 등록하여 AMP 이메일 전달을 참조하십시오](#delivering-amp-emails-by-registering).
...
Gmail 및 Outlook의 경우, 먼저 대상 Gmail 및 Outlook 계정의 Adobe Campaign에서 배달할 허용 목록에 사용 중인 보낸 사람 주소를 추가해야 합니다.
...
AMP 베타 프로그램에 참여하는 이메일 제공자에 등록하여 보낸 사람 도메인을 허용 목록에 추가하도록 함으로써 동적 이메일 제공을 실험해 볼 수 있습니다.
