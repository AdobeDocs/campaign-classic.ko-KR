...
## 몇 개의 컨트롤 인스턴스 지원 {#supporting-several-control-instances}
...
>[!NOTE]
>
>필요한 구성에 대한 자세한 내용은 [여러 제어 인스턴스 사용](../../message-center/using/creating-a-shared-connection.md#using-several-control-instances)을 참조하십시오.
...
여러 채널을 사용해야 하는 경우 트랜잭션 메시지 패키지를 설치하기 전에 관련 패키지를 설치하고 구성해야 합니다. [배달 채널 추가](#adding-a-delivery-channel)를 참조하십시오.
...
1. 실행 인스턴스에 포함된 모바일 응용 프로그램뿐만 아니라 **Mobile 응용 프로그램** 유형을 Adobe Campaign 서비스에 복제합니다.

이벤트는 다음 요소를 포함해야 합니다.

* 모바일 장치 ID(**registrationId**, iOS의 경우 **deviceToken**). 이 ID는 알림을 보낼 &quot;주소&quot;를 나타냅니다.
* 응용 프로그램과 관련된 연결 정보를 복구할 수 있는 모바일 응용 프로그램 또는 통합 키(**uuid**)에 대한 링크입니다.
* 알림을 보낼 채널(**whoseChannel**):iOS용 41 및 Android용 42
* 개인화에 유용한 모든 데이터
...
* 동일한 이름을 사용하여 두 인스턴스에 모두 LINE **외부 계정** 및 **서비스**&#x200B;를 만듭니다. LINE 외부 계정 및 서비스를 만드는 방법에 대한 자세한 내용은 이 [페이지](../../delivery/using/line-channel.md#creating-a-line-account-and-an-external-account-)를 참조하십시오.

그런 다음 **[!UICONTROL Explorer]** , **[!UICONTROL Platform]** > **[!UICONTROL External account]**&#x200B;에서 두 인스턴스 모두에 대해 다른 외부 계정을 구성해야 합니다.
...
1. 다음 구성으로 **실행** 인스턴스에 **[!UICONTROL External database]** 외부 계정을 만듭니다.
   ...
   * **[!UICONTROL Type]** :선택합니다 **[!UICONTROL External database]** .
...
1. 다음 구성으로 **[!UICONTROL External Database]** 계정을 **marketing** 인스턴스에 만듭니다.
   ...
   * **[!UICONTROL Type]** :선택합니다 **[!UICONTROL HTTP relay to remote Database]** .
...
1. 데이터 동기화 워크플로우를 만들기 위해 다음 구성을 사용하여 **marketing** 인스턴스에 **[!UICONTROL Execution instance]** 외부 계정을 만듭니다.

   ![](assets/line_config_mc_2.png)

   * **[!UICONTROL Label]** 및:  **[!UICONTROL Internal name]** 필요에 따라 외부 계정 이름을 지정합니다.
   * **[!UICONTROL Type]** :선택합니다 **[!UICONTROL Execution instance]** .
   * 활성화 상자를 선택해야 합니다.
   ...
   * **[!UICONTROL Method]** :선택합니다 **[!UICONTROL Federated Data Access (FDA)]** .
   * **[!UICONTROL FDA account]** :드롭다운에서 FDA 계정을 선택합니다.
   * **[!UICONTROL Create the archiving workflow]** 버튼을 클릭합니다.
   * **[!UICONTROL Create data synchronization workflow]** 단추를 클릭하여 LINE 데이터 동기화 작업 과정을 만듭니다.



1. 이제 트랜잭션 메시지 작성을 시작할 수 있습니다. 자세한 정보는 이 [페이지](../../message-center/using/introduction.md)를 참조하십시오.
