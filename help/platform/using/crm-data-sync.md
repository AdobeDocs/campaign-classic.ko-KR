...
Adobe Campaign과 CRM 간의 데이터 동기화는 전용 워크플로우 활동([CRM 커넥터](../../workflow/using/crm-connector.md))을 통해 수행됩니다.
...
1. **[!UICONTROL Remote fields]** 섹션으로 이동하여 가져올 필드를 입력하십시오.

   필드를 추가하려면 도구 모음에서 **[!UICONTROL Add]** 단추를 클릭한 다음 **[!UICONTROL Edit expression]** 아이콘을 클릭합니다.

   ![](assets/crm_task_import_add_field.png)

   필요한 경우 **[!UICONTROL Conversion]** 열의 드롭다운 목록을 통해 데이터 형식을 변경합니다. 가능한 변환 형식은 [데이터 형식](#data-format)에 자세히 설명되어 있습니다.
...
1. 필요에 따라 가져올 데이터를 필터링할 수도 있습니다. 이렇게 하려면 **[!UICONTROL Edit the filter...]** 링크를 클릭하십시오.
...
1. **[!UICONTROL Use automatic index...]** 옵션을 사용하면 날짜 및 마지막 수정 사항에 따라 CRM과 Adobe Campaign 간의 증분 개체 동기화를 자동으로 관리할 수 있습니다.

   자세한 내용은 [변수 관리](#variable-management)를 참조하세요.
...
내보내기를 수행하려면 **[!UICONTROL CRM Connector]** 활동에 다음 구성을 적용하십시오.
...
1. **[!UICONTROL Remote object]** 드롭다운 목록으로 이동하여 프로세스와 관련된 개체를 선택하십시오. 이 개체는 커넥터 구성 중에 Adobe Campaign에서 만든 표 중 하나와 일치합니다.
   ...
   >[!IMPORTANT]
   >
   >**[!UICONTROL CRM Connector]** 활동의 내보내기 기능은 CRM측의 필드를 삽입하거나 업데이트할 수 있습니다. CRM에서 필드 업데이트를 활성화하려면 원격 테이블의 기본 키를 지정해야 합니다. 키가 없으면 데이터가 업데이트 대신 삽입됩니다.

1. 더 빠른 내보내기가 필요하면 **[!UICONTROL Export in Batches]**&#x200B;을(를) 확인하십시오.
...
* **[!UICONTROL Copy/Paste]**: 이 모드에서는 문자열과 같은 원시 데이터를 사용합니다(전환 없음).

### 오류 처리 중 {#error-processing}

데이터 가져오기 또는 내보내기 프레임워크 내에서 특정 프로세스를 오류와 거부에 적용할 수 있습니다. 이렇게 하려면 **[!UICONTROL Behavior]** 탭에서 **[!UICONTROL Process rejects]** 및 **[!UICONTROL Process errors]** 옵션을 선택하십시오.
...
>[!NOTE]
>
>**[!UICONTROL Process rejects]** 옵션이 활성화되지 않은 경우에도 각 거부된 열에 오류 코드와 메시지가 있는 경고가 생성됩니다.
...
1. **[!UICONTROL Remote object]** 드롭다운 목록으로 이동하여 프로세스와 관련된 개체를 선택하십시오. 이 개체는 커넥터 구성 중에 Adobe Campaign에서 만든 표 중 하나와 일치합니다.
1. **[!UICONTROL Start date]** 및 **[!UICONTROL End date]** 필드에 고려할 삭제 기간을 지정하십시오. 이 날짜는 기간에 포함됩니다.
...
>[!NOTE]
>
>**[!UICONTROL Process rejects]** 옵션을 사용하지 않도록 설정한 경우에도 거부된 각 열에 대해 경고가 생성됩니다.
>
