...
Campaign Classic v7의 호스팅 모델, 배포 차이점 및 마이그레이션 경로에 대한 답변을 찾을 수 있습니다. 이러한 질문은 인프라 선택 및 관련 책임에 중점을 둡니다.
...
**워크플로우, 게재, 대상자, 보고, 개인화 등에 대한 일반적인 Campaign 질문에 대한 자세한 답변을 보려면 [Campaign v8 종합 FAQ](https://experienceleague.adobe.com/docs/campaign/campaign-v8/start/campaign-faq-comprehensive.html){target="_blank"}를 참조하세요.**

+++

## 빌드 업그레이드(Campaign Classic v7) {#build-upgrades-v7}

빌드 업그레이드 지침 및 관련 FAQ에 대해서는 [빌드 업그레이드 FAQ](faq-build-upgrade.md) 및 [빌드 업그레이드 설명서](../../production/using/build-upgrade.md)를 참조하십시오.

## Campaign Classic v7 구성 {#v7-configuration}

이러한 질문에서는 언어 설정에서 보안 강화에 이르기까지 Campaign Classic v7의 일반적인 구성 작업 및 정책을 다룹니다. 이를 통해 설정 선택 사항 및 운영 사례를 검증할 수 있습니다.
...
+++ 호스팅된 고객은 어디에서 인스턴스 설정을 관리할 수 있습니까? {#where-to-manage-instance-settings}

[Campaign 컨트롤 패널](https://experienceleague.adobe.com/docs/control-panel/using/control-panel-home.html?lang=ko){target="_blank"}은(는) Adobe Campaign의 제품 관리자가 설정을 관리하고 각 인스턴스의 사용량을 추적할 수 있도록 지원합니다. 컨트롤 패널의 직관적인 인터페이스를 활용하면 주요 자산을 모니터링하고 IP 허용 목록 업데이트, SFTP 스토리지 모니터링, 키 관리 등의 관리 작업을 수행할 수 있습니다.

**주요 이점:**

* 고객 지원 센터에 문의하지 않고도 설정을 빠르게 변경할 수 있습니다.
* 서로 다른 시간에 다양한 비즈니스 요구 사항을 기반으로 설정을 구성합니다.
...
커뮤니티 포럼 및 Adobe 지원에 대한 링크를 포함하여 Campaign Classic v7에 대한 주요 설명서, FAQ 및 지원 채널을 찾아보십시오.

+++ Campaign Classic v7 설명서는 어디에서 찾을 수 있습니까? {#where-to-find-more-info-v7}
...
+++ Campaign Classic v7에 대한 커뮤니티 또는 Adobe 지원을 받으려면 어떻게 해야 합니까? {#where-to-get-support-v7}
