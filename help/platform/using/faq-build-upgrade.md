...
product: campaign
title: 빌드 업그레이드 FAQ
description: Campaign Classic v7 빌드 업그레이드와 관련된 일반적인 질문
...
새 기능 및 설명서를 포함한 최신 Campaign Classic 버전은 최신 [릴리스 정보](../../rn/using/latest-release.md)에 자세히 설명되어 있습니다.

## 실행 중인 버전을 어떻게 알 수 있습니까?

Adobe Campaign 클라이언트 콘솔의 **[!UICONTROL Help > About...]** 메뉴에서 버전을 확인하세요. **[!UICONTROL About]** 상자에는 콘솔 및 서버 모두에서 실행 중인 버전 및 빌드에 대한 자세한 정보가 들어 있습니다.
...
자세한 정보는 [이 문서를 참조하세요](https://helpx.adobe.com/kr/campaign/kb/acc-build-upgrade.html).

## 빌드 업그레이드에 필요한 리소스는 무엇입니까?

빌드 업그레이드 프로세스에는 다음 리소스가 필요합니다.

* Adobe Architect - 호스팅 또는 클라우드 메시징/하이브리드 아키텍처의 경우 설계자는 고객 지원 센터와 협력해야 합니다.
...
아래의 [빌드 업그레이드 검사 목록](#check-list)과 테스트 계획을 검토하고 이러한 테스트를 수행할 수 있는 리소스를 24~48시간 내에 사용할 수 있는지 확인하십시오. 업그레이드를 완료해야 합니다.

자세한 정보는 [이 섹션을 참조하세요](../../production/using/build-upgrade.md).
...
## 업그레이드 프로세스 중에도 Campaign 인스턴스를 계속 사용할 수 있습니까?

아니요. Adobe에서는 데이터가 누락되거나 손실되지 않도록 유지 관리 기간 동안 캠페인에서 모든 작업을 비활성화하는 것이 좋습니다. 업그레이드 중에 캠페인, 쿼리, 워크플로우 또는 게재를 만들거나 배포해서는 안 됩니다.
...
아니요. 빌드 업그레이드 중에는 워크플로우 및 메일 서비스가 모두 중지됩니다. 즉, 워크플로우가 실행되지 않고 게재가 전송되지 않습니다. 시스템이 다시 시작되면 다시 시작됩니다. 그러나 Adobe에서는 업그레이드 후 모든 중요 경로 워크플로우가 정상적으로 실행되고 있는지 확인하는 것이 좋습니다.

## 업그레이드 중에도 내 추적 링크가 계속 작동합니까?

Campaign Classic v7.3.5부터 이미 전송된 이메일의 추적 링크는 업그레이드 중에도 계속 작동합니다.

## 빌드 업그레이드 프로세스 중에 를 사용할 수 있어야 합니까?

예. 고객은 프로덕션 인스턴스 업그레이드 중 또는 업그레이드 직후 사용 가능한 연락 지점을 Adobe에 제공해야 합니다.  다른 합의를 하지 않는 한 Adobe은 이메일을 통해 이 담당자에게 연락합니다. 이를 통해 원활한 전환과 중요한 작업의 즉각적인 검증을 보장합니다. 확인을 위해 빌드 업그레이드가 완료되면 Adobe이 고객에게 연락합니다.
...
아래의 [빌드 업그레이드 검사 목록](#check-list)을 참조하세요.
...
아래 [빌드 업그레이드 검사 목록](#check-list)을 참조하세요. 고객은 환경에 필요한 다른 테스트와 유사한 테스트를 실행해야 합니다.

## 빌드 업그레이드를 얼마나 자주 수행해야 합니까?

Adobe은 시스템의 최적의 성능, 가용성 및 보안을 보장하기 위해 고객과 협력하여 매년 최소 한 번 이상 시스템을 업그레이드하도록 할 것입니다.
...
빌드 업그레이드 후 문제가 발생하면 [Adobe 고객 지원 센터](https://helpx.adobe.com/kr/enterprise/admin-guide.html/enterprise/using/support-for-experience-cloud.ug.html)에 문의하십시오. 고객 지원 센터에서 빌드 날짜를 예약하고 빌드 업그레이드 관련 티켓을 엽니다.

[Campaign Classic 도움말 및 지원 옵션](../../support.md)에서 자세히 알아보기
...
* Campaign 데이터베이스에 데이터를 삽입하는 웹 애플리케이션이 있습니까? 그렇다면 실행하고
API를 통해 새 레코드를 삽입할 수 있는지 확인합니다.
* 테스트 이메일을 정상적으로 보낼 수 있습니까? 알려진 템플릿을 사용하여 새 게재를 만들고 (으)로 보내기
한 명의 테스트 받는 사람, 개인화 확인, 구독 취소 링크, 미러 페이지 모든 작업.
* 모든 중요 경로 워크플로우가 실행 중입니까? 워크플로우 확인, 워크플로우 저널 열기, 확인
오류가 없습니다.
