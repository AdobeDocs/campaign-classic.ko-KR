...
* **사용자 패키지**: 내보낼 엔터티 목록을 선택할 수 있습니다. 이 유형의 패키지는 종속성을 관리하고 오류를 확인합니다.
* **플랫폼 패키지**: 추가된 모든 기술 리소스(비표준)를 포함합니다. 스키마, JavaScript 코드 등
...
>[!CAUTION]
>
>**platform** 및 **admin** 형식에 내보낼 미리 정의된 엔터티 목록이 있습니다. 각 엔티티는 필터링 조건에 연결되어 만들어진 패키지의 기본 리소스를 제거할 수 있습니다.
...
**`<entities>`** 요소에 **schema** 특성에 입력한 데이터 스키마 형식의 패키지 데이터가 포함되어 있습니다.

패키지의 데이터에는 자동 생성 키(**autopk** 옵션)와 같이 기본 간에 호환되지 않는 내부 키가 포함되지 않아야 합니다.
...
값이 &quot;none&quot;인 **`operation`** 특성은 조정 링크를 정의합니다.
...
1. 문서 유형별로 내보낼 엔티티 나열:
   ...
   목록 관리를 사용하면 구성에서 내보낼 엔티티를 추가하거나 삭제할 수 있습니다. 새 엔터티를 선택하려면 **[!UICONTROL Add]**&#x200B;을(를) 클릭하십시오.

   **[!UICONTROL Detail]** 단추를 사용하면 선택한 구성이 편집됩니다.

   >[!NOTE]
   >
   >종속성 메커니즘은 엔티티 내보내기 시퀀스를 제어합니다. 자세한 내용은 [종속성 관리](#managing-dependencies)를 참조하세요.
...
1. 추출 중에 데이터의 순서를 지정하려면 **[!UICONTROL Next]**&#x200B;을(를) 클릭하고 정렬 열을 선택하십시오.
...
* **own** 또는 **owncopy** 형식 무결성이 있는 링크에 연결된 개체를 내보낸 개체와 동일한 패키지로 내보냅니다.
* **neutral** 또는 **define** 형식 무결성(정의된 링크)이 있는 링크에 연결된 개체를 별도로 내보내야 합니다.

>[!NOTE]
>
>스키마 요소에 연결된 무결성 유형이 [이 섹션](../../configuration/using/database-mapping.md#links--relation-between-tables)에 정의되어 있습니다.
...
패키지 유형에 대한 제휴는 **@pkgAdmin 및** 특성이 @pkgPlatform 스키마에 정의되어 있습니다. 이러한 속성은 모두 패키지 제휴 조건을 정의하는 XTK 식을 받습니다.
...
마지막으로 **@pkgStatus** 특성을 사용하면 이러한 요소 또는 특성에 대한 내보내기 규칙을 정의할 수 있습니다. 속성 값에 따라 요소 또는 속성은 내보낸 패키지에서 찾을 수 있습니다. 이 속성에 대해 가능한 세 가지 값은 다음과 같습니다.

* **절대**: 필드/링크를 내보내지 않습니다.
* **always**: 이 필드에 대한 내보내기를 강제 적용합니다.
* **preCreate**: 연결된 엔터티의 만들기를 허용합니다.

>[!NOTE]
>
>**preCreate** 값은 링크 유형 이벤트에만 사용할 수 있습니다. 내보낸 패키지에 아직 로드되지 않은 엔티티를 생성하거나 가리키도록 허용합니다.
...
**[!UICONTROL Content]** 탭에서 **[!UICONTROL Add]** 단추를 클릭하여 패키지로 내보낼 엔터티를 선택합니다. 엔터티를 선택할 때의 모범 사례는 [이 섹션](#exporting-a-set-of-objects-in-a-package) 섹션에 나와 있습니다.
...
1. 엔터티가 패키지 정의에 추가되면 패키지와 함께 내보내집니다([이 섹션](#exporting-packages-from-a-package-definition) 참조).
...
* **[!UICONTROL Include an installation script]**: 패키지 가져오기 시 실행할 javascript 스크립트를 추가할 수 있습니다. 선택하면 패키지 정의 화면에 **[!UICONTROL Script]** 탭이 추가됩니다.
* **[!UICONTROL Include default values]**: 모든 엔터티의 특성 값을 패키지에 추가합니다.
  ...
  >
  >이 경우 이전 인스턴스의 모든 특성을 패키지와 함께 내보내므로 **[!UICONTROL Include default values]** 옵션을 선택하면 버전이 병합되지 않습니다.

### 패키지 정의에서 패키지 내보내기 {#exporting-packages-from-a-package-definition}

패키지 정의에서 패키지를 내보내려면 아래 단계를 수행합니다.

1. 내보낼 패키지 정의를 선택한 다음 **[!UICONTROL Actions]** 단추를 클릭하고 **[!UICONTROL Export the package]**&#x200B;을(를) 선택합니다.
...
기존 데이터 패키지를 가져오려면 XML 파일을 선택하고 **[!UICONTROL Open]**&#x200B;을(를) 클릭합니다.
...
기본 제공 패키지에 대한 자세한 내용은 [이 페이지](../../installation/using/installing-campaign-standard-packages.md)를 참조하세요.
...
>[!NOTE]
>
>패키지를 자동으로 내보내는 워크플로우를 설정하는 방법에 대한 자세한 내용은 [이 페이지](https://helpx.adobe.com/campaign/kb/export-packages-automatically.html)를 참조하십시오.
