...
CRM에 연결하는 것은 전용 워크플로우 활동을 통해 수행됩니다. 이러한 활동은 [이 섹션에 제시된 장에 자세히 설명되어 있습니다](../../workflow/using/crm-connector.md).

### 호환 CRM 시스템 및 제한 사항 {#compatible-crm-systems-and-limitations}
...
* **MS Dynamics CRM** 및 **MS Dynamics Online**
   ...
   Microsoft Dynamics와의 연결 설정 방법을 알아보려면 [이 섹션을](#example-for-microsoft-dynamics) 참조하십시오.

   ![](assets/do-not-localize/how-to-video.png) 이 비디오에서 Adobe Campaign 및 Microsoft Dynamics 통합 활용 사례 [에 대해 알아보십시오](https://helpx.adobe.com/campaign/kt/acc/using/acc-integrate-dynamics365-with-acc-feature-video-set-up.html).
...
Salesforce. **com** 커넥터를 Adobe Campaign과 구성하려면 아래 단계를 따르십시오.
...
1. 구성 마법사를 실행하여 사용 가능한 CRM 테이블을 생성합니다.
   ...
   실행을 **[!UICONTROL Start]** 실행하려면 을 클릭합니다.

   ![](assets/crm_connectors_sfdc_launch.png)

   >[!NOTE]
   >
   >설정을 승인하려면 로그오프하고 Adobe Campaign 콘솔으로 다시 로그온해야 합니다.
...
1. 일치하는 스키마 **[!UICONTROL Next]** 를 만들려면 을 클릭합니다.

   일치하는 데이터 스키마를 Adobe Campaign에서 사용할 수 있습니다.
...
Microsoft Dynamics 커넥터가 Adobe Campaign에서 작동하도록 구성하려면 다음 단계를 수행하십시오.

1. Adobe Campaign 트리의 노드를 통해 새 외부 계정 **[!UICONTROL Administration > Platform > External accounts]** 을 만듭니다.

   ![](assets/crm_connectors_msdynamics_01_4.png)

1. 배포 **유형을 선택합니다**. **[!UICONTROL On-premise]**, **[!UICONTROL Office 365]** or **[!UICONTROL Web API]**, depending on the connector you want to configure.

   Adobe Campaign Classic은 인증을 위해 OAuth 프로토콜이 있는 Dynamics 365 REST 인터페이스를 지원합니다.

   배포를 선택하는 경우 **[!UICONTROL WebAPI]** Azure 디렉터리에 앱을 등록하고 Azure 디렉터리에서 **clientId** 를 받아야 합니다. 이 등록은 [이 페이지에 설명되어 있습니다](https://docs.microsoft.com/en-us/previous-versions/dynamicscrm-2016/developers-guide/mt622431(v=crm.8)?redirectedfrom=MSDN).

   >[!NOTE]
   >
   >redirectURL 매개 변수는 Adobe Campaign Classic에서 필요하지 않습니다.

   clientId **값은 사용자 이름/암호와 함께 사용되어 부여 유형 암호를 사용하여 전달자 토큰을 가져옵니다** . 이를 리소스 **소유자 암호 자격 증명 부여라고 합니다**. 자세한 정보는 이 [페이지](https://blogs.msdn.microsoft.com/wushuai/2016/09/25/resource-owner-password-credentials-grant-in-azure-ad-oauth/)를 참조하십시오.

   ![](assets/crm_connectors_msdynamics_01_3.png)

   CRM 버전 호환성에 대한 자세한 내용은 [호환성 매트릭스를 참조하십시오](https://helpx.adobe.com/kr/campaign/kb/compatibility-matrix.html).
...
1. 을 **[!UICONTROL Next]** 클릭하고 해당 스키마 만들기를 시작합니다.
...
1. Adobe Campaign과 Microsoft Dynamics 간 열거형 동기화를 시작합니다.
...
* CRM에서 삭제된 개체 가져오기(CRM에서 [삭제된 개체 가져오기 참조](#importing-objects-deleted-in-the-crm)),
* CRM에서 개체를 삭제합니다(CRM에서 [개체 삭제 참조](#deleting-objects-in-the-crm)).
...
이 활동의 구성은 수행하는 프로세스에 따라 다릅니다. 다양한 구성은 아래에 자세히 나와 있습니다.
...
1. 필요에 따라 가져올 데이터를 필터링할 수도 있습니다. 이렇게 하려면 **[!UICONTROL Edit the filter...]** 링크를 클릭합니다.
   ...
   다음 예에서, Adobe Campaign은 2012년 11월 1일부터 일부 활동이 기록된 연락처만 가져옵니다.
...
1. 이 **[!UICONTROL Use automatic index...]** 옵션을 사용하면 날짜 및 마지막 수정 사항에 따라 CRM과 Adobe Campaign 간의 증분 객체 동기화를 자동으로 관리할 수 있습니다.
...
* 각 필터링 수준에는 하나의 연산자 유형만 사용할 수 있습니다.
* AND NOT 연산자는 지원되지 않습니다.
* 비교는 null 값(&#39;is empty&#39;/&#39;is not empty&#39; type) 또는 숫자만 포함할 수 있습니다. 즉, 값(오른쪽 열)이 평가되고 이 평가의 결과가 숫자여야 합니다. 따라서 JOIN 유형 비교는 지원되지 않습니다.
* 오른쪽 열에 포함된 값은 JavaScript로 평가됩니다.
* JOIN 비교는 지원되지 않습니다.
* 왼쪽 열의 표현식은 필드여야 합니다. 여러 표현식, 숫자 등의 조합일 수 없습니다.

예를 들어 OR 연산자는 AND 연산자와 동일한 수준에 있으므로 CRM 가져오기에 다음 필터링 조건이 적합하지 않습니다.
...
이렇게 하려면 링크를 클릭하고 **[!UICONTROL Order by]** 열을 목록에 추가합니다.
...
CRM에 포함된 요소(및 필터링될 수 있는 요소)를 가져오는 대신 워크플로우에서 미리 계산된 모집단을 사용할 수 있습니다.

이렇게 하려면 **[!UICONTROL Use the population calculated upstream]** 옵션을 선택하고 원격 식별자가 포함된 필드를 지정합니다.

그런 다음 가져올 인바운드 모집단 필드를 아래에서 보듯이 선택합니다.
...
1. 섹션에서 **[!UICONTROL Mapping]** 내보낼 필드와 CRM에서 해당 매핑을 지정합니다.
   ...
   >[!NOTE]
   >
   >내보낼 레코드 목록과 내보내기 결과는 워크플로우가 완료되거나 다시 시작될 때까지 액세스할 수 있는 임시 파일에 저장됩니다. 이렇게 하면 동일한 레코드를 여러 번 내보내거나 데이터를 잃을 위험이 없는 오류가 발생하는 경우 프로세스를 다시 시작할 수 있습니다.
...
이 **[!UICONTROL Default]** 모드는 자동 데이터 변환을 적용하며 대부분의 경우 데이터의 복사/붙여넣기와 같습니다. 하지만 표준 시간대 관리는 적용됩니다.
...
데이터 가져오기 또는 내보내기 프레임워크 내에서 특정 프로세스를 오류와 거부에 적용할 수 있습니다. 이렇게 하려면 탭에서 **[!UICONTROL Process rejects]** 및 **[!UICONTROL Process errors]** 옵션을 **[!UICONTROL Behavior]** 선택합니다.
...
거부는 오류 코드와 관련 메시지로 수집되므로, 동기화 프로세스를 최적화하기 위해 거부 추적을 설정할 수 있습니다.

>[!NOTE]
>
>옵션이 활성화되지 않은 경우에도 **[!UICONTROL Process rejects]** 오류 코드와 메시지와 함께 거부된 각 열에 대한 경고가 생성됩니다.
...
광범위한 데이터 동기화 프로세스 설정을 활성화하려면 CRM에서 삭제된 개체를 Adobe Campaign으로 가져올 수 있습니다.

이렇게 하려면 다음 단계를 적용합니다.

1. 작업을 **[!UICONTROL Import objects deleted in the CRM]** 선택합니다.
1. 드롭다운 **[!UICONTROL Remote object]** 목록으로 이동하여 해당 프로세스에 해당하는 개체를 선택합니다. 이 개체는 커넥터 구성 중에 Adobe Campaign에서 만든 테이블 중 하나와 일치합니다.
1. 및 필드에서 고려할 삭제 기간 **[!UICONTROL Start date]** 을 **[!UICONTROL End date]** 지정합니다. 이 날짜는 기간에 포함됩니다.
...
이 **[!UICONTROL Behavior]** 탭에서는 거부 처리를 활성화할 수 있습니다. 이 옵션은 활동에 대한 두 번째 출력 전환을 **[!UICONTROL CRM connector]** 생성합니다. For more on this, refer to [Error processing](#error-processing).

>[!NOTE]
>
>옵션이 비활성화되어 있어도 **[!UICONTROL Process rejects]** 거부된 각 열에 대한 경고가 생성됩니다.
