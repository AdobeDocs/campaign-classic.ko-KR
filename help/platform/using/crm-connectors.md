...
CRM에 연결하는 것은 전용 워크플로우 활동을 통해 수행됩니다. 이러한 활동은 [이 섹션](../../workflow/using/crm-connector.md)에 제시된 장에서 자세히 설명합니다.
...
* **Salesforce.com**
   ...
   Salesforce.com과의 연결을 설정하는 방법을 알아보려면 [이 섹션](#example-for-salesforce-com)을 참조하십시오.
...
* **Oracle On Demand**
   ...
   oracle On Demand와의 연결을 설정하는 방법을 알아보려면 [이 섹션](#example-for-oracle-on-demand)을 참조하십시오.

   >[!IMPORTANT]
   >
   >Adobe Campaign과 Oracle On Demand를 연결하면 제한 사항은 다음과 같습니다.
   >
   >    
   >    
   ...
   >    * API 버전 v1.0에서는 쿼리 중에 데이터를 정렬하거나 필터링할 수 있지만 두 작업을 동시에 수행할 수는 없습니다.
   >    * oracle On Demand에서 보낸 날짜에 시간대 정보가 포함되지 않습니다.
   >    * Adobe Campaign에서 여러 선택 열거형을 지원하지 않습니다.


* **MS Dynamics** CRM 및  **MS Dynamics Online**

   Microsoft Dynamics와의 연결을 설정하는 방법을 알아보려면 [이 섹션](#example-for-microsoft-dynamics)을 참조하십시오.

   ![](assets/do-not-localize/how-to-video.png) 이 비디오에서 Adobe Campaign 및 Microsoft Dynamics 통합 활용 사례 [에 대해 알아봅니다](https://helpx.adobe.com/campaign/kt/acc/using/acc-integrate-dynamics365-with-acc-feature-video-set-up.html).
   ...
   >    
   >    
   >    * 플러그인을 설치하면 CRM의 비헤이비어가 변경되므로 Adobe Campaign와의 호환성 문제가 발생할 수 있습니다.
   >    * Adobe Campaign에서 여러 선택 열거형을 지원하지 않습니다.


## {#setting-up-the-connection} 연결 설정
...
### Salesforce.com {#example-for-salesforce-com}의 예

Adobe Campaign에서 **Salesforce.com** 커넥터를 구성하려면 아래 단계를 따르십시오.
...
1. 구성 마법사를 실행하여 사용 가능한 CRM 테이블을 생성합니다.
   ...
   **[!UICONTROL Start]**&#x200B;을 클릭하여 실행을 실행합니다.
...
### oracle On Demand {#example-for-oracle-on-demand}의 예

**Oracle On Demand** 커넥터를 Adobe Campaign에서 작동하도록 구성하려면 다음 단계를 수행하십시오.

1. Adobe Campaign 트리의 **[!UICONTROL Administration > Platform > External accounts]** 노드를 통해 새 외부 계정을 만듭니다.

   ![](assets/crm_connectors_ood_1.png)

1. 구성 마법사를 엽니다.Adobe Campaign은 Oracle 데이터 모델의 표를 자동으로 표시합니다. 수집할 테이블을 선택합니다.

   ![](assets/crm_connectors_ood_2.png)

1. 일치 스키마 만들기를 시작하려면 **[!UICONTROL Next]**&#x200B;을 클릭합니다.
...
1. oracle On Demand 데이터를 Adobe Campaign으로 가져오려면 다음 유형의 워크플로우를 만드십시오.
   ...
   이 워크플로우에서는 Oracle On Demand를 통해 연락처를 가져오고, 기존 Adobe Campaign 데이터와 동기화하며, 중복된 연락처를 삭제하고, Adobe Campaign 데이터베이스를 업데이트합니다.

   **[!UICONTROL CRM Connector]** 활동을 다음과 같이 구성해야 합니다.
...
### Microsoft Dynamics {#example-for-microsoft-dynamics}의 예

Microsoft Dynamics 커넥터가 Adobe Campaign에서 작동하도록 구성하려면 다음 단계를 적용합니다.

1. Adobe Campaign 트리의 **[!UICONTROL Administration > Platform > External accounts]** 노드를 통해 새 외부 계정을 만듭니다.

   ![](assets/crm_connectors_msdynamics_01_4.png)

1. **배포 유형**&#x200B;을 선택합니다.구성할 커넥터에 따라 **[!UICONTROL On-premise]**, **[!UICONTROL Office 365]** 또는 **[!UICONTROL Web API]**.
   ...
   **clientId** 값은 권한 부여 유형 암호를 사용하여 무기명 토큰을 가져오는 데 사용자 이름/암호와 함께 사용됩니다. 이를 **리소스 소유자 암호 자격 증명 부여**&#x200B;라고 합니다. 자세한 정보는 이 [페이지](https://docs.microsoft.com/en-us/azure/active-directory/develop/v2-oauth-ropc)를 참조하십시오.
   ...
   CRM 버전 호환성에 대한 자세한 내용은 [호환성 매트릭스](https://helpx.adobe.com/kr/campaign/kb/compatibility-matrix.html)를 참조하십시오.

1. 구성 마법사를 엽니다. Adobe Campaign은 Microsoft Dynamics 데이터 템플릿에서 표를 자동으로 검색합니다.
...
1. **[!UICONTROL Next]**&#x200B;을 클릭하고 해당 스키마 만들기를 시작합니다.
...
1. Microsoft Dynamics 데이터를 Adobe Campaign으로 가져오려면 다음 유형의 작업 과정을 만드십시오.
...
이 활동을 통해 다음 작업을 수행할 수 있습니다.

* CRM에서 가져오기([CRM](#importing-from-the-crm)에서 가져오기 참조),
* CRM으로 내보내기([CRM](#exporting-to-the-crm)에 내보내기 참조),
...
* oracle On Demand의 경우&#x200B;**마지막 업데이트**, **ModifiedDate**, **LastLoggedIn**,
...
예를 들어 OR 연산자는 AND 연산자와 동일한 수준에 배치되므로 다음 필터링 조건이 CRM 가져오기에 유효하지 않습니다.
...
내보내기의 경우 **CRM 커넥터** 활동에 다음 구성을 적용합니다.
...
1. **[!UICONTROL Remote object]** 드롭다운 목록으로 이동하여 해당 프로세스의 개체를 선택합니다. 이 객체는 커넥터 구성 중에 Adobe Campaign에서 만든 테이블 중 하나와 일치합니다.
   ...
   >[!IMPORTANT]
   >
   >**CRM 커넥터** 활동의 내보내기 기능은 CRM 측에 필드를 삽입하거나 업데이트할 수 있습니다. CRM에서 필드 업데이트를 사용하려면 원격 테이블의 기본 키를 지정해야 합니다. 키가 없는 경우 데이터가 삽입됩니다(업데이트되는 대신).
...
**[!UICONTROL Reject]** 출력 전환을 사용하면 오류 메시지 및 코드와 관련된 특정 열이 포함된 출력 스키마에 액세스할 수 있습니다. 이러한 열은 다음과 같습니다.

* oracle On Demand의 경우&#x200B;**errorLogFilename**(Oracle 측의 로그 파일 이름), **errorCode**(오류 코드), **errorSymbol**(오류 코드와 다른 오류 기호), **errorMessage**(오류 컨텍스트 설명)
