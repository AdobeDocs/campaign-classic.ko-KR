...
Adobe Campaign 트리(홈 페이지의 메뉴 **[!UICONTROL Explorer]** )에서 탐색하면 데이터베이스에 포함된 데이터가 목록에 표시됩니다. 이러한 목록은 연산자에 필요한 데이터만 표시하도록 구성할 수 있습니다. 그런 다음 필터링된 데이터에 대해 작업을 시작할 수 있습니다. 필터 구성을 사용하여 목록에서 데이터를 선택할 수 있습니다 **[!UICONTROL dynamically]**. 데이터가 수정되면 필터링된 데이터가 업데이트됩니다.
...
* 기본 필터
   ...
   목록 위에 있는 필드를 통해 **기본 필터에** 액세스할 수 있습니다. 미리 정의된 필드(수신자 프로필의 경우 기본적으로 이름 및 이메일 주소)를 필터링할 수 있습니다. 필드를 사용하여 필터링하거나 드롭다운 목록에서 필터 조건을 선택할 문자를 입력할 수 있습니다.
...
* 간단한 필터
   ...
   **단순 필터는** 열에 대해 일회성 필터입니다. 표시된 열에 하나 이상의 간단한 검색 기준으로 정의됩니다.

   동일한 데이터 목록에 여러 개의 간단한 필터를 결합하여 검색을 세분화할 수 있습니다. 필터 필드가 다른 필터 아래에 표시됩니다. 서로 독립적으로 삭제할 수 있습니다.

   ![](assets/filters_recipient_simple_filter.png)

   단순 필터는 간단한 필터 [만들기에서 자세히 설명합니다](#creating-a-simple-filter).

* 고급 필터

   **고급 필터는** 데이터에 대한 쿼리 또는 쿼리 조합을 사용하여 만들어집니다.

   고급 필터 만들기에 대한 자세한 내용은 고급 필터 [만들기를 참조하십시오](#creating-an-advanced-filter).

   함수를 사용하여 필터 컨텐츠를 정의할 수 있습니다. 자세한 내용은 함수를 사용하여 [고급 필터 만들기를 참조하십시오](#creating-an-advanced-filter-with-functions).

   >[!NOTE]
   >
   >Adobe Campaign에서 쿼리 작성에 대한 자세한 내용은 [이 섹션을 참조하십시오](../../platform/using/about-queries-in-campaign.md).

* 사용자 필터

   애플리케이션 **필터는** 다른 연산자와 구성을 사용하고 공유할 수 있도록 저장된 고급 필터입니다.

   목록 위에 있는 **[!UICONTROL Filters]** 단추는 필터링을 구체화하기 위해 결합할 수 있는 애플리케이션 필터 집합을 제공합니다. 이러한 필터를 만드는 방법은 필터 저장 [에서 제공됩니다](#saving-a-filter).
...
다른 모든 유형의 데이터에 대해 노드를 통해 기본 필터를 **[!UICONTROL Administration > Configuration > Predefined filters]** 구성합니다.

다음 단계를 적용합니다.

1. 기본적으로 사용할 필터를 선택합니다.
1. 탭을 **[!UICONTROL Parameters]** 클릭하고 선택합니다 **[!UICONTROL Default filter for the associated document type]**.
...
1. 필터 **[!UICONTROL Save]** 를 적용하려면 클릭하십시오.

   >[!NOTE]
   >
   >필터 정의 창은 고급 필터 [만들기](#creating-an-advanced-filter) 및 필터 [저장에서 자세히 설명합니다](#saving-a-filter).

## 간단한 필터 만들기 {#creating-a-simple-filter}

**단순 필터를**&#x200B;만들려면 다음 단계를 적용합니다.

1. 필터링할 필드를 마우스 오른쪽 단추로 클릭하고 선택합니다 **[!UICONTROL Filter on this field]**.
...
1. 수신자 목록에서 **연령** 열을 마우스 오른쪽 단추로 클릭하고 선택합니다 **[!UICONTROL Filter on this column]**.
...
1. 입력 값을 **[!UICONTROL Expression]** 정의하려면 열을 클릭합니다.
1. 필터 **[!UICONTROL Edit expression]** 를 적용할 필드를 클릭하여 선택합니다.

   ![](assets/s_user_filter_choose_field.png)

1. 목록에서 데이터를 필터링할 필드를 선택합니다. 을 **[!UICONTROL Finish]** 클릭하여 확인합니다.
1. 열을 **[!UICONTROL Operator]** 클릭하고 드롭다운 목록에서 적용할 연산자를 선택합니다.
1. 열에서 예상 값을 **[!UICONTROL Value]** 선택합니다. 여러 필터를 결합하여 쿼리를 세분화할 수 있습니다. 필터 조건을 추가하려면 을 클릭합니다 **[!UICONTROL Add]**.

   ![](assets/s_ncs_user_filter_add_button_alone.png)

1. 표현식에 계층을 할당하거나 도구 모음 화살표를 사용하여 쿼리 표현식 순서를 변경할 수 있습니다.
1. 표현식 간의 기본 연산자는 **And**&#x200B;이지만 필드를 클릭하여 변경할 수 있습니다. Or 연산자를 선택할 수 **있습니다** .

   ![](assets/s_ncs_user_filter_operator.png)

1. 필터 **[!UICONTROL OK]** 를 클릭하여 만들고 목록에 적용합니다.
...
1. 사용할 공식 유형을 선택합니다.집계, 기존 사용자 필터 또는 표현식.
   ...
   * **[!UICONTROL User filter]** 을 클릭하여 기존 사용자 필터 중 하나를 선택합니다. 사용자 필터는 필터 [저장에 자세히 설명되어 있습니다](#saving-a-filter).
   * **[!UICONTROL Expression]** 표현식 편집기에 액세스하려면
      ...
      >[!NOTE]
      >
      >표현식을 기반으로 하는 필터 작성 예를 보려면 생일이 [있는 받는 사람 식별을 참조하십시오](../../workflow/using/sending-a-birthday-email.md#identifying-recipients-whose-birthday-it-is).
...
마우스 오른쪽 단추를 클릭하고 선택하거나 목록 위에 있는 아이콘 **[!UICONTROL No filter]** 을 통해 현재 목록의 모든 필터를 삭제할 수 **[!UICONTROL Filters]** 있습니다.

단추를 클릭하고 메뉴를 사용하여 필터를 결합할 수 **[!UICONTROL Filters]** **[!UICONTROL And...]** 있습니다.

![](assets/s_ncs_user_filter_combination.png)

## 받는 사람 필터링 {#filtering-recipients}

미리 정의된 필터(필터 [저장](#saving-a-filter)참조)를 사용하면 데이터베이스에 포함된 받는 사람의 프로필을 필터링할 수 있습니다. 트리 노드에서 필터 **[!UICONTROL Profiles and Targets > Predefined filters]** 를 편집할 수 있습니다. 필터는 **[!UICONTROL Filters]** 단추를 통해 작업 공간의 상단 섹션에 나열됩니다.
...
>[!NOTE]
>
>사전 정의된 필터 만들기에 대한 자세한 예는 사용 [사례를 참조하십시오](../../platform/using/use-case.md).
...
<table> 
 ...
 <td> 차단 목록의 이메일 주소<br /> </td> 
  ...
  <tr> 
   <td> 열지 클릭하지 않음<br /> </td> 
   <td> 배달을 열지 않았거나 배달을 클릭하지 않은 수신자를 선택합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> 새 받는 사람(일)<br /> </td> 
  ...
  <tr> 
   <td> 새 받는 사람(분)<br /> </td> 
   <td> 마지막 X분 동안 만든 수신자를 선택합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> 새 받는 사람(개월)<br /> </td> 
  ...
  <tr> 
   <td> 게시 배달 동작으로<br /> </td> 
   <td> 배달을 받은 후 자신의 행동에 따라 수신자를 선택합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> 생성 날짜별<br /> </td> 
   <td> X개월(현재 날짜 빼기 n개월)부터 Y 개월(현재 날짜 - n개월)까지의 기간 동안 만들기 날짜별로 받는 사람을 선택합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> 목록별<br /> </td> 
  ...
  <tr> 
   <td> 열기 횟수<br /> </td> 
   <td> Z 시간에 걸쳐 X와 Y 배달 사이에 연 수신자를 선택합니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> 이름 또는 이메일<br /> </td> 
...
>[!NOTE]
>
>계산 및 기간과 관련된 모든 비교는 더 광범위한 의미에서 이해할 수 있습니다(쿼리 제한에 해당하는 수신자는 비교에 포함됩니다).
...
* 이 **[!UICONTROL Advanced parameters]** 링크를 통해 추가 설정을 정의할 수 있습니다. 특히 SQL 테이블을 필터와 연결하여 테이블을 공유하는 모든 편집자에게 공통으로 제공할 수 있습니다.

   사용자가 이 필터를 덮어쓰는 것을 중지하려면 **[!UICONTROL Do not restrict the filter]** 옵션을 선택합니다.

   배달 마법사에서 제공하는 &quot;배달 받는 사람&quot; 및 &quot;폴더에 속하는 배달 받는 사람&quot; 필터에 대해 과부할 수 없는 이 옵션이 활성화됩니다.
