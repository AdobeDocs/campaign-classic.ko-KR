...
기본적으로 모든 입력 **`<attribute>`** 및 **`<element>`** 요소는 데이터 스키마 테이블의 SQL 필드에 매핑됩니다. 그러나 SQL 대신 XML에서 이 필드를 참조할 수 있습니다. 즉, 데이터는 모든 XML 필드의 값이 들어 있는 테이블의 메모 필드(&quot;mData&quot;)에 저장됩니다. 이러한 데이터의 저장은 스키마 구조를 준수하는 XML 문서입니다.
...
소스 스키마에 pkSequence 속성과 함께 사용할 시퀀스의 이름을 지정할 수 **있습니다** . 이 특성이 소스 스키마에 지정되지 않으면 **XtkNewId** 기본 시퀀스가 사용됩니다. 응용 프로그램은 **nms:broadLog** 및 **nms:trackingLog** 스키마(**NmsBroadLogId** 및 **** NmsTrackingLogIdId)에 대해 전용 시퀀스를 사용합니다. 이 테이블은 레코드가 가장 많이 들어 있는 테이블이기 때문입니다.
...
## 링크:테이블 간의 관계 {#links--relation-between-tables}

링크는 한 테이블과 다른 테이블 간의 연결을 설명합니다.

다양한 유형의 연결(&quot;카디널리티&quot;라고 함)은 다음과 같습니다.

* 카디널리티 1-1:소스 테이블 중 한 가지에는 대상 테이블에 해당하는 항목이 최대 한 개까지 있을 수 있습니다.
* 카디널리티 1-N:소스 테이블 일수에 타겟 테이블의 해당 항목이 여러 개 있을 수 있지만, 대상 테이블 일수의 경우 소스 테이블에 해당하는 항목이 최대 하나 있을 수 있습니다.
* 카디널리티 N-N:소스 테이블 일수에 대상 테이블에 해당하는 항목이 여러 개 있을 수 있으며 그 반대의 경우도 있습니다.

인터페이스에서 아이콘 덕분에 서로 다른 유형의 관계식을 쉽게 구분할 수 있습니다.

캠페인 테이블/데이터베이스와의 연결 관계

* ![](assets/join_with_campaign11.png) :카디널리티 1-1. 예를 들어, 받는 사람과 현재 주문 간의 경우입니다. 받는 사람은 현재 주문 테이블의 한 번에 한 번만 관련될 수 있습니다.
* ![](assets/externaljoin11.png) :카디널리티 1-1, 외부 조인. 예를 들어, 받는 사람과 해당 국가 간의 경우입니다. 받는 사람은 테이블 국가의 한 번만 관련될 수 있습니다. 국가 테이블의 내용은 저장되지 않습니다.
* ![](assets/join_with_campaign1n.png) :카디널리티 1-N.예를 들어 받는 사람과 구독 테이블 간의 경우입니다. 수신자는 가입 테이블에 있는 여러 작업과 관련될 수 있습니다.
...
* 링크의 정의는 다음 속성을 가진 **링크**&#x200B;유형 **`<element>`** 에입력됩니다.
   ...
   * **integrity** (선택 사항):대상 테이블 발생에 대한 소스 테이블의 참조 무결성 가능한 값은 다음과 같습니다.
      ...
      * **정의**:대상 발생을 더 이상 참조하지 않을 경우 소스 발생을 삭제할 수 있습니다.
      * **일반**:소스 발생을 삭제하면 해당 링크의 키가 대상 발생으로 초기화됩니다(기본 모드). 이 무결성 유형은 모든 외래 키를 초기화합니다.
      * **자체**:소스 발생을 삭제하면 타겟 항목이 삭제됩니다.
      * **다운로드**:자체 **** (삭제의 경우)와 동일하거나, 복제(중복의 경우)를 복제합니다.
      * **중립**:아무 것도 하지 않습니다.
   * **revIntegrity** (선택 사항):대상 스키마의 무결성(선택 사항, 기본적으로 &quot;normal&quot;),
   * **revCardinality** (선택 사항):값이 &quot;single&quot;이면 기수를 1-1(기본적으로 1-N)으로 채웁니다.
   * **externalJoin** (선택 사항):외부 연결로
   * **revExternalJoin** (선택 사항):역링크에 외부 연결을 강제 적용


* 링크는 소스 테이블에서 대상 테이블로 하나 이상의 필드를 참조합니다. 조인( `<join>` 요소)을 구성하는 필드는 대상 스키마의 내부 키를 사용하여 기본적으로 자동으로 추론되므로 채워지지 않아도 됩니다.
* 인덱스가 확장 스키마의 링크의 외래 키에 자동으로 추가됩니다.
* 링크는 두 개의 절반 링크로 구성되어 있는데, 여기서 첫 번째 링크는 소스 스키마에서 선언되고 두 번째 링크는 대상 스키마의 확장 스키마에서 자동으로 생성됩니다.
* 외부 **조인** 속성이 추가된 경우 조인은 외부 조인으로, 값 &quot;true&quot;(PostgreSQL에서 지원됨)를 사용할 수 있습니다.

>[!NOTE]
>
>일반적으로 링크는 스키마의 끝에 선언된 요소입니다.
...
링크 정의는 대상 스키마에서 XPath(&quot;@id&quot;)가 있는 기본 키와 스키마에 있는 XPath(&quot;@company-id&quot;)를 사용하는 외래 키가 스키마에 의해 보완됩니다.

외래 키는 다음 명명 규칙과 함께 대상 테이블의 관련 필드와 동일한 특성을 사용하는 요소에 자동으로 추가됩니다.대상 스키마의 이름 뒤에 관련 필드 이름(&quot;company-id&quot;)이 옵니다.
...
* **이름**:소스 스키마의 이름에서 자동으로 추론됨(소스 스키마의 링크 정의에서 &quot;revLink&quot; 속성으로 강제 설정될 수 있음)
* **revLink**:역링크 이름
...
기본값은 &quot;DefaultFolder(&#39;nmsFolder&#39;)&quot; 함수에 입력된 첫 번째 자격 매개 변수 유형 파일의 식별자를 반환합니다.

### Example 5 {#example-5}

이 예에서는 xlink 속성과 (&quot;email&quot;) 테이블의 필드를 사용하여 링크(&quot;company&quot; to &quot;cus:company&quot; 스키마)에 대한 키 **를** 생성하려고 합니다.
