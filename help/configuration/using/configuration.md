...
이 섹션](../../platform/using/adobe-campaign-explorer.md#about-navigation-hierarchy)에서 Campaign 탐색기 및 탐색 계층 [에 대해 자세히 알아보십시오.
...
XML 문서에는 문서 이름 및 네임스페이스를 지정할 **name** 및 **namespace** 특성이 있는 **`<navtree>`** 루트 요소가 포함되어 있습니다. 이름 및 네임스페이스는 문서 식별 키를 구성합니다.
...
전역 명령에 대한 설명은 다음 속성을 사용하여 **`<command>`** 요소에 입력됩니다.

* **이름**: 명령의 내부 이름: 이름을 입력하고 고유해야 합니다.
* **레이블**: 명령의 레이블입니다.
* **desc**: 기본 화면의 상태 표시줄에 표시되는 설명입니다.
* **양식**: 시작할 양식: 입력할 값은 입력 양식의 식별 키(예: &quot;cus:recipient&quot;)
* **권한**: 이 명령에 액세스할 수 있는 명명된 권한 목록(쉼표로 구분됨) 사용 가능한 권한 목록은 **[!UICONTROL Administration > Access management > Named rights]** 폴더에서 액세스할 수 있습니다.
* **promptLabel**: 명령을 실행하기 전에 확인 상자를 표시합니다.

**`<command>`** 요소는 **`<command>`** 하위 요소를 포함할 수 있습니다. 이 경우 상위 요소를 사용하면 이러한 하위 요소로 구성된 하위 메뉴를 표시할 수 있습니다.

명령은 XML 문서에 선언된 것과 같은 순서로 표시됩니다.

명령 구분 기호를 사용하면 명령 간 분리 막대를 표시할 수 있습니다. 명령 레이블에 포함된 **&#39;-&#39;** 값으로 식별됩니다.

입력 매개 변수와 함께 **`<soapcall>`** 태그의 선택적 존재 여부는 실행할 SOAP 메서드 호출을 정의합니다. SOAP API에 대한 자세한 내용은 [Campaign JSAPI 설명서](https://docs.adobe.com/content/help/en/campaign-classic/technicalresources/api/index.html) 를 참조하십시오.

양식 컨텍스트는 **`<enter>`** 태그의 초기화 시 업데이트할 수 있습니다. 이 태그에 대한 자세한 내용은 입력 양식에 대한 설명서를 참조하십시오.
...
* &quot;xtk:import&quot; 양식을 시작하는 전역 명령 선언:
   ...
   명령 레이블에 **&amp;**&#x200B;이 있는 경우 &#39;I&#39; 문자로 키보드 단축키가 선언됩니다.
...
폴더 유형 선언은 **`<model>`** 요소 아래에 입력해야 합니다. 이 요소를 사용하면 **[!UICONTROL Add new folder]** 메뉴에서 볼 수 있는 계층 조직을 정의할 수 있습니다. **`<model>`** 요소에는 **`<nodemodel>`** 요소와 기타 **`<model>`** 요소가 포함되어야 합니다.

**name** 및 **label** 속성은 요소의 내부 이름과 **[!UICONTROL Add new folder]** 메뉴에 표시되는 레이블을 채웁니다.

**`<nodemodel>`** 요소에는 다음 속성을 갖는 폴더 유형에 대한 설명이 포함되어 있습니다.

* **이름**: 내부 이름
* **레이블**: 폴더를 삽입할  **[!UICONTROL Add new folder]** 때 및 기본 레이블로 사용되는 레이블입니다.
* **img**: 폴더 삽입의 기본 이미지입니다.
* **hiddenCommands**: 마스크할 명령 목록(쉼표로 구분) 가능한 값: &quot;adbnew&quot;, &quot;adbsave&quot;, &quot;adbcancel&quot; 및 &quot;adbdup&quot;.
* **newFolderShortCuts**: 폴더 작성 시 모델(**`<nodemodel>`** 쉼표로 구분)의 바로 가기 목록입니다.
* **insertRight**,  **editRight**,  **deleteRight**: 폴더 삽입, 편집 및 삭제에 대한 권한.

**`<nodemodel>`** 요소 아래의 **`<view>`** 요소에는 보기와 연결된 목록의 구성이 포함되어 있습니다. 목록의 스키마가 **`<view>`** 요소의 **스키마** 속성에 입력됩니다.

목록의 레코드를 편집하려면 목록 스키마와 이름이 같은 입력 양식이 암시적으로 사용됩니다. **`<view>`** 요소의 **type** 속성은 양식 표시에 영향을 줍니다. 가능한 값은 다음과 같습니다.
...
>[!NOTE]
>
>입력 양식의 이름은 **`<view>`** 요소에 **form** 속성을 입력하여 오버로드될 수 있습니다.

목록 열의 기본 구성은 **`<columns>`** 요소를 통해 입력됩니다. 열은 스키마에서 해당 값으로 참조할 필드가 있는 **xpath** 특성이 들어 있는 **`<node>`** 요소에 선언됩니다.
...
명령 설명은 다음 속성을 사용하여 **`<command>`** 요소에 입력됩니다.
...
연결된 폴더의 경우 **`<nodemodel>`** 요소의 **folderLink** 속성을 채워야 합니다. 이 속성에는 데이터 스키마에 구성된 폴더에 있는 링크의 이름이 포함되어 있습니다.
...
&quot;folder&quot;라는 폴더의 링크에서 **`<nodemodel>`**&#x200B;의 구성은 다음과 같습니다.
