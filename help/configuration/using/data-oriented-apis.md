...
데이터 모델의 프레젠테이션은 [Adobe Campaign 데이터 모델 설명](../../configuration/using/data-model-description.md)을 참조하세요.
...
트랜잭션 API를 사용하면 **updateOrInsert** 명령을 통해 조정을 관리할 수 있습니다. 하나의 명령을 사용하면 데이터를 만들거나 업데이트할 수 있습니다. 수정 병합(**병합**)을 구성할 수도 있습니다. 이 운영 모드에서는 부분 업데이트를 승인할 수 있습니다.
...
쿼리는 **schema** 특성에서 시작 스키마를 참조해야 합니다.

원하는 작업 유형이 **operation** 특성에 입력되었으며 다음 값 중 하나를 포함합니다.

* **get**: 테이블에서 레코드를 검색하고 데이터가 없으면 오류를 반환합니다.
* **getIfExists**: 테이블에서 레코드를 검색하고 데이터가 없으면 빈 문서를 반환합니다.
* **select**: 여러 레코드를 반환하는 커서를 만들고 데이터가 없으면 빈 문서를 반환합니다.
* **count**: 데이터 개수를 반환합니다.

**XPath** 구문을 사용하여 입력 스키마를 기준으로 데이터를 찾습니다. XPath에 대한 자세한 내용은 [데이터 스키마](../../configuration/using/data-schemas.md)를 참조하세요.
...
반환되는 레코드 수를 제한하려면 **lineCount** 특성을 `<querydef>` 요소에 추가하십시오.
...
>[!NOTE]
>
>다시 이전 예제의 조건을 사용합니다. `<select>` 및 절을 사용하지 않습니다. `</select>`
...
* 컬렉션 링크(1N): 컬렉션 테이블의 필드에 대한 필터링은 **EXISTS** 또는 **NOT EXISTS** 연산자를 통해 수행해야 합니다.
  ...
  쿼리가 카디날 제품을 반환하므로 `<select>` 절에서 컬렉션 링크의 필드를 직접 검색하지 않는 것이 좋습니다. 연결된 테이블에 하나의 레코드만 있는 경우에만 사용됩니다(예: `<node expr="">`).
  ...
  `<select>` 절에서 컬렉션 링크의 요소가 포함된 하위 목록을 검색할 수 있습니다. 참조된 필드의 XPath는 컬렉션 요소의 컨텍스트에 따라 다릅니다.
...
업데이트할 엔터티의 스키마 키가 **xtkschema** 특성을 기반으로 완료되었습니다.

따라서 키를 구성하는 XPath 목록을 포함하는 **_key** 특성을 사용하여 조정 키를 강제 적용할 수 있습니다(쉼표로 구분).

**_operation** 특성을 다음 값으로 채워서 작업 형식을 강제 적용할 수 있습니다.

* **insert**: 레코드를 강제로 삽입합니다(조정 키가 사용되지 않음).
* **insertOrUpdate**: 조정 키(기본 모드)에 따라 레코드를 업데이트하거나 삽입합니다.
* **업데이트**: 레코드를 업데이트합니다. 데이터가 없으면 아무 작업도 하지 않습니다.
* **삭제**: 레코드를 삭제합니다.
...
기본 엔터티(&quot;nms:recipient&quot;)의 키 정의는 연결된 테이블(요소 `<folder>` 스키마 &quot;xtk:folder&quot;) 및 전자 메일의 필드로 구성됩니다.
