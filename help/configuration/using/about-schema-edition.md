---
title: 스키마 에디션 기본 정보
seo-title: 스키마 에디션 기본 정보
description: 스키마 에디션 기본 정보
...
작업할 스키마를 확장하거나 만든 경우, XML 내용 요소를 아래에서 표시되는 순서대로 정의하는 것이 좋습니다.

## 열거형 {#enumerations}

열거형은 스키마의 주 요소 앞에 먼저 정의됩니다. 이 매개 변수를 사용하면 지정된 필드에 대한 사용자의 선택 사항을 제한하기 위해 목록에 값을 표시할 수 있습니다.
...
이러한 필드는 고유하거나 고유하지 않을 수 있으며 하나 이상의 필드를 참조할 수 있습니다.
...
>[!IMPORTANT]
>
>대부분의 속성은 데이터베이스의 물리적 필드에 1-1 카디널리티에 따라 연결되어 있지만 XML 필드나 계산 필드의 경우에는 그렇지 않습니다.\
>XML 필드는 표의 메모 필드(&quot;mData&quot;)에 저장됩니다.\
>하지만 계산된 필드는 쿼리가 시작될 때마다 동적으로 만들어지므로 응용 프로그램 레이어에만 존재합니다.
...
수신자 테이블(기본 스키마)과 사용자 지정 트랜잭션 테이블 간의 1N 링크의 예:

```
<element label="Recipient" name="lnkRecipient" revLink="lnkTransactions" target="nms:recipient" type="link"/>
```

사용자 지정 스키마 &quot;Car&quot;(&quot;cus&quot; 네임스페이스에 있음)와 받는 테이블 간의 1-1 링크의 예:
...
스키마 하단에 포함할 유용한 요소 중 하나는 추적 요소입니다(감사 추적).
