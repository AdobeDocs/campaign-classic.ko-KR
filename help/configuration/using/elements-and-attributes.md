...
요소에 정의된 `<attribute>` 요소는 데이터베이스의 필드 생성 시퀀스에 `<srcschema>` 영향을 주지 않습니다. 작성 시퀀스는 알파벳 순으로 표시됩니다.
...
정의된 항목 `<compute-string>` 이 없으면 `<compute-string>` 기본적으로 스키마의 기본 키 값으로 요소가 입력됩니다.
...
&quot;id&quot; 필드에 색인 만들기 (요소의 &quot;@unique&quot; 속성은 데이터베이스(쿼리)에서 색인을 만들 때 &quot;UNIQUE&quot; SQL 키 단어의 `<dbindex>` 추가를 트리거합니다.)
...
요소는 상위 요소 `<join>` 가 &#39;link&#39; 유형인 경우에만 사용할 수 `<element>` 있습니다. 즉, 상위 요소에 &quot;@type=link&quot; 특성이 선언되어야 합니다.

요소에 있는 원격 테이블의 이름과 네임스페이스를 지정할 필요는 `<join>` 없습니다. 상위에 지정해야 합니다 `<element>`.

규칙으로, 링크는 스키마의 끝에 정의됩니다.

링크 유형 요소를 정의할 때 요소를 지정하지 않으면 두 테이블의 기본 키에 링크가 자동으로 배치됩니다. `<join>`
...
STRING 형식의 &quot;이름&quot; 필드 및 일치하는 SQL 쿼리 `<srcschema>` 의 기본 키 선언:
...
이 요소를 사용하면 SOAP 메서드를 호출하는 매개 변수를 정의할 수 있습니다.
...
이 요소는 요소의 단일 `<param>` 하위 요소에 대해서도 `<method>` 필수입니다.
...
스키마 `<srcschema>` 의 루트 요소입니다. 스키마 정의에 대한 입력 점입니다.

### 사용 및 사용 상황 {#use-and-context-of-use-9}

스키마 프레젠테이션은 스키마 참조 [및 스키마 구조](../../configuration/using/about-schema-reference.md) 정보에서 사용할 수 [있습니다](../../configuration/using/schema-structure.md).
