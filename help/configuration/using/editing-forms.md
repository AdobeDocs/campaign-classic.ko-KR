...
이 예제에서 컨테이너는 **및** 입력 필드를 구성하는 **[!UICONTROL Created by]** Creation **[!UICONTROL Name]** 섹션을 정의합니다.
...
이 예제에서는 `book.png` 네임스페이스의 `detail.png` 및 `ncm` 이미지에 대한 참조를 보여 줍니다.
...
1. `type` 요소의 `<form>` 특성을 `iconbox`(으)로 변경합니다.
...
1. 각 양식 페이지에 대한 컨테이너를 설정합니다.
   ...
   또는 기존 `type="frame"` 요소에서 `<container>` 특성을 제거합니다.
...
1. `type` 요소의 `<form>` 특성을 `notebook`(으)로 변경합니다.
...
이 예제에서는 `iRecipientId` 스키마의 기본 키(`nms:recipient`)가 읽기 전용 액세스로 표시됩니다.
...
1. JS 파일에 유효성 검사 함수를 만듭니다.
   ...
   이 예제에서 함수의 이름은 `checkValue`입니다. 이 함수는 `recipient` 네임스페이스에서 `nms` 데이터 형식을 확인하는 데 사용합니다. 확인 중인 값이 기록됩니다. 값이 올바르지 않으면 오류 메시지가 기록됩니다. 값이 유효하면 값 1이 반환됩니다.
