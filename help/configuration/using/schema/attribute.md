...
`<srcschema>`에서 `<attribute>` 요소가 정의된 순서는 데이터베이스의 필드 만들기 순서에 영향을 주지 않습니다. 생성 순서는 알파벳순입니다.
...
* **길이(문자열)**: 최대 &quot;문자열&quot; 유형 SQL 필드의 값에 대한 문자 수입니다. &quot;@length&quot; 속성을 지정하지 않으면 Adobe Campaign에서 255자에 대한 필드를 자동으로 만듭니다.
* **localizable(부울)**: 활성화되면 이 특성은 컬렉션 도구에 변환을 위해 &quot;@label&quot; 특성의 값을 복구하도록 알려줍니다(내부 사용).
* **이름(MNTOKEN)**: 테이블의 필드 이름과 일치하는 특성의 이름입니다. &quot;@name&quot; 속성의 값은 짧아야 하며, 영어가 가급적이면 XML 이름 지정 제한을 준수해야 합니다.
...
* **sql(부울)**: 이 특성이 활성화된 경우(@sql=&quot;true&quot;), 특성을 포함하는 요소에 xml=&quot;true&quot; 속성이 있더라도 이 특성을 강제로 SQL 특성을 저장합니다.
* **sqlDefault(문자열)**: 이 특성을 사용하면 @notNull 특성이 활성화된 경우 데이터베이스 업데이트를 위해 고려할 기본값을 정의할 수 있습니다. 속성을 만든 후 이 속성을 추가하면 새 레코드에 대해서도 스키마 동작이 변경되지 않습니다. 스키마를 변경하고 새 레코드의 값을 업데이트하려면 속성을 삭제하고 다시 만들어야 합니다.
* 테이블을 만드는 동안 필드의 **sqlname(문자열)**: @sqlname을 지정하지 않으면 기본적으로 &quot;@name&quot; 속성 값이 사용됩니다. 스키마가 데이터베이스에 작성되면 필드 유형에 따라 접두사가 자동으로 추가됩니다.
* **템플릿(문자열)**: 이 특성은 여러 스키마에서 공유하는 `<attribute>` 요소에 대한 참조를 정의합니다. 이 정의는 현재 스키마에 자동으로 복사됩니다.
* **translatedDefault(문자열)**: &quot;@default&quot; 특성이 발견되면 &quot;@translatedDefault&quot;을 사용하여 @default에 정의된 것과 일치하도록 식을 재정의하여 번역 도구에서 수집할 수 있습니다(내부 사용).
* **translatedExpr(문자열)**: &quot;@expr&quot; 특성이 있으면 &quot;@translatedExpr&quot; 특성을 사용하여 @expr에 정의된 것과 일치하도록 식을 재정의하여 번역 도구에서 수집할 수 있습니다(내부 사용).
* **유형(MNTOKEN)**: 필드 유형입니다.
