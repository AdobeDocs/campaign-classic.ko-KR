...
* **revCardinality(문자열)**: 이 속성은 두 테이블 간의 링크의 카디널리티를 정의합니다. &quot;링크&quot; 유형 `<element>`에 사용됩니다.
...
* **revLink(문자열)**: 반대 링크의 이름입니다. 값이 &quot;_NONE_&quot;이면 대상 스키마에 반대 링크가 만들어지지 않습니다.
...
* **tableSpace(문자열)**: 이 속성을 사용하면 테이블(루트에서 유효)에 대해 테이블스페이스를 저장하는 새 데이터를 지정할 수  `<element>`있습니다.
...
* **템플릿(문자열)**: 이 속성은 여러 스키마에서 공유되는  `<element>` 요소에 대한 참조를 정의합니다. 정의가 현재 스키마에 자동으로 복사됩니다.
