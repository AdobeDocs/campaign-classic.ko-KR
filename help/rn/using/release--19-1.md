...
* Debian 9에서 SFTP **키 기반** 인증이 작동하지 않는 파일 전송 작업 문제를 수정했습니다. (NEO 파섹)
* 고주파로 전송할 때 푸시 알림에 영향을 줄 수 있는 문제를 수정했습니다. (NEO 파섹)
* 웹 서버 충돌을 야기할 수 있는 오퍼 응답 관리 문제를 수정했습니다. (NEO 파섹)
* 보고서를 내보내지 못하게 하는 LibreOffice 관리의 오류를 수정했습니다. (NEO 파섹)
* 설문 조사 활동을 사용하여 다양한 워크플로우를 업그레이드할 때 오류가 발생하는 문제를 해결했습니다.
* .odt 파일을 사용한 이메일 미리 보기 오류가 발생하지 않도록 LibreOffice 관리를 개선했습니다.
...
* 정리 워크플로우 로그에서 사소한 회귀 문제를 수정했습니다.
