...
* 로그 파일에서 빈 줄을 관리하여 MTA 프로세스 동작에 오류를 발생시키고 전달 시 성능이 저하되는 문제를 해결했습니다.
...
Linux의 경우, 이제 서버 서비스 시작 시 /etc/init.d/nlserver6 스크립트 대신 시스템 단위를 사용합니다. 새 시작 구성표에 대한 마이그레이션은 19.1.8 패키지를 설치할 때 자동으로 수행됩니다. /etc/init.d/nlserver6는 여전히 제공되지만, nlserver 서비스와의 상호 작용(시작, 다시 시작, 중지 등)을 위해 systemctl 명령을 직접 사용하는 것이 좋습니다.
...
* 업그레이드 후 게시 후 사용할 수 없는 회귀 문제를 해결했습니다.
...
* 일부 API 호출이 실행되지 않도록(RetrieveAllEntities)할 수 있는 Microsoft Dynamics(버전 8.2에서) 호환성 문제를 수정했습니다. (NEO-24528)
...
* 초기화되지 않은 변수(m_pCurlReader)로 인해 Azure Blob 저장소 외부 계정을 사용하여 파일 전송 활동에 파일을 업로드할 수 없는 문제를 수정했습니다. (NEO-13717)
...
* SQL 데이터 관리 작업(&quot;SQL 데이터 관리&quot; 이름이 오른쪽인 &quot;SQL 데이터 관리&quot;가 누락됨)을 완전히 사용할 수 없는 문제를 수정했습니다.
...
>[!NOTE]
>
>19.1.4 Gold Standard 릴리스는 이 [페이지](../../rn/using/gold-standard.md)에 나열되어 있습니다.
 ...
 <tbody> 
  <tr> 
   ...
   <td> <p>관리 사용자로서의 작업의 효율성을 높이려면 저장소를 모니터링하여 SFTP 서버의 설정을 관리하고,에 IP 주소를 추가하고, 각 인스턴스에 허용 목록에 추가하다 대한 SSH 키를 설치하도록 합니다. Campaign 컨트롤 패널은 오늘부로 AWS에서 호스팅되는 고객에게만 제공됩니다(<a href="https://experiencecloud.adobe.com/campaign/controlpanel/">오늘 Experience Cloud을 통해 로그인</a>).</p> <p>자세한 내용은 <a href="https://docs.adobe.com/content/help/ko-KR/control-panel/using/control-panel-home.html">세부 설명서</a> 및 <a href="https://docs.adobe.com/content/help/ko-KR/campaign-classic-learn/control-panel/control-panel-overview.html">방법 비디오</a>를 참조하십시오. </p><p>참고:campaign 컨트롤 패널에 액세스하려면 최신 캠페인 빌드로 업그레이드할 필요가 없습니다.</p> </td> 
    ...
    <tr> 
   <td> 감사 추적<br /> </td> 
   <td> <p>관리자는 Adobe Campaign Classic 인스턴스 내에서 수행된 변경 사항을 모니터링하고 관리하여 생산성을 높일 수 있습니다. 감사 추적은 소스 스키마, 워크플로우 및 옵션에 대한 작업을 기록합니다. 요소가 만들어졌는지, 수정되었는지 또는 삭제되었는지 빠르게 확인할 수 있습니다.</p><p>자세한 내용은 <a href="../../production/using/audit-trail.md">자세한 설명서</a> 및 <a href="https://docs.adobe.com/content/help/en/campaign-classic-learn/tutorials/monitoring/audit-trail.html">how-to video</a>를 참조하십시오.</p></td> 
  </tr> 
  <tr> 
   <td> Guardelail, Robustness &amp; Scalability<br /> </td> 
   <td> Campaign Classic에 일련의 개선 사항이 추가되었습니다. Guardrail, Robustness 및 확장성 개선 사항은 아래에 나와 있습니다.<br /> </td> 
  </tr> 
  <tr> 
   <td> 호환성 매트릭스 업데이트<br /> </td> 
   <td> 이제 Adobe Campaign에서 이 새 버전을 사용하여 다음 데이터베이스 시스템을 지원합니다. <a href="https://helpx.adobe.com/kr/campaign/kb/compatibility-matrix.html">호환성 매트릭스</a>를 참조하십시오.<br /> 
    <ul> 
...
* 보안상의 이유로 **[!UICONTROL Data loading (file)]** 워크플로우 활동에서 **[!UICONTROL Pre-process the file]** 옵션을 사용할 때 임의의 명령을 삽입할 수 없습니다. 이제 3가지 옵션 중에서 선택할 수 있는 드롭다운 목록을 사용할 수 있습니다.**[!UICONTROL None]**, **[!UICONTROL Decompression]**(zcat) 또는 **[!UICONTROL Decrypt]**(gpg) XtkSecurity_Disable_Preproc 보안 플래그가 추가되었습니다. 새 고객의 경우 이 옵션이 0으로 설정됩니다. 기존 고객의 경우 이전 동작을 유지하기 위해 업그레이드 후 이 옵션이 1로 설정됩니다. 이 [섹션](../../workflow/using/data-loading--file-.md)을 참조하십시오.
...
* 이제 디메일 배달을 분석하는 동안 배포 마법사에서 게시 모드가 **[!UICONTROL None]**&#x200B;으로 설정된 경우 오류가 기록되고 분석이 중지됩니다.&quot;게시 모드가 &#39;없음&#39;으로 설정되었습니다.이미지를 포함할 수 없습니다. 기능폰에는 이미지가 표시되지 않습니다.&quot; (NEO-12208)
* 트랜잭션 메시징에 대한 브로드캐스트 로그 관리가 개선되었습니다. 브로드로그가 실행 인스턴스에서 제어 인스턴스로 동기화되면 @lastModified 필드가 시스템의 현재 날짜로 업데이트됩니다. 제어 인스턴스에 대해 MC_Update_BlLastModified 옵션이 추가되었습니다. True는 컨트롤 인스턴스(기본 비헤이비어)에서 현재 날짜가 사용됩니다. False는 실행 인스턴스 브로드로그의 @lastModified 날짜를 사용함을 의미합니다. (NEO-12579)
...
* 트랜잭션 푸시 알림에 대한 잠재적인 반복 서버 충돌을 수정했습니다. 다른 충돌 문제가 수정되었습니다.
* 열려 있는 배달 표시기에 대한 사용자 활동과 추적 보고서 간 보고가 불일치하던 문제를 수정했습니다. (NEO-11742)
* IMS 로그인 문제가 해결되었습니다.
* 라이브러리에서 이미지를 추가할 때 배달에 이미지가 누락되는 문제가 해결되었습니다. (NEO-11900)
* 직접 메일 배달에서 오퍼 세부 정보를 추출할 때 발생할 수 있는 문제를 수정했습니다. (NEO-11700)
* SMS 트랜잭션 메시지 성능에 영향을 줄 수 있는 문제를 수정했습니다. (NEO-9812)
* 배달의 기본 대상에 대해 외부 파일에서 정의 옵션을 사용할 때 발생할 수 있는 콘솔 충돌을 해결했습니다. (NEO-12349)
* 일본어(.JP) 도메인에 대해 대상 받는 사람을 분석할 때 발생하는 문제를 수정했습니다. (NEO-12246)
* 1:N 링크와 함께 값 배포를 사용할 때의 표시 문제를 수정했습니다. (NEO-12212, NEO-11820)
* 업그레이드 후 MTA 로그에서 NmsMxDomain 오류가 발생하는 문제를 수정했습니다. (NEO-12752)
* 농축 워크플로우 활동에서 &quot;기본 세트에 모든 추가 데이터 유지&quot; 옵션을 사용할 때 발생하는 문제가 해결되었습니다. (NEO-13291)
* HTTP2를 사용하여 푸시 알림을 전송할 때 발생하는 Tomcat 충돌 문제가 해결되었습니다. (NEO-12701)
* 모든 콜백이 완료될 때까지 기다리지 않던 HTTPRequest API 관련 문제를 수정했습니다. (NEO-12628)
* 트랜잭션 메시지에 대한 지표 추적 프로세스의 계산 처리 문제를 수정했습니다. (NEO-12529)
* 오퍼 관리에서 테마를 사용하는 문제가 해결되었습니다. (NEO-11804)
* 푸시 알림을 전송할 때 발생하는 성능 문제가 해결되었습니다. (NEO-11787)
* 직접 메일 배달을 위해 오퍼 관리에서 아웃바운드 XML 또는 CSV 파일을 미리 볼 때 문제를 해결했습니다. (NEO-11290)
* **소셜 네트워크 관리**(소셜 마케팅) 패키지를 설치할 때 발생하는 문제가 해결되었습니다. (NEO-12081)
* 올바른 액세스 권한이 있더라도 웹 애플리케이션을 삭제하지 못했던 문제를 수정했습니다. (NEO-12072)
* XML을 통해 객체를 내보내고 가져올 때 일부 값을 덮어쓸 수 있는 문제를 수정했습니다. XtkExport_IncludeDefaultValues 옵션이 추가되었습니다. True(기본 비헤이비어)로 설정하면 모든 값이 내보내집니다. False로 설정하면 수정 내용이 기본값으로 덮어쓰여집니다. (NEO-11979)
* 쿼리 후에 추가 활동이 추가되면 **[!UICONTROL Alert]** 워크플로우 활동이 실패하는 문제를 해결했습니다. (NEO-12132)
...
* 가설 보고서의 통화 기호에 대한 표시 문제를 일으킬 수 있는 문제를 수정했습니다. (NEO-11634)
* 추적 링크에서 특정 문자를 사용할 때 리디렉션 및 추적 문제가 수정되었습니다.
...
* 배달에서 시드 주소를 사용할 때 ID 소모 문제가 수정되었습니다. (NEO-11842)
* phantomjs 및 Debian 9 호환성 문제를 해결했습니다.
* 증명 내용을 승인할 때 발생하는 오류가 수정되었습니다. (NEO-12725)
* &quot;모집단에서 이 하위 집합 제외&quot; 워크플로우 기능 관련 문제를 수정했습니다. (NEO-12441)
* 모든 콜백이 완료될 때까지 기다리지 않던 HTTPRequest-wait API 관련 문제를 수정했습니다. (NEO-12628)
* 분할 활동에서 &quot;공유 대상 업데이트&quot; 작업의 문제를 수정했습니다. (NEO-11562)
* 웹 서버 충돌 문제가 수정되었습니다. (NEO-12904)
* 거래 템플릿의 자연 매개 변수 관련 문제를 수정했습니다. (NEO-12334)
* 이메일 텍스트 편집기에서 추적된 URL을 표시할 때 콘솔 충돌 문제가 수정되었습니다. (NEO-13122)
...
* 잘못된 형식의 URL에 대해 추적 워크플로가 실패하던 문제를 수정했습니다.
* 모바일 응용 프로그램 패키지를 가져온 후 외부 계정 문제를 수정했습니다.
* 연산자에 표준 시간대를 할당할 때 발생하는 문제가 해결되었습니다. (NEO-12464)
* 일치 필드 로그에서 오류가 발생할 수 있는 문제를 수정했습니다. (NEO-11539, NEO-8978)
