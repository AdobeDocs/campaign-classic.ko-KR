...
![](assets/green_2.png) 2020 **년 4월 3일**:새 빌드(9032-...e8b36257e). 다음 수정 사항을 포함합니다.

* Adobe는 잠재적인 악성 사용(피싱)을 방지하기 위해 이메일의 링크를 추적하는 서명 메커니즘을 도입했습니다. 이렇게 하면 사용자를 리디렉션하는 데 사용되는 URL이 포함될 수 있는 추적 매개 변수의 재작성으로부터 보호할 수 있습니다. 이 메커니즘은 현재 기본적으로 비활성화되어 있습니다. 활성화해야 하는 경우 고객 지원 센터에 문의하십시오.

* 이전 빌드에서 생성된 잘못된 형식의 URL을 리디렉션하거나 서명 메커니즘이 꺼져 있는 경우를 방지하기 위해 보조 보안 보호가 추가되었습니다. 사용해야 하는 경우 고객 지원 센터에 문의하십시오.

![](assets/orange_2.png) 2020 **년 3월 5일**:다음 수정 사항이 포함된 새 빌드(9032-...205c981c3)를 참조하십시오.
...
* Database size - XtkJobLog: a purge mechanism has been added to this table. 이는 데이터베이스 크기에 긍정적인 영향을 줍니다.
* BCC archiving: the default parameters for BCC archiving have been changed to increase archiving velocity. [자세한 내용](../../installation/using/email-archiving.md#parameters)
...
* Guardrails for operator actions: several guardrails have been implemented to prevent operators from performing actions which could impact the platform&#39;s integrity. 기본 제공 스키마는 더 이상 인터페이스를 통해 삭제할 수 없습니다. Also, the workflow source XML can no longer be edited by non-admin users.
* Two new options have been made available: **XtkSecurity_Restrict_EditXML** (allows you to disable the edition of deliveries’ XML code) and **NmsOperation_OperationMgtDebug** (allows you to monitor the operationMgt technical workflow execution). [자세한 내용](../../installation/using/configuring-campaign-options.md)
...
* The broadlog management has been improved for transactional messaging. When broadlogs are synchronized from the execution instance to the control instance, the @lastModified field is updated to the system’s current date. The MC_Update_BlLastModified option has been added for control instances. True는 현재 날짜가 제어 인스턴스(기본 동작)에 사용됨을 의미합니다. False means that we use the execution instance broadlog’s @lastModified date. (NEO-12579)
...
* You can now limit Write access to Update data activity in Workflow using sysfilter attributes. [자세한 내용](../../configuration/using/filtering-schemas.md)

**패치**

* Fixed an issue which prevented the certificate from being uploaded for iOS mobile push notifications.
* Fixed potential recurrent server crashes for transactional push notifications. 다른 충돌 문제가 해결되었습니다.
