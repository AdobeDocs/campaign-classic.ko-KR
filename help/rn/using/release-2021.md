...
* 과금 워크플로를 개선했습니다. 이 문제 해결은 빌드 9342에서 사용할 수 있으며, 적용하려면 워크플로를 수동으로 다시 시작해야 합니다. 이제 업그레이드 후 워크플로가 자동으로 다시 시작됩니다.
...
<tr>
<td>
<p>새로 추가된 <b>데이터 소스 변경</b> 워크플로 활동을 사용하면 워크플로의 작업 테이블의 데이터 소스를 변경할 수 있습니다. 이를 통해 다양한 데이터 소스(FDA 및 로컬 데이터베이스)에서 데이터를 유연하게 관리할 수 있습니다.</p>
...
* 전체 데이터베이스 연결 세부 정보를 반환하는 **xtk:session#GetCnxInfo** API 메서드에 대한 액세스는 이제 관리 사용자만 제한됩니다. (NEO-27779)
...
* [과금 기술 워크플로](../../production/using/monitoring-processes.md#billing-report)를 마케팅 인스턴스에서만 실행할 수 있도록 하기 위한 가드레일을 추가했습니다.
...
* HTTPs 연결을 통해 FDA를 사용할 때 **중간 소싱(게재 로그)**(defaultMidSourcingLog) 워크플로가 **NmsMidSourcing_LogsPeriodHour** 옵션에 의해 설정된 시간대에 중단되었습니다. 이렇게 하면 이 설정 일정 후에 발생한 데이터로 레코드가 업데이트되지 않습니다. (NEO-30833)
* 메시지 센터 동기화 워크플로를 실행한 후 발생하는 문제를 해결했습니다. 게재 개체 폴더를 사용자 지정 폴더로 이동할 때마다 워크플로는 게재를 일반 **트랜잭션 메시지 기록** 폴더로 다시 이동합니다. (NEO-27445)
* **브로드캐스트 통계**, **추적 표시기** 및 **공유 활동 통계** 보고서를 표시하려고 할 때 오류 메시지가 표시되는 문제를 수정했습니다.
* **Oracle On Demand** 워크플로 활동이 Oracle CRM 커넥터 사용 중단 후 인터페이스에서 제거되었습니다.
* 워크플로 서버(wfserver) 모듈이 매일 다시 시작한 후 처리 워크플로 실행이 중지되는 문제를 해결했습니다. (NEO-30047)
...
* 웹 애플리케이션을 사용할 때 추적 태그가 **nms:trackingURL** 스키마에 잘못된 기본 키를 생성하는 문제를 해결했습니다. (NEO-27931)
...
* 여러 작업으로 워크플로를 실행 시 시간 초과를 방지하기 위해 Workflow Heatmap을 개선했습니다. (NEO-27423).
...
* 새로 만든 워크플로의 이름이 저장 후 기본값으로 대체되는 새로 고침 문제를 해결했습니다(NEO-26106).
* 워크플로를 실행할 때 **외부 파일** 타겟 매핑을 사용하여 **게재** 활동 이전에 **데이터 보강** 활동의 일부로 새 필드가 추가되는 문제가 해결되었습니다. 원치 않는 필드가 **외부 파일** 타겟 매핑에 추가되었습니다. (NEO-27687)
...
* Redshift xtk 함수를 사용할 때 발생하는 워크플로 쿼리 문제를 해결했습니다. 이제 SubDays, SubSeconds, SubMinutes 및 SubHours는 두 Redshift 타임스탬프 유형(NEO-24962)을 모두 허용합니다.
...
* 워크플로에서 **JavaScript** 활동을 사용하여 HTTP 요청을 할 때 발생하는 문제를 해결했습니다. 호스트 이름에서 포트 번호를 정의한 후 다음 오류로 호출이 실패합니다(NEO-29146).
...
* 다음 오류로 인해 **추적** 워크플로가 실패하던 문제를 해결했습니다(NEO-25206).

```
There is no index on the sourceId field of the 'NmsTrackingLogRcp' table required for the current Web tracking mode. Please add this index.
```

* 캠페인의 **타겟팅 및 워크플로** 탭 내에서 게재를 만들고 저장할 때 발생하는 문제를 해결했습니다. 다음 오류로 미리 보기가 실패합니다(NEO-29440).
