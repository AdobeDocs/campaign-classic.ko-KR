...
이 섹션에는 Campaign Classic에서 제거된 기능 및 기능이 나열됩니다.
...
Adobe Campaign Classic 클라이언트 콘솔은 편집기에서 더 이상 사용하지 않으므로 다음 시스템에서 더 이상 실행할 수 없습니다. 이러한 버전 중 하나에서 캠페인 클라이언트 콘솔을 실행하는 고객은 대상 제거 날짜 전에 최신 버전으로 업그레이드해야 합니다. 호환성 [매트릭스를 참조하십시오](https://helpx.adobe.com/campaign/kb/compatibility-matrix.html).
...
Adobe는 편집기에서 더 이상 사용하지 않으므로 다음 데이터베이스 엔진을 지원하지 않습니다. 이러한 버전을 실행하는 고객은 최신 버전으로 업그레이드하거나 다른 버전으로 이동해야 합니다.

호환 버전 목록에 액세스하려면 [Campaign Classic](https://helpx.adobe.com/campaign/kb/compatibility-matrix.html) 호환성 매트릭스를 참조하십시오.
