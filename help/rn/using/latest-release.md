...
* IMS API의 보안이 개선되었습니다. 클라이언트에 민감한 정보(즉, 액세스 토큰)가 URL 매개 변수에서 제거되었습니다. 이제 이러한 자격 증명이 post 데이터 또는 인증 헤더로 전송되어 보다 안전한 통신 프로세스가 보장됩니다. (NEO-63045)
* DDOS 공격을 방지하기 위해 웹 앱의 보안이 개선되었습니다. (NEO-50757)
* PII 데이터가 웹 로그 오류에 노출되지 않도록 보안이 향상되었습니다. (NEO-46827)
* 보안 토큰이 Campaign 홈페이지 URL에 포함되지 않도록 보안이 최적화되었습니다. (NEO-38519)
...
* 새 항목 `cusHeader` 서버 구성 파일(serverConf.xml)에 섹션이 추가되었습니다. 외부 서버에서 파일을 업로드할 때 사용자 지정 헤더를 추가할 수 있습니다. (NEO-58339) [자세히 보기](../../installation/using/the-server-configuration-file.md#cusheaders).
* lastMsgId에 대한 부정적인 ID를 방지하기 위해 추적 로그 관리가 개선되었습니다. int32에서 int64로 변경되었습니다. (NEO-52290)
* 중간 소싱(게재 통계) 워크플로우가 즉시 추가되었습니다. 이 새 워크플로우는 게재 통계 데이터(nms:deliveryStat)를 mid에서 마케팅 인스턴스로 동기화합니다. (NEO-36802)
...
* 여러 로그 스키마와 연결된 대상 스키마를 사용할 때 청구 워크플로우에서 활성 연락처 계산이 제대로 작동하지 않는 회귀 문제를 해결했습니다. (NEO-61468)
...
* Big Query 커넥터를 타임스탬프 또는 날짜/시간 필드와 함께 사용할 때 발생하는 문제를 수정했습니다. (NEO-59502, NEO-49768)
* 누적 게재 보고서를 사용할 수 없는 문제를 해결했습니다. (NEO-59211)
* People 핵심 서비스와 대상자를 공유할 때 오류가 발생하는 문제를 해결했습니다. (NEO-58637)
* 게재의 미러 페이지를 표시할 때 발생하는 문제를 해결했습니다. (NEO-58325)
* XtkLibrary.Right() xtk 표현식이 작동하지 않는 문제를 해결했습니다. (NEO-57870)
* 디지털 콘텐츠 편집기에서 이미지를 업로드할 때 본문 태그의 스타일 특성이 변경되는 문제를 해결했습니다. (NEO-57697)
* CRM 커넥터 활동으로 일괄 내보내기를 수행할 때 특수 문자 문제를 해결했습니다. (NEO-54300)
* 데이터 로드 활동 및 빅 쿼리 커넥터를 사용할 때 대량 로드가 &quot;문자열&quot; 데이터 유형에서 작동하지 않는 문제를 해결했습니다. (NEO-53748)
* 오퍼 렌더링 문제로 이어질 수 있는 캐시 키 문제를 수정했습니다. (NEO-51516, NEO-49995)
...
* 푸시 알림을 전송할 때 발생하는 성능 문제를 해결했습니다. (NEO-49953) 보고서를 내보낼 때 일본어 문자가 잘못 표시되는 문제를 해결했습니다(NEO-49308).
* Tomcat 오류 보고서에 너무 많은 오류 세부 정보가 표시되는 문제를 해결했습니다. (NEO-49029)
* 많은 수의 오퍼를 사용할 때 게재 오류가 발생할 수 있는 문제를 해결했습니다. (NEO-48807)
* 을(를) 방지할 수 있는 문제를 해결했습니다. **데이터 업데이트** 워크플로우 활동이 올바르게 작동하지 않습니다. (NEO-48140)
...
* 을(를) 막을 수 있는 문제를 해결했습니다. `<img-amp>` 이메일 콘텐츠 작업 태그. (NEO-38685)
* HTTP 릴레이를 사용할 때 메시지 센터 보관 워크플로우가 실패하는 문제를 해결했습니다. (NEO-33783)
* 이메일 콘텐츠 편집기에서 글꼴 이름 및 크기 오류가 발생할 수 있는 문제를 해결했습니다. (NEO-61342)
