...
* 을 구성할 때 발생하는 문제를 수정했습니다 **웹 분석** 오류가 발생한 경우에도 통합 상태가 항상 &quot;통합에 성공했습니다&quot;고 표시되는 외부 계정입니다. (NEO-36672)
* 음수 ID가 있을 때 시퀀스 ID 메커니즘과 관련된 몇 가지 업그레이드 후 오류가 수정되었습니다. (NEO-43205, NEO-42846, NEO-42845)
* 를 사용할 때 발생하는 문제를 수정했습니다 **웹 분석** 반복 및 연속 게재를 사용하는 외부 계정으로서, 외부 계정의 데이터가 부분적으로 유실됩니다. (NEO-38548)
* NmsActiveContact 테이블을 업데이트할 때 업그레이드 후 속도가 느려지는 문제를 해결했습니다. (NEO-43206)
* 기본 제공 폴더가 **관리** 노드를 다른 위치에 추가합니다. (NEO-42875)
* 를 사용할 때 발생하는 문제를 수정했습니다 **데이터 업데이트** Google Cloud 외부 데이터베이스의 수신자 데이터로 수신자 스키마가 업데이트되지 않는 워크플로우 활동. (NEO-42343)
* Adobe Analytics 커넥터와 관련된 업그레이드 후 문제를 해결했습니다. (NEO-43318, NEO-38136)
* 업그레이드 후 &#39;VALUE_TO_CHANGE&#39;로 덮어쓴 CUID 문제를 해결했습니다. (NEO-43267)
* 중간 구성에서 중간 소싱 및 마케팅 인스턴스를 동기화할 때 오류가 발생하는 문제를 수정했습니다. (NEO-10432)
