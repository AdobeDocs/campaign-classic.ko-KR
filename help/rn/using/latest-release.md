...
<p>에서 중요한 알림을 만드는 방법을 알아봅니다 <a href="../../delivery/using/create-notifications-ios.md">세부 설명서</a>.</p>
</td> 
</tr> 
</tbody> 
</table>
...
* 이제 Adobe Campaign SDK는 푸시 알림용 Android 12 및 iOS 15를 지원합니다.
* Adobe Campaign은 이제 MySQL 8과 호환됩니다.
* Adobe Campaign은 이제 Windows 11와 호환됩니다.
* Adobe Campaign은 이제 Debian 11과 호환됩니다.

[캠페인 호환성 매트릭스](../../rn/using/compatibility-matrix.md#OperatingSystems)를 참조하십시오.

**개선 사항**

* Internet Explorer 11의 수명 종료에 따라 이제 콘솔에서 Adobe 서비스의 HTML 렌더링 엔진이 Edge Chromium을 사용합니다. 또한 이제 모든 클라이언트 콘솔 설치(Campaign Classic 7.3 빌드 버전)에 Microsoft Edge Webview 2 런타임을 설치해야 합니다. [자세히 보기](../../installation/using/installing-the-client-console.md)
* 안정성을 최적화하기 위해 Adobe Campaign의 데이터베이스 연결 관리가 개선되었습니다.
...
* 외부 데이터에서 데이터 보강 워크플로우 활동을 사용할 때 발생하는 다양한 문제를 수정했습니다. (NEO-38069)
* SAP Hana FDA 커넥터가 최신 SAP Hana 데이터베이스 버전(2.x)에서 사용할 수 있도록 업데이트되었습니다.
* teradata FDA 커넥터가 최신 Teradata 버전(17)에서 작동하도록 업데이트되었습니다.
* 20.2에서는 새 게재 및 게재 템플릿에 대해 iOS 게재에 대한 토큰 기반 인증 지원이 도입되었습니다. 7.2에서는 이전에 만든 게재 및 게재 템플릿 10,000개에 토큰 기반 인증 지원을 적용할 수 있도록 패치가 Postgrade에 추가되었습니다. 7.3에서 패치가 개선되었으며 제한이 제거되었습니다.

**패치**

* 사용자가 IMS 로그인 페이지의 크기를 조정할 수 없는 이전 빌드의 오류를 수정했습니다.
* 기존 인스턴스에 콘텐츠 관리자 패키지를 설치할 때 발생하는 오류를 수정했습니다.
* 에서 문제가 해결되었습니다. **캠페인** &quot;작업 진행 중&quot; 메시지가 계속 표시되는 메뉴.
* Adobe Analytics이 활성화되어 있으면 게재를 저장하지 않고 URL로 이메일을 보낼 때 URL에서 BID(Broadlog ID) 및 CID(캠페인 ID)를 제거하는 문제를 수정했습니다.
* 메시지 센터별 구성이 있는 인스턴스의 공개 리소스 폴더에서 이미지를 업로드할 때 발생하는 문제를 수정했습니다. 다음 오류 메시지가 표시됩니다. &quot;이미지를 추적 서버에 업로드할 수 없습니다.&quot;
* 구성 파일이 잘못된 경우 구성을 다시 생성할 때 시스템이 충돌하는 문제를 해결했습니다.
* 게재 지표가 올바르게 업데이트되지 않는 문제를 수정했습니다. (NEO-44827)
* 복잡한 쿼리를 사용할 때 업그레이드 후 오류가 발생하는 문제를 해결했습니다. (NEO-43648)
* webApps 미리 보기가 작동하지 않는 문제를 수정했습니다. (NEO-43242)
* 데이터 로드(파일) 활동이 있는 워크플로우에서 외부 대상 매핑 파일을 사용할 때 게재 준비가 실패하는 문제를 해결했습니다. (NEO-43691)
* 충돌을 야기할 수 있고 인스턴스를 완전히 다시 시작해야 하는 문제를 해결했습니다. (NEO-44645)
* 워크플로우 Heatmap 이 결과를 로드할 수 없는 문제를 해결했습니다. (NEO-43360)
* FDA 외부 커넥터를 사용할 때 연결 문제가 발생할 수 있는 문제를 해결했습니다. (NEO-42722)
* 주소 대체 및 제어 그룹 제외를 사용할 때 증명 문제를 해결했습니다. (NEO-39695)
* Snowflake 커넥터 문제로 인해 워크플로우 오류가 발생할 수 있는 문제를 해결했습니다. (NEO-46299)
* 개인화 블록의 잘못된 문자로 인해 클라이언트 콘솔을 동결할 수 있는 문제를 해결했습니다. (NEO-45761)
* 외부 데이터베이스로 Snowflake에 대한 외부 계정을 만들 때 연결 문제가 발생하는 문제를 수정했습니다. (NEO-45744)
* visibleIf 속성으로 보호된 테이블 정보를 표시하는 문제가 해결되었습니다. (NEO-37865)
* 게재 분석 단계 동안 &quot;$ 이(가) 정의되지 않음&quot; 오류 메시지를 표시할 수 있는 문제를 수정했습니다. (NEO-32940)
* 게재가 잘못된 eventType과 연결되던 문제를 수정했습니다. (NEO-45743)
* 간헐적인 코어 덤프로 인해 충돌이 발생할 수 있는 문제를 수정했습니다(NEO-30549).
* 게재에서 잘못된 HTML 코드를 사용할 때 충돌이 발생하는 문제를 수정했습니다. (NEO-40385)
* 관리자가 아닌 사용자가 **분석** 탭 을 포함합니다. (NEO-34025)
