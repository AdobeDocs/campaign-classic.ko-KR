...
1. 데이터베이스 백업:[데이터베이스 및 현재 설치](#back-up-the-database-and-the-current-installation)를 참조하십시오.
1. 플랫폼 마이그레이션:[Adobe Campaign v7 배포](#deploying-adobe-campaign-v7)를 참조하십시오.
1. 리디렉션 서버(IIS) 마이그레이션:[리디렉션 서버 마이그레이션(IIS)](#migrating-the-redirection-server--iis-)을 참조하십시오.
1. 서비스 다시 시작:[서비스 다시 시작](#re-starting-the-services)을 참조하십시오.
1. 이전 Adobe Campaign 버전을 삭제 및 정리합니다.[Adobe Campaign 이전 버전 삭제 및 정리](#deleting-and-cleansing-adobe-campaign-previous-version)를 참조하십시오.

## 서비스 중지 {#service-stop}

먼저 관련 모든 컴퓨터에서 데이터베이스에 대한 액세스 권한을 가진 모든 프로세스를 중지합니다.

1. 리디렉션 모듈(**webmdl** 서비스)을 사용하는 모든 서버는 중지해야 합니다. IIS의 경우 다음 명령을 실행하십시오.

   ```
   iisreset /stop
   ```

1. **mta** 모듈 및 해당 하위 모듈(**mtachild**)은 다음 명령을 사용하여 중지해야 합니다.
...
## 데이터베이스 및 현재 설치 {#back-up-the-database-and-the-current-installation} 백업
...
1. 다음 명령을 사용하여 **Neolane v5** 디렉토리를 백업하십시오.
   ...
   >[!IMPORTANT]
   >
   >예방 차원에서 **Neolane v5.back** 폴더를 압축하고 서버 이외의 안전한 위치에 저장하는 것이 좋습니다.
...
1. **config-`<instance name>`.xml**(**Neolane v5에서)을 편집합니다.** 폴더)를 사용하십시오. ************ 서비스가 자동으로 시작됩니다. 예를 들어 **autoStart**&#x200B;을 **_autoStart**&#x200B;로 바꿉니다.
...
1. Adobe Campaign 데이터베이스를 백업합니다.
1. 다음 명령을 사용하여 **Neolane v6** 디렉토리를 백업하십시오.
   ...
   >[!IMPORTANT]
   >
   >예방 차원에서 **Neolane v6.back** 폴더를 압축하고 서버 이외의 안전한 위치에 저장하는 것이 좋습니다.
...
1. **config-`<instance name>`.xml**(**Neolane v6에서)을 편집합니다.** 폴더)를 사용하십시오. ************ 서비스가 자동으로 시작됩니다. 예를 들어 **autoStart**&#x200B;을 **_autoStart**&#x200B;로 바꿉니다.
...
1. Adobe Campaign 데이터베이스를 백업합니다.
1. 다음 명령을 사용하여 **Adobe Campaign v6** 디렉토리를 백업하십시오.
   ...
   >[!IMPORTANT]
   >
   >예방 차원에서 **Adobe Campaign v6.back** 폴더를 압축하고 서버 이외의 안전한 위치에 저장하는 것이 좋습니다.
...
1. **setup.exe** 설치 파일을 실행하여 최신 Adobe Campaign v7 빌드를 설치합니다. Windows에서 Adobe Campaign 서버 설치에 대한 자세한 내용은 [이 섹션](../../installation/using/installing-the-server.md)을 참조하십시오.

   ![](assets/migration_wizard_1_7.png)

   >[!NOTE]
   >
   >Adobe Campaign v7는 기본적으로 **C:\Program Files\Adobe\Adobe Campaign v7** 디렉토리에 설치됩니다.
...
1. 다음 명령을 사용하여 처음 사용할 인스턴스를 시작합니다.
   ...
   >[!NOTE]
   >
   >이러한 명령을 사용하여 Adobe Campaign v7 내부 파일 시스템을 만들 수 있습니다.**conf** 디렉토리(**config-default.xml** 및 **serverConf.xml** 파일 사용), **var** 디렉토리 등.

1. **Neolane v5.back**, **Neolane v6.back** 또는 **Adobe Campaign v6.back** 백업 파일을 통해 각 인스턴스의 구성 파일 및 하위 폴더를 복사하여 붙여넣기(덮어쓰기)합니다( 마이그레이션하고 있는 버전에 따라 [이 섹션](#back-up-the-database-and-the-current-installation)을 참조하십시오.)
...
1. Adobe Campaign v7의 **serverConf.xml** 및 **config-default.xml** 파일에서 Adobe Campaign 이전 버전에서 수행한 특정 구성을 적용합니다. **serverConf.xml** 파일의 경우 **Neolane v5/conf/serverConf.xml.diff**, **Neolane v6/conf/serverConf.xml.diff** 또는 **Adobe Campaign v6/conf/serverConf.xml.diff** 파일을 사용하십시오.
...
이 단계에서는 IIS 서버를 중지해야 합니다. [서비스 중지](#service-stop)를 참조하십시오.
...
1. **iis_neolane_setup.vbs** 스크립트를 실행하여 이전에 만든 가상 디렉터리에 Adobe Campaign 서버가 사용하는 리소스를 자동으로 구성합니다.
   ...
   * **[!UICONTROL OK]** 을 클릭하여 스크립트 실행을 확인합니다.

      ![](assets/s_ncs_install_iis7_parameters_step2_7.png)

   * Adobe Campaign v7용으로 이전에 만든 웹 사이트의 수를 입력하고 **[!UICONTROL OK]** 을 클릭합니다.
...
1. **Neolane v5.back** 폴더의 이름을 **Neolane v5**&#x200B;로 변경합니다.
...
1. **Neolane v6.back** 폴더의 이름을 **Neolane v6**&#x200B;로 변경합니다.
...
1. **Adobe Campaign v6.back** 폴더의 이름을 **Adobe Campaign v6**&#x200B;로 변경합니다.
